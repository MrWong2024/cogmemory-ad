...
const CORE_SCENARIOS: readonly B12ScenarioDefinition[] = [
  {
    ...
    routes: [
      ...
      route({
        ...
        expectedFixtureOwnedMutationClass: 'none',
        postBrowserFinalStateContract:
          'historical fallback remains exact with no repeat entry or guessed lock facts',
        requiresIndependentSession: false,
      }),
      route({
  ...
  },
  {
    ...
    routes: [
      ...
      route({
        ...
        expectedFixtureOwnedMutationClass: 'none',
        postBrowserFinalStateContract:
          'two independent doctor Sessions receive first-lock then alreadyLocked success while only the secondary lock persists',
        requiresIndependentSession: true,
      }),
    ],
  },
