...
import {
  type PatientAdministrationStepConfig,
  ScaleVersion,
...
function toLegacyMmsePatientAdvanceSteps(
  steps: PatientAdministrationStepConfig[],
): PatientAdministrationStepConfig[] {
  return structuredClone(steps).map((step) =>
    MMSE_PATIENT_ADVANCE_LEGACY_STEP_KEYS.has(step.stepKey)
...
describe('ScaleCatalogService', () => {
  ...
  it('atomically migrates only the exact legacy MMSE patient-advance predecessor', async () => {
    const definitionId = new Types.ObjectId();
    const versionId = new Types.ObjectId();
    const seed = seedDataService.getScaleSeedByCode('mmse');

    if (!seed?.version.patientAdministrationSteps) {
      throw new Error('Expected MMSE patient administration seed');
    }

    const legacySteps = toLegacyMmsePatientAdvanceSteps(
      seed.version.patientAdministrationSteps,
    );
    const legacyVersion = {
      _id: versionId,
      scaleDefinitionId: definitionId,
      ...structuredClone(seed.version),
      patientAdministrationSteps: legacySteps,
    };
    const patchedVersion = {
      ...legacyVersion,
      patientAdministrationSteps: structuredClone(
  ...
  it('rejects the legacy MMSE predecessor with any fourth advanceBy drift', async () => {
    const definitionId = new Types.ObjectId();
    const seed = seedDataService.getScaleSeedByCode('mmse');
  ...
  it('rejects the legacy MMSE predecessor with a patientText drift', async () => {
    const definitionId = new Types.ObjectId();
    const seed = seedDataService.getScaleSeedByCode('mmse');
    ...
    const readingStep = legacySteps.find(
      (step) => step.stepKey === 'mmse-reading-command',
    );
    if (!readingStep) {
      throw new Error('Expected MMSE reading step');
    }
    readingStep.patientText = 'Conflicting clinical instruction';
    definitionModel.findOneAndUpdate.mockReturnValue(
      createExecQuery({
        _id: definitionId,
        code: 'mmse',
  ...
  it('converges when another caller wins the exact MMSE patient-advance migration', async () => {
    const definitionId = new Types.ObjectId();
    const versionId = new Types.ObjectId();
    const seed = seedDataService.getScaleSeedByCode('mmse');
    ...
    const legacyVersion = {
      ...
      scaleDefinitionId: definitionId,
      ...structuredClone(seed.version),
      patientAdministrationSteps: toLegacyMmsePatientAdvanceSteps(
        seed.version.patientAdministrationSteps,
      ),
    };
