...
describe('ClinicalReportPublicMapper', () => {
  ...
  it('supports B12-16 and B12-18 lockedAt-first read semantics', () => {
    const complete = createReport();
    complete.status = 'confirmed';
    complete.lockedAt = now;
    complete.lockedBy = id;
    expect(mapper.toPublicReport(complete)).toEqual(
      expect.objectContaining({
        status: 'confirmed',
        lockedAt: now,
        isFinal: true,
    ...
    expect(mapper.toPublicReport(complete).lock).toEqual(
      expect.objectContaining({ lockedAt: now }),
    );

    const incomplete = createReport();
    incomplete.status = 'confirmed';
    incomplete.lockedAt = now;
    incomplete.lockedBy = null;
    const response = mapper.toPublicReport(incomplete);
    expect(response.lockedAt).toEqual(now);
    expect(response.lock).toBeNull();
  });

  it('documents the B12-17 and B12-19 public-read contract gap without leaking inconsistent A22 metadata', () => {
    const auditWithoutTopLevel = createReport();
    auditWithoutTopLevel.status = 'confirmed';
    if (auditWithoutTopLevel.metadata) {
      auditWithoutTopLevel.metadata.a22Lock = {
        version: 1,
        lockId: '55555555-5555-4555-8555-555555555555',
        lockedAt: now,
        lockedBy: id,
        lockedByName: 'B12 P1 synthetic doctor',
        lockedByRole: 'doctor',
        lockNote: 'B12 P1 synthetic lock note',
      };
    }
    expect(mapper.toPublicReport(auditWithoutTopLevel).lock).toBeNull();

    const mismatch = createReport();
    mismatch.status = 'confirmed';
    mismatch.lockedAt = now;
    mismatch.lockedBy = id;
    if (mismatch.metadata) {
      mismatch.metadata.a22Lock = {
        version: 1,
        ...
        lockedByName: 'B12 P1 synthetic doctor',
        lockedByRole: 'doctor',
        lockNote: 'B12 P1 synthetic mismatch',
      };
    }
    const response = mapper.toPublicReport(mismatch);
    expect(response.lockedAt).toEqual(now);
    expect(response.lock).toBeNull();
    expect(JSON.stringify(response)).not.toContain('synthetic mismatch');
  });

  it('supports B12-70 by preserving independent lock and archive timestamps', () => {
    const report = createReport();
    ...
    report.archivedBy = id;
    const response = mapper.toPublicReport(report);
    expect(response.lockedAt).toEqual(now);
    expect(response.archivedAt).toEqual(archivedAt);
    expect(response.lockedAt).not.toEqual(response.archivedAt);
  });
