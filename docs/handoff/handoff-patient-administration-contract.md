...
- `ItemResponse` 已有独立 revision / CAS、草稿、分步结果、提示后表现、计时、证据引用和提交写屏障；医生整体提交前即可通过现有 A14 草稿链人工录入和修订，但患者原始事实、ASR 候选或医护观察不得自动写入为正式答案。
...
- 医生负责复核原始作答、录音、ASR 候选、书写绘图、医护观察和异常，通过现有 `ItemResponse` 草稿链形成或修订结构化答案，并在 readiness 满足后执行 A16 整体正式提交；既有评分和报告链继续在正式提交之后运行。
...
| 结构化业务数据 | MongoDB | 量表版本与步骤合同、资产逻辑键、展示和播放规则、OSS 对象键与元数据、患者原始作答、ASR 候选、医护观察、医生复核形成的 `ItemResponse` 草稿、整体提交、评分、报告和审计事实 | 不保存图片或录音二进制，不保存 Base64 文件；患者候选、医生复核草稿与整体提交后的正式结果必须可区分 |
...
表中“技术重播”均指医护发起并记录原因；“医生确认”表示医生依据相关事实通过现有 `ItemResponse` 草稿链复核，并在整份量表满足 readiness 后执行 A16 整体正式提交，不表示确认后才创建或复制 `ItemResponse`。所有明确口头回答的步骤默认形成该步骤短录音，不进行整场持续录音。
...
2. ASR 机器候选文本。
3. 医护动作观察、实物操作结果、控制动作和影响因素；正常现场观察可以在后续复核阶段形成正式记录，不要求逐题同步写入才能推进患者 Session。
4. 医生复核过程中人工形成、修订并拟提交的结构化答案；该层通过现有 `ItemResponse` 草稿承载。
5. 医生对满足既有 readiness 的整份 `ScaleInstance` 执行 A16 显式整体提交后形成的正式提交结果。

第 1～3 层不得自动成为 `ItemResponse` 正式答案；患者完成、ASR 候选、原始录音、医护观察、媒体上传成功、自动评分或系统规则均不得自动写入正式答案。医生在第 4 层通过现有 A14 revision / CAS 草稿 PATCH 受控录入或修订答案，复用 step、prompt、`operatorNote` 等既有字段，并按既有规则 `markAsAnswered`。第 5 层不创建或复制第二份答案；全部 `ItemResponse` 满足 submission readiness 后，医生使用现有 A16 `submit(confirm=true)` 对整份 `ScaleInstance` 做整体正式提交。提交成功后，这批既有 `ItemResponse` 作为该次已提交 `ScaleInstance` 的正式作答结果，再进入既有评分、认知域和报告链。

“谁负责临床判定”与“谁推动 patient Session 到下一步”是两个独立职责。正常患者主链由患者端连续推进，医护在现实中观察和辅助；后续复核才把观察结论形成正式记录。暂停、接管、重做、technical replay、重签和终止只在异常或控制需要时使用。
...
医生复核采用异常优先：录音 / 转写差异、提示后回答、书写绘图、动作观察、暂停 / 接管 / 换设备 / 重做、上传降级、缺失、不一致、影响因素和无法完成原因优先展示。无异常且无歧义的客观步骤可汇总确认，但正式提交前仍须医生显式整体确认。正常首次复核形成 `ItemResponse` 草稿不强制逐题填写“修改原因”，也不要求为形式写入无业务价值的 `operatorNote`；对已有草稿作实质纠正、与原始证据存在需要解释的明显差异、临床判断需要额外解释，或既有业务合同本来要求说明时，记录必要说明。继续复用现有 `operatorNote`、submission actor / time、Audit 和提交留痕，不新增必填原因字段或审计系统。
...
- ASR 失败绝不成为现有提交屏障的新增阻断条件；录音降级后的答案仍须由医生通过现有 `ItemResponse` 草稿链复核，并经 A16 整体正式提交后成为正式结果。
...
- 准备练习隔离、短期患者会话、六位一次性进入码十分钟、单设备、两小时绝对有效期、same-device staff Session 失效与重新认证、cross-device staff Session 保留、服务端权威步骤和安全退出。
- 患者正常题目主链连续推进，医护现场观察与后续复核记录解耦；双设备不等于双写者，独立患者 / 独立 `ScaleInstance` 正常并行，同一评估不默认多人实时协同编辑。
- 患者原始事实、ASR 候选、医护观察、医生复核草稿和整体正式提交结果的五层语义；第 4 层由现有 `ItemResponse` 草稿承载，第 5 层通过现有 A16 整体提交使同一批 `ItemResponse` 成为正式作答结果，不创建第二套答案。
...
- `ReviewAnswer`、`DoctorAnswer`、`PendingAnswer`、`FormalAnswer` 等第二套答案集合，独立 Answer / Attempt / Capture / Review、确认后复制到 `ItemResponse`、新 overall-confirm endpoint、新审核状态机、事件溯源或通用 AI Provider 平台。
