...
- `cross_device` 推荐用于 `supervised_patient_input`，但不是强制唯一方式；同设备与跨设备都必须遵守同一患者原始事实、医生复核和正式结果边界。
...
same-device 中重新认证是有意接受的简单安全边界，也是共享设备从 patient 身份重新取得 staff 权限的明确授权动作。允许为此多一次登录；不得为了减少一次重新认证而建设同一浏览器内 staff + patient 双身份长期共存、隐藏身份、自动恢复 staff Session 或复杂安全解锁状态机。

`supervised_patient_input + cross_device` 中，staff 设备保留 staff Session，patient 设备持有 patient Session；患者端负责正常题目呈现和患者操作，医护端主要负责准备、现实观察、必要辅助和异常控制。双设备只是角色分工，不等于双写者；正常 happy path 尽量不要求医护端逐题写入同一 Session。设备屏幕大小不决定角色，具体屏幕分配与响应式合同以 frontend design baseline 为准。
...
- 同一浏览器 staff + patient 双身份长期共存、隐藏身份、自动恢复 staff Session、复杂安全解锁状态机，以及同一评估的多人实时协同编辑。
