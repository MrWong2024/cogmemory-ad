...
表中“技术重播”均指 staff 授权并记录原因；授权后患者端可显式触发一次获授权的 `stimulus` 播放。“医生确认”表示医生依据相关事实通过现有 `ItemResponse` 草稿链复核，并在整份量表满足 readiness 后执行 A16 整体正式提交，不表示确认后才创建或复制 `ItemResponse`。所有明确口头回答的步骤默认形成该步骤短录音，不进行整场持续录音。
