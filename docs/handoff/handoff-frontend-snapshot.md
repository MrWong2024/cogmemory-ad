...
- 受监督患者施测端：尚无面向患者的一步一屏施测页面、覆盖平板与具备触摸功能电脑大屏的标准触控设备适配、患者短期受控施测会话和安全进入、分层的测量刺激与普通指导、患者录音与基础 ASR 人工降级、医生及时状态查看与控制权接管、患者端服务端权威恢复，以及患者原始采集经医生复核后形成正式 `ItemResponse` 的专用流程；患者端与医护监管端协同完成完整 MMSE / MoCA 的多模态编排尚未实现。现有量表执行页属于医护侧工作流，不能表述为上述患者端已经落地。
- 当前尚未确定同设备工作模式交接、跨设备安全配对、具体同步时效和刷新策略，以及原始录音、绘图与中间媒体的保留、删除和正式备份边界；这些缺口不表示二维码、强实时协作、特定传输技术、全部固定录音或永久保存全部原始证据已经成为未来实现合同。
- 患者：编辑、删除、归档、合并。
- 访视：编辑、删除、完整状态流转。
