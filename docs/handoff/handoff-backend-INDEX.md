...
当前内容记录后端公共底座、已进入真实代码阶段的 `scales` 量表定义模型底座、MMSE / MoCA seed 与安全目录 / 按需物化能力、`patients` / `assessments` 患者、访视、量表实例和 `ItemResponse` 运行时底座、访视详情与量表执行初始化工作流、`media` / `scoring` / `cognitive-domains` / `reports` 内部底座、`users` / `auth` 认证与角色权限底座、A11 三个认证 API、A12 五个患者 / 访视 API，以及 A13 三个评估初始化前置 API；未实现的业务能力仍只能标记为待后续阶段确认。
...
- `AssessmentsModule` 当前包含 `AssessmentVisit` / `ScaleInstance` / `ItemResponse` Schema、既有内部读取能力、`AssessmentExecutionService`、`AssessmentScaleWorkflowService`，以及 `AssessmentVisitsController` 的访视列表、创建、详情和量表实例初始化公开 API。
...
- A12 五个接口与 A13 三个接口均显式绑定 `SessionAuthGuard`、`RolesGuard` 与 `@Roles('admin', 'doctor', 'nurse', 'research_assistant')`；未注册全局 Guard。
- users me / 用户管理 API、权限菜单、患者编辑 / 删除 / 归档、访视编辑 / 删除 / 状态流转、前端 A13 页面接入、作答查询 / 保存 / 提交、媒体上传 / 下载 / 签名 URL、全量数据库 seed runner、量表配置编辑、计分触发、认知域计算触发、MMSE / MoCA 专用规则执行、报告生成、医生确认写库、PDF 导出、疾病诊断、AI、科研导出等业务能力仍未实现。
- A13 验证结果为 26 个单元测试套件、252 个测试通过；真实 HTTP E2E 为 2 个测试套件、14 个测试通过，使用隔离 `cogmemory_ad_test` 数据库、fake / stub 外部服务和脱敏人工数据。
