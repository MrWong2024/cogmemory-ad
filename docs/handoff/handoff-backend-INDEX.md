...
- 当前公开 API 为 `GET /health`、`POST /auth/login`、`POST /auth/logout`、`GET /auth/me`；`scales`、`patients`、`assessments`、`media`、`scoring`、`cognitive-domains`、`reports`、`users` 均未新增 Controller 或公开业务接口。
- users me / 用户管理 API、前端登录页、认证态联动、权限菜单、真实患者建档流程、访视管理接口、公开评估执行业务接口、作答提交、媒体上传 / 下载 / 签名 URL、数据库 seed runner、seed 写库、公开 MMSE / MoCA 配置查询接口、计分触发、认知域计算触发、MMSE / MoCA 专用计分规则执行、MMSE / MoCA 专用认知域规则执行、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、PDF 导出、疾病诊断、AI、科研导出等业务能力仍未实现。
