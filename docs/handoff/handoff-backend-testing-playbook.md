...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 与 B8 `core-workflow` 已完成，B8 整体仍未完成且下一阶段为 `resilience-security`，B9–B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B8 | `scripts/b8-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 9 scenarioKey；`core-workflow` 39 audit ID，`resilience-security` 21 audit ID | `core-workflow` 39 项已完成并清理；`resilience-security` 尚未执行 |
...
- Batch C / B7 已通过组合证据完成。B8 `core-workflow` 的 39 项已完成，core post-browser verify 通过且双次 cleanup 均为 `residualCount=0`；B8 整体仍未完成，下一阶段为 `resilience-security`，B9–B10 尚未启动。
