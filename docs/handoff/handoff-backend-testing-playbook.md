...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 已完成且 B8–B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
Batch C / D fixture 的验证意图、前置状态和关键边界必须从 frontend testing playbook 的 B7–B15（含 B14.1）待验合同设计；B7 现有资产不覆盖 B8–B15，不得从已完成 Batch A / B、B7 的旧 namespace、操作流水或中间失败状态反推。
...
- Batch C / B7 已通过组合证据完成：原完整验收的 39 项、完整 post-browser verify 与双次 cleanup 事实继续有效；B7-38 修复后的 390×844、768×900、1280×720 定向回归通过，每次 latest GET 1 次、compute POST 0 次，Browser 前后 prepared verify 与双次 cleanup 通过。只读回归 namespace 的 prepared 状态不适用 post-browser 写终态验证，不构成当前阻断；B8–B10 尚未启动。
