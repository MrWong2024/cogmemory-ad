...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 已完成，B8 的 fixture 与两个独立验收 profile 已准备完成，但 B8 Browser 验收尚未开始，B9–B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B7–B10 | B7 已完成；B8 fixture-ready；B8 Browser 与 B9–B10 尚未启动 | B8 的 60 项已唯一映射到 `core-workflow` / `resilience-security`，两个 profile 的 prepared / post-browser / cleanup 合同相互独立；下一阶段为 `core-workflow` Browser 验收 |
...
| Batch C / B8 | `scripts/b8-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 9 scenarioKey；`core-workflow` 39 audit ID，`resilience-security` 21 audit ID | fixture 与分批合同已准备；Browser 验收尚未开始 |
...
- Batch C / B7 已通过组合证据完成。B8 fixture 与 `core-workflow` / `resilience-security` 两个独立 profile 已准备完成，但 B8 Browser 验收尚未开始；下一阶段为 `core-workflow` Browser 验收，B9–B10 尚未启动。
