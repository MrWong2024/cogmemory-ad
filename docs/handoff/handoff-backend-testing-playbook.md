...
| Batch D / B13 | `B13-P0-contract-evidence` 继续完成且合同前置证据 `gap=0`；`B13-U01` 三种持久状态 Browser 验收已完成，`B13-U02`、`B13-U03` 保持待验，状态 `passed=1`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`；权威明细见 frontend testing playbook 10.1 |
...
- B13：已重新生成 `B13-U01`～`B13-U03` 3 个活动用户场景；`B13-U01` 三种持久状态 Browser 验收已完成，状态为 `passed=1`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`，`B13-U02`、`B13-U03` 因其独立真实写入与恢复风险尚未执行而保持 `pending`。B13-P0-A 已完成 G1 动态覆盖与 G2 分层校准，B13-P0-A2 已完成 G3 双 Session 真实 HTTP 并发绿色覆盖；G4、G5 已由同一份 frontend Node-only contract spec 覆盖，`B13-P0-contract-evidence` 继续完成且合同前置证据 `gap=0`，下一阶段为 `B13-U02`。后端合同、pure/static、防御证据、P0～P4 职责及原 1～116 迁移的权威明细见 frontend testing playbook 10.1；本手册只维护后端证据分层、数据库、fixture、cleanup 与状态摘要。
