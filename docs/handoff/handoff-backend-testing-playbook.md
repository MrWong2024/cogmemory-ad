...
| A29 / WP-03 backend | 实现已落地；后端验证闭合 pending；阻断性 `gap=1` | 同值保存、`timing=null`、legacy GET 零写入已补强；A14 PATCH / A16 submit 相邻生命周期并发写保护违反锁定合同 | B18 暂不作为下一实施阶段 |
...
| A29 / WP-03 backend | `item-response-timing.spec.ts` + `item-response-draft.service.spec.ts`：2 suites / 69 tests；`item-response-draft.e2e-spec.ts` + `media-evidence.e2e-spec.ts`：2 suites / 20 tests；新增同值保存及 T1/T2 服务端时间、`timing=null` 持久复位、legacy timing 公开 GET 零写入；既有 legacy、完整 timing 转换、CAS、stale / 同 revision 并发冲突、protected fields 与媒体版本隔离仍保留 | 确定性 A14 PATCH / A16 submit 诊断使用两个真实公开请求；确认产品 gap 后按任务合同删除临时测试块，不执行 Browser、fixture profile 或 Browser verifier | A29 实现已落地，但后端验证闭合 pending；阻断性 `gap=1`；不得进入 B18 | 本行及 roadmap / A29 maps |

B12～B15 的 Browser 操作、页面文案、keyboard、viewport、Storage 和活动场景结果只在 frontend testing playbook 的语义索引中维护；本手册不复制执行流水或历史迁移表。

A29 原实现代码态曾实际通过 backend `lint`、`typecheck`、`build`、完整 unit（90 suites / 826 tests）与完整 HTTP E2E（26 suites / 146 tests）；这些既有套件未覆盖下述相邻生命周期窗口。本次独立验证保留态实际通过 `item-response-draft.service.spec.ts`（1 suite / 28 tests）与 `item-response-draft.e2e-spec.ts`（1 suite / 14 tests），HTTP E2E 建连后逐字确认实际库为 `cogmemory_ad_test`，Storage 为 fake、LLM / SMS 为 stub；未加载 browser acceptance 配置，未启动 Browser 或长期保留 namespace。

A29 阻断性产品 gap 的可达路径与确定性证据如下：同一个合法 doctor Session 先通过既有 A14 / A15 公开流程把真实 MMSE 实例准备到 submission readiness=true；随后向一个 answered ItemResponse 发起公开 A14 PATCH，并在其完成 Patient / Visit / ScaleInstance / ItemResponse 前置读取、到达 ItemResponse `findOneAndUpdate().exec()` 原子写之前用一次性闩锁暂停。闩锁期间，真实 `POST /patients/:patientId/visits/:visitId/scale-instances/:scaleInstanceId/submit` 返回 200，MongoDB ScaleInstance 已为 `completed` 且只形成一次新提交事实；释放闩锁后，原 PATCH 实际返回 200，而非合同要求的 409 / `SCALE_INSTANCE_NOT_EDITABLE`。MongoDB ItemResponse 完整 raw snapshot 已变化：`draftRevision` 从 1 增至 2，`draftSavedAt`、`updatedAt` 和目标草稿字段均被改写；ScaleInstance 保持 `completed`，`score_results`、`cognitive_domain_results`、`clinical_reports` 计数均为 0。该结果是已确认的跨生命周期并发写保护产品 gap，不是 `unknown`；临时诊断测试块已按任务合同删除，风险不因此关闭。下一阶段必须是编号和方案尚未锁定的独立后端合同修复，B18 暂不进入。
