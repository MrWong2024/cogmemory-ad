...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 已进入 B11-A fixture 阶段但尚未完成，下一阶段为 B11-B `core-workflow` Browser；Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
当前最终代码态门禁证据为：lint 0 errors / 0 warnings，typecheck 0 errors，build 通过，full unit 89 suites / 761 tests，B11 fixture 定向 E2E 1 suite / 10 tests，full E2E 26 suites / 134 tests；E2E 实际连接 `cogmemory_ad_test`。这些是当前测试资产代码态的证据，不代表后续代码修改可以免于重跑，也不代表任何 B11 Browser 项通过。
...
| Batch D / B11 | `scripts/b11-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 20 route / 58 Browser audit ID，`resilience-security` 4 scenarioKey / 9 route / 11 Browser audit ID；B11-70 为 static-gate | B11-A fixture ready；Browser 尚未启动，70 项均未最终关闭；下一阶段为 B11-B `core-workflow` Browser |
...
B11 fixture CLI 固定使用 `browser_acceptance` / `cogmemory_ad_browser_test` / db_admin / `dbOwner`，只允许 prepare、prepared/post-browser verify、replace、两个 allowlist Stage、runtime descriptor 与 cleanup；未来 Playwright runner 数据库用途仍为 `none`，未来 Browser backend 仍使用 app / `readWrite`。两个 Stage 仅为 `confirmation-conflict-touch` 与 `forbidden-confirm-role`，均要求 profile、namespace、scenario、route、transition、role 命中固定合同并先通过 prepared verify。Runtime 只输出固定字段白名单，不输出密码、URI、Cookie、Session、独立动态 ID 或动态 path；两个 profile 的实际 prepare、prepared verify、replace、再次 prepared verify、runtime 生成/删除、Stage 冒烟与双次 cleanup 均已通过，cleanup 1 为 `residualCount=0` / `matched=true`，cleanup 2 为 `residualCount=0` / `matched=false`。本阶段未启动 Browser、production frontend 或 Browser test backend，未执行 post-browser Browser 终态验收。
...
- Batch D 已进入 B11-A fixture 阶段但尚未完成。B11 fixture ready，`core-workflow` 58 项、`resilience-security` 11 项、B11-70 static-gate 1 项均有唯一 owner；Browser 尚未启动，70 项均未最终关闭，下一阶段为 B11-B `core-workflow` Browser。B14.1 的行为等价 Browser 回归仍属于待验合同，不因 B16 / WP-02 已完成而自动覆盖；不存在可填写的新 evidence commit。
