...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 与 B8 已完成，B8 的 `core-workflow` 39 项与 `resilience-security` 21 项共 60 项全部闭环；B9-B5 已使用全新 namespace 完整重跑 core，B9-01–B9-31、B9-33–B9-38 共 37 个 active 项全部通过，B9-32 保持唯一 `obsolete`，post-browser verify 通过且双次 cleanup 均为 `residualCount=0`。B9-B `core-workflow` 已完成；B9 整体仍未完成，下一阶段为 `resilience-security`，B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B7–B10 | B7、B8 与 B9-B `core-workflow` 已完成；B9 整体仍未完成；B10 尚未启动 | B9-B5 core 为 37 active pass + B9-32 obsolete；post-browser verify 通过且双次 cleanup `residualCount=0`；下一阶段为 `resilience-security` |
...
| Batch C / B9 | `scripts/b9-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 10 业务 scenarioKey；`core-workflow` 19 route / 38 audit ID（37 active + B9-32 obsolete），`resilience-security` 11 route / 14 active audit ID | B9-B5 已完成 core：37 active pass + B9-32 obsolete，post-browser verify 与双 cleanup 通过；`resilience-security` 与 B9 整体仍未完成 |
