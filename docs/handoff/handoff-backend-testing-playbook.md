...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 与 B8 已完成，B8 的 `core-workflow` 39 项与 `resilience-security` 21 项共 60 项全部闭环；B9 `core-workflow` 验收发现 fixture seed 基线和 `local_write_gate` 合同缺陷，并受缺失稳定 B9 fixture 密码阻断，Browser 生命周期未开始，B9 仍未完成；B9 `resilience-security` 与 B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B7–B10 | B7、B8 已完成；B9 `core-workflow` 验收受 fixture 合同缺陷和本地凭据缺失阻断，仍未完成；B10 尚未启动 | B9 定向 E2E 因 prepare 改变初始全局 seed hash 而失败；`local_write_gate` 五条 route 不能观察声明的 primary blocker；`resilience-security` 尚未执行 |
...
| Batch C / B9 | `scripts/b9-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 10 业务 scenarioKey；`core-workflow` 19 route / 38 audit ID，`resilience-security` 11 route / 14 audit ID | 合同代码已建立，但定向 E2E 存在 seed 基线失败，`local_write_gate` 五条 route 不能验证 primary blocker；B9 仍未完成 |
...
- B9 `core-workflow` 未完成：定向 E2E 的 suite 初始 seed hash 在 prepare 自动物化量表 seed 后改变，导致 4 个稳定断言失败；`local_write_gate` 五条 route 均只能观察更早的实例 / 来源评分依赖门禁；本地 Browser 配置没有稳定的 `B9_FIXTURE_PASSWORD`。修复 fixture 与配置前不得把 prepared、Browser 或 post-browser 阶段写成通过；`resilience-security` 与 B10 均尚未启动。
