...
| Batch D / B15 | 已从原 10 组候选重新生成 `B15-U01`、`B15-U02` 两个活动场景；状态 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`。P0 只读审计确认 4 个 gap：persisted resume 的 correctionId 安全展示合同、A25 真实双 Session HTTP 并发、correction 专属 frontend pure/static、Correction Action recovery；尚未执行 Browser |
...
- B15：已按当前 A25 合同独立重新生成两个且仅两个活动场景：`B15-U01` 首次真实版本化更正与当前会话切换，`B15-U02` 显式恢复与网络不确定结果；状态 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`。原 10 组只作反向迁移输入，不再是活动清单。当前后端证据直接或分层覆盖角色/DTO/ownership/readiness、首次成功、安全 mapper、start/create/record/complete、completed 真实 HTTP 顺序幂等、V2/V3 生命周期、protected facts 与 defensive lineage；但 `backend/test/clinical-report-correction.e2e-spec.ts` 没有 doctor/admin 两个独立合法 Session 对同一 report/expectedUpdatedAt 的真实 HTTP/DB 并发，顺序幂等、mock atomic miss、workflow 顺序调用和 fixture helper 均不能替代，记为 P0-G1。P0-G2/G3 分别是 correction 专属 frontend pure/static 和 Action recovery 的当前直接证据缺口；P0-G4 是新 U02 要求显示原 correctionId/correctionNo，但当前 Panel/Summary 只显示 correctionNo/actor/time 并明确隐藏 correctionId 的实现/安全展示合同不一致。
  ...
  下一步先关闭 P0-G1～G4，再执行 U01；随后执行 U02，最终只执行一次 P3 discovery/lint/typecheck/build/依赖/路由/脱敏/轻量 smoke。详细 C01～C23 合同表、B16 复用表、7 个当前 evidence gap（其中 4 个 P0 gap）、supplemental compatibility/defensive、generic conflict 结论和原 10 组迁移均以 frontend testing playbook 10.4 为唯一权威来源，本手册不复制。
