...
| Batch D / B11–B15 | B11 已完成；B12-A fixture 已完成；Batch D 尚未完成 | B11 的 70 项保持闭环；B12 已建立 62 core + 23 resilience Browser ID 和 3 static-gate ID 的 fixture 合同，但 88 项均未最终关闭；下一阶段为 B12-B `core-workflow` Browser 验收 |
...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-A fixture、verifier、runtime 与 CLI lifecycle 已闭环；Browser 尚未启动，85 个 Browser ID 与 3 个 static-gate ID 均未最终关闭；下一阶段为 B12-B `core-workflow` Browser 验收 |
...
B12 fixture 固定拆分为两个互不依赖的 profile，且每个 profile 都建立 doctor、admin、nurse、research_assistant、system 五个精确角色。`core-workflow` 覆盖 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID；`resilience-security` 覆盖 6 个 scenarioKey / 11 条 route / 23 个 Browser audit ID；B12-86–B12-88 是无 Browser route 的 3 个 static-gate。88 个稳定 ID 均有唯一 primary owner，85 个 Browser ID 均恰有一条 route，所有 route 各自拥有独立 Patient、Visit、ScaleInstance、ClinicalReport 与 marker，不共享可写 report，也不跨 profile 复用资源。core prepared 状态矩阵为 draft 1、pending 1、confirmed unlocked 10、quality blocked 1、confirmation missing 1、Visit locked V1 1、Visit voided V1 1、confirmed locked 5、historical locked fallback 1；resilience 为 confirmed unlocked 11。

B12 CLI 只允许 prepare、prepared / post-browser verify、replace、五个 allowlist Stage、runtime descriptor 与 cleanup。五个 Stage 固定为 `lock-conflict-touch`、`lock-conflict-latest-locked-touch`、`lock-audit-unavailable`、`lock-metadata-unsupported`、`forbidden-lock-role`；每次都校验 profile、namespace、scenario、route、transition、role 与 transition-aware 进度，重复执行只返回幂等结果，任意扩展或非法中间态均拒绝。四条 controlled public-read boundary 以合法持久化报告为基线，未来只允许单次初始 public read 分别改变 `isFinal`、top-level `lockedAt`、`lock` summary 或 `lock.lockedAt`；数据库、其他公开字段、后续 read 与写入合同均不得改变。runtime 仅写 version、batch、profile、scenarioKey、routeKey、primaryRole、可选 secondaryRole、对应 login identifier 与 navigation path；目录、basename、existing target、symlink、原子写入和权限均受门禁，CLI stdout 不输出动态账号或路径。

B12 prepared verifier 只读校验合法报告状态、真实 ReportsService summary 与 public mapper、A20/A21/A22 事实、资源矩阵、profile isolation、canonical seed 与 baseline hash；post-browser verifier区分 zero-write、doctor/admin 单次 A22 lock、already-locked readonly、两个 conflict touch + 单次产品 lock，以及三个 fixture-only mutation。定向 E2E 已证明它接受全部合法状态和 mutation class，并拒绝漏写、多写、错误 actor、第二个或非法 A22、A23–A25、narrative / snapshot、Patient / Visit / ScaleInstance、marker、seed 与跨 namespace / profile 漂移。最终门禁为 lint、typecheck、build、89 suites / 765 unit tests、B12 定向 E2E 1 suite / 11 tests、full E2E 27 suites / 148 tests，standard_test 实际为 `cogmemory_ad_test` / app / `readWrite`。

B12 两个 profile 的 CLI 冒烟均在独立进程中只加载 `.env.browser-acceptance`，实际使用 `browser_acceptance` / `cogmemory_ad_browser_test` / db_admin / `dbOwner`，未叠加 `.env.test`；未来 Browser backend 固定使用同库 app / `readWrite`，未来 Playwright runner 数据库用途为 `none`。两个 profile 均完成 prepare、prepared verify、replace、再次 prepared verify、runtime 生成与删除、Stage 首次与幂等执行和双次 cleanup；resilience 额外逐一完成三个代表性 Stage，Stage 之间显式 replace。cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`；canonical seed、其他 namespace 与非 namespace 数据不变，namespace、Session、runtime 与 manifest 无残留。本阶段没有启动 production frontend、Browser test backend、Playwright 或 Chromium，也没有执行真实 B12 Browser Action；B12-86–B12-88 未关闭，不填写不存在的 evidence commit。
...
- Batch D 的 B11 `core-workflow` 58 个 Browser audit ID、`resilience-security` 11 个 Browser audit ID 与 B11-70 static-gate 均已通过，共 70 项完成，B11 已完成。B12-A fixture 已完成：62 core + 23 resilience Browser ID 与 3 static-gate ID 均已建立唯一 owner，fixture、verifier、runtime、Stage 与双 cleanup 已闭环；B12 Browser 尚未启动，88 项均未最终关闭。Batch D 仍未完成；下一阶段为 B12-B `core-workflow` Browser 验收；不存在可填写的新 evidence commit。
