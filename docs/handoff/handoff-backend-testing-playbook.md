...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 的 B11 `core-workflow` 58 Browser 项、`resilience-security` 11 Browser 项与 B11-70 static-gate 1 项均已通过，共 70 项完成，B11 已完成。B12-B4 已基于指定基线用全新 namespace 完整执行 22 条 core route，历史结果仍为 0 route pass / 22 route fail、0 audit pass / 62 audit fail；完整 post-browser verify 与双次 cleanup 通过。B12-B5 已完成登录前认证分区、doctor / admin 多正文 section 与 denied-role system Session logout 的代码级 Playwright 资产修复并通过 synthetic 验证，但未形成新产品 Browser 证据。当前分类为产品缺陷 0、fixture 缺陷 0；两项 Playwright 验收资产缺陷与一项 Session 收口缺陷已完成代码级修复，等待全新 namespace 完整重跑确认。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-86–B12-88 未关闭。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11 已完成；B12-B5 已完成代码级 Playwright 资产修复并等待完整重跑；Batch D 尚未完成 | B12-B4 的 22 条 core route 均实际执行但仍为 0 pass / 22 fail，62 个 core Browser ID 仍为 0 pass / 62 fail；认证双边界、正文逐 section 与 B12-08 system Session logout 收口已修复并由 synthetic 验证，尚未形成新产品 Browser 证据；post-browser verify 与双 cleanup 的历史通过事实保持；B12-B、B12 与 Batch D 未完成，不进入 B12-C，B12-86–B12-88 未关闭 |
...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-B4 的历史结果保持 0 route pass / 22 route fail、0 audit pass / 62 audit fail，post-browser verify 与双 cleanup 通过；B12-B5 已完成认证双边界、正文逐 section 与 B12-08 system logout 收口的代码级 Playwright 资产修复并通过 synthetic，未执行产品 Browser；产品/fixture 缺陷为 0；B12-B、B12 与 Batch D 未完成 |
...
- Batch D 的 B11 `core-workflow` 58 个 Browser audit ID、`resilience-security` 11 个 Browser audit ID 与 B11-70 static-gate 均已通过，共 70 项完成，B11 已完成。B12-B2 已完整执行 22 条 core route，但统一因 logout 后的 `/auth/me` 账本断言失败，62 个 core Browser ID 未关闭；post-browser verify 与双次 cleanup 通过。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C；B12-86–B12-88 未关闭，不存在可填写的新 evidence commit。
