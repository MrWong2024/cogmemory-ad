...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 已执行 B11-B `core-workflow` Browser；B11-B1 已修复当时暴露的 fixture Stage 前置校验与 Playwright 验收资产缺口，但尚未使用全新 namespace 完整重跑，B11-B 仍为 51 pass / 7 not_executed 且未完成，不得进入 B11-C。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11-B1 测试资产修复完成，B11-B 尚未完成 | Stage 已支持前序 route 为精确 prepared 或精确合法 product-completed；完整 post-browser verifier 未放宽。Browser 历史状态仍为 51 pass / 7 not_executed，必须以全新 namespace 重跑 `core-workflow`，不进入 B11-C；B11-70 与 B14.1 仍待验 |
...
| Batch D / B11 | `scripts/b11-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 20 route / 58 Browser audit ID，`resilience-security` 4 scenarioKey / 9 route / 11 Browser audit ID；B11-70 为 static-gate | B11-B1 已修复 Stage 进度基线和 Playwright 资产；Browser 历史状态仍为 51 pass / 7 not_executed，B11-B、B11 与 Batch D 均未完成，不进入 B11-C |
...
- Batch D 已执行 B11-B `core-workflow` Browser 但尚未完成。历史 Browser 状态为 51 pass / 7 not_executed；B11-B1 已修复 Stage 全 profile prepared 假设、Stage 后进度完整性、B11-15 / B11-16 / B11-17 / B11-29 / B11-62 的 Playwright 证据，以及 capture task、collect、logout、失败产物收口和 corrected route Console 一一对应分类资产。本阶段后端门禁、前端 test list / infrastructure / lint / typecheck / build 与两个 profile CLI 冒烟通过，但未启动产品应用、Browser test backend 或正式 B11 Browser，不能将静态/E2E/infrastructure 结果计作 Browser pass。B11-B、B11 与 Batch D 均保持未完成；下一阶段从全新 namespace 完整重跑 B11-B `core-workflow`，不进入 B11-C，不关闭 B11-70；不存在可填写的新 evidence commit。
