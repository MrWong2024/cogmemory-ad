...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 与 B8 `core-workflow` 已完成，B8 `resilience-security` 本轮历史结果仍为 19 pass / 2 fail；B8-56、B8-59 fixture 前置合同已修复并通过 E2E、prepared verify 和双次 cleanup，但尚未重新进行 Browser 验收，B8 整体仍未完成，B9–B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B8 | `scripts/b8-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 9 scenarioKey；`core-workflow` 39 audit ID，`resilience-security` 21 audit ID | `core-workflow` 39 项已完成并清理；`resilience-security` 历史结果 19 pass / 2 fail；B8-56、B8-59 前置合同修复已通过 E2E、prepared verify 和双次 cleanup，Browser 尚待复验 |
...
Batch C / D fixture 的验证意图、前置状态和关键边界必须从 frontend testing playbook 的 B7–B15（含 B14.1）待验合同设计；B7 现有资产不覆盖 B8–B15，不得从已完成 Batch A / B、B7 的旧 namespace、操作流水或中间失败状态反推。B8 的两个 profile 使用不同 namespace 前缀、独立 manifest、scenario 所有权和 cleanup 范围；`verify --phase post-browser` 只验证命令所选 profile，未执行该 profile 的 Browser 动作时不得调用或要求其通过。
