...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7、B8 与 B9 已完成，B9 最终为 51 active pass + B9-32 obsolete。B10-A fixture 与分批验收合同已完成；B10-B `generation-workflow` 的 48 项已全部通过，B10-B 已完成；B10-C `public-surface-security` 为 45 pass / 2 fail，B10 整体仍未完成，Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B7–B10 | B7、B8 与 B9 已完成；B10-A fixture、B10-B4 测试资产修复与 B10-B `generation-workflow` 已完成；B10-C `public-surface-security` 为 45 pass / 2 fail，B10 整体仍未完成 | B10-85 产品缺陷、B10-89 fixture / Browser 工具阻断；post-browser verify 与双 cleanup 已闭环；Batch D 尚未启动 |
...
B10-C 基于 `44ac1f3ddb5bb2352a4215b20fee8a628035016f`，使用全新 `b10p-` namespace 完整执行 `public-surface-security` 的 13 个 scenarioKey / 21 条 route / 47 项。静态子进程未加载数据库；backend build、B10 fixture 定向 E2E 1 suite / 11 tests、frontend lint / typecheck / build、prepare 与 prepared verify 均通过。standard_test 实际库为 `cogmemory_ad_test`；Browser backend 与 fixture CLI 实际库均为 `cogmemory_ad_browser_test`，角色分别为 app / `readWrite` 与 db_admin / `dbOwner`，未叠加 `.env.test`，未注入 generation-workflow fault 配置。B10-85 为产品缺陷：真实 generate POST 在服务端写入前中止后，scope 保留且无 retry，Body 仅含 `confirm` 与 `primaryScaleInstanceIds`，但确认 checkbox 被清除。B10-89 为 fixture / 测试资产阻断：合同声明 `long_report` 提供 native checkbox，实际代表页 checkbox 数量为 0；另有当前 Browser 键盘注入无法可靠触发 Enter / Space 的工具限制。其余 45 项通过；post-browser verify 通过，prepared / post-browser 均为 patients 13、visits 21、instances 24、itemResponses 309、MediaEvidence 4、ScoreResults 2、CognitiveDomainResults 2、ClinicalReports 19，业务数据及 hash 完全一致，canonical seed 不变。五角色 Session 已 logout，Browser / Chrome 与服务已关闭，3002 / 5002 已释放；cleanup 1 为 `residualCount=0` / `matched=true`，cleanup 2 为 `residualCount=0` / `matched=false`，未影响其他 namespace 或非 namespace 数据。B10-B `generation-workflow` 48 项完成事实、B9 已完成事实均保持不变；B10-C `public-surface-security` 与 B10 整体仍未完成，Batch D 尚未启动，不填写不存在的 evidence commit。
...
当前最终代码态门禁证据为：lint 0 errors / 0 warnings，typecheck 0 errors，build 通过，full unit 89 suites / 761 tests，B10 fixture 定向 E2E 1 suite / 11 tests，full E2E 25 suites / 121 tests；E2E 实际连接 `cogmemory_ad_test`。这些是当前测试资产代码态的证据，不代表后续代码修改可以免于重跑。
...
| Batch C / B10 | `scripts/b10-browser-fixtures.ts` | 两个独立 profile；每个 5 角色；`generation-workflow` 10 scenarioKey / 26 route / 48 audit ID，`public-surface-security` 13 scenarioKey / 21 route / 47 audit ID | B10-B `generation-workflow` 48 项全部通过且已完成；B10-C `public-surface-security` 为 45 pass / 2 fail；B10 整体仍未完成 |
...
- B10-B5 已使用全新 namespace 完整重跑 `generation-workflow`，B10-01–B10-45、B10-93–B10-95 共 48 项全部通过；真实 HTTP 500、retry-abort、幂等生成、两个 Stage、generation conflict、来源 readiness、patient/report state、post-browser verify、logout/停服与双次 cleanup 全部闭环。B10-C 已完整执行 `public-surface-security`，当前为 45 pass / 2 fail：B10-85 的 generate 网络中止后确认 checkbox 被清除，B10-89 的 `long_report` 缺少合同声明的 checkbox 且当前 Browser 键盘注入不可靠；post-browser verify、logout/停服和双次 cleanup 已闭环。B10-C 与 B10 整体仍未完成，Batch D 尚未启动，不存在可填写的新 evidence commit。
