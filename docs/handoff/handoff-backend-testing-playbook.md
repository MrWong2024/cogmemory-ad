...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 的 B11-B2 已使用全新 namespace 完整执行 20 条 `core-workflow` route，历史结果为 2 route / 9 audit ID pass、18 route / 49 audit ID fail；B11-B3 已在代码级修复 A21 public workflow actor 的 `operatorId` 泄露，但未重跑 Browser，B11-B 仍未完成且不得进入 B11-C。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11-B3 代码修复完成，B11-B 尚未完成 | Browser 历史仍为 2 route / 9 audit ID pass、18 route / 49 audit ID fail；A21 actor 泄露已完成代码级修复，下一阶段必须用全新 namespace 完整重跑 core-workflow；不进入 B11-C，B11-70 与 B14.1 仍待验 |
...
| Batch D / B11 | `scripts/b11-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 20 route / 58 Browser audit ID，`resilience-security` 4 scenarioKey / 9 route / 11 Browser audit ID；B11-70 为 static-gate | B11-B2 已完整执行 20 条 core route，当前为 2 route / 9 audit ID pass、18 route / 49 audit ID fail；B11-B、B11 与 Batch D 均未完成，不进入 B11-C |
...
B11-B3 已把 A21 review actor 从 A20 generation 与 A22–A25 lifecycle actor 中独立拆分；latest / historical report、edit / submit / confirm 首次响应、alreadySubmitted、alreadyConfirmed 与 confirmation 历史 fallback 均只公开 operatorName / operatorRole，数据库 editedBy / submittedBy / confirmedBy 审计 ID 保持真实。mapper / workflow 定向 unit 2 suites / 21 tests、A21 HTTP 定向 E2E 1 suite / 3 tests、B11 fixture 定向 E2E 1 suite / 13 tests、后端 lint / typecheck / build、full unit 89 suites / 765 tests、full E2E 26 suites / 137 tests 全部通过；产品缺陷已完成代码级修复，fixture / 测试资产缺陷 0，环境或工具限制 0。本阶段未启动 Browser、production frontend、Browser test backend、Playwright、Chromium 或 fixture CLI，未重跑 B11-B；Browser 历史仍为 9 pass / 49 fail。B11-B、B11 与 Batch D 仍未完成，下一阶段必须使用全新 namespace 完整重跑 B11-B `core-workflow`；不得进入 B11-C，B11-70 尚未最终关闭，不填写不存在的 evidence commit。
...
- Batch D 的 B11-B2 已使用全新 namespace 完整执行 B11-B `core-workflow` 的 20 条 route，Browser 历史结果为 2 route / 9 audit ID pass、18 route / 49 audit ID fail；当时的 A21 public workflow actor `operatorId` 产品缺陷已由 B11-B3 完成代码级修复，但 Stage 未执行、post-browser verify 未通过的历史事实不变。Browser、服务、runtime、测试产物和双次 cleanup 已完整收口；当前代码级分类为产品缺陷已修复、fixture / Playwright 资产缺陷 0、环境或工具限制 0。B11-B、B11 与 Batch D 均保持未完成；下一阶段必须使用全新 namespace 完整重跑 B11-B `core-workflow`，不进入 B11-C，不关闭 B11-70；不存在可填写的新 evidence commit。
