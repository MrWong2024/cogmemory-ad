...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 与 B8 已完成，B8 的 `core-workflow` 39 项与 `resilience-security` 21 项共 60 项全部闭环；B9-B `core-workflow` 为 37 active pass + B9-32 obsolete。B9-C 已完整执行 `resilience-security`，B9-39–B9-50 与 B9-52 共 13 项通过，B9-51 因响应式全局横向溢出失败；post-browser verify 与双次 cleanup 均通过。B9 整体仍未完成，下一步为修复并定向重验 B9-51，B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B9 | `scripts/b9-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 10 业务 scenarioKey；`core-workflow` 19 route / 38 audit ID（37 active + B9-32 obsolete），`resilience-security` 11 route / 14 active audit ID | core 已完成；`resilience-security` 为 13 pass + B9-51 fail，post-browser verify 与双 cleanup 通过；B9 整体仍未完成 |
