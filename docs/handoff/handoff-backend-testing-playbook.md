...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 的 B11-B4 已使用全新 namespace 完整执行 20 条 `core-workflow` route，当前结果为 18 route / 48 audit ID pass、2 route / 10 audit ID fail；A21 public actor 安全边界与 post-browser verify 已通过，仍有 2 个 Playwright 资产缺陷，B11-B 未完成且不得进入 B11-C。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11-B4 已完整重跑，B11-B 尚未完成 | 20 条 `core-workflow` route 全部执行，18 route / 48 audit ID pass、2 route / 10 audit ID fail；A21 public actor 安全边界与 post-browser verify 已通过，仍有 2 个 Playwright 资产缺陷；不进入 B11-C，B11-70 与 B14.1 仍待验 |
...
| Batch D / B11 | `scripts/b11-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 20 route / 58 Browser audit ID，`resilience-security` 4 scenarioKey / 9 route / 11 Browser audit ID；B11-70 为 static-gate | B11-B4 已完整执行 20 条 core route，当前为 18 route / 48 audit ID pass、2 route / 10 audit ID fail；B11-B、B11 与 Batch D 均未完成，不进入 B11-C |
...
- Batch D 的 B11-B4 已使用全新 namespace 完整执行 B11-B `core-workflow` 的 20 条 route，当前结果为 18 route / 48 audit ID pass、2 route / 10 audit ID fail。A21 public actor `operatorId` 产品缺陷已验证关闭，Stage、产品 mutation 与 post-browser verify 通过；当前阻断为 `edit-success` 的过时 actor-keys 期待和 `corrected-readonly` 的 Console / network 相关性收集两个 Playwright 资产缺陷。Browser、服务、runtime、测试产物和双次 cleanup 已完整收口；产品缺陷 0、fixture / Playwright 资产缺陷 2、稳定环境限制 0。B11-B、B11 与 Batch D 均保持未完成，不进入 B11-C，不关闭 B11-70；不存在可填写的新 evidence commit。
