...
| Batch D / B13 | `B13-P0-contract-evidence` 继续完成且合同前置证据 `gap=0`；`B13-U01` 三种持久状态与 `B13-U02` 首次真实来源冻结 Browser 验收已完成，`B13-U03` 保持待验，状态 `passed=2`、`pending=1`、`failed=0`、`blocked=0`、`not_executed=0`；权威明细见 frontend testing playbook 10.1 |
...
- B13：已重新生成 `B13-U01`～`B13-U03` 3 个活动用户场景；`B13-U01` 三种持久状态与 `B13-U02` 首次真实 Browser `freeze-sources` 验收已完成，状态为 `passed=2`、`pending=1`、`failed=0`、`blocked=0`、`not_executed=0`，`B13-U03` 因其独立显式恢复与不确定结果风险尚未执行而保持 `pending`。U02 后置数据库验证已证明唯一 completed 事实、精确来源终态和受保护边界，两个控制场景未变化。B13-P0-A、P0-A2、G4 与 G5 既有证据继续有效，`B13-P0-contract-evidence` 继续完成且合同前置证据 `gap=0`，下一阶段为 `B13-U03`。后端合同、pure/static、防御证据、P0～P4 职责及原 1～116 迁移的权威明细见 frontend testing playbook 10.1；本手册只维护后端证据分层、数据库、fixture、cleanup 与状态摘要。
