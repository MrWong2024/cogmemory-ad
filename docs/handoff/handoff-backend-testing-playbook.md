...
| Batch C / B10 | `scripts/b10-browser-fixtures.ts` | 两个独立 profile；每个 5 角色；`generation-workflow` 10 scenarioKey / 26 route / 48 audit ID，`public-surface-security` 13 scenarioKey / 21 route / 47 audit ID | B10-B2 五项稳定前置、stage CLI、定向 E2E 与 fixture 冒烟已闭环；generation-workflow 仍为 43 pass / 0 fail / 5 not_executed / 0 obsolete；B10-B 未完成 |
...
- B10-B `generation-workflow` 当前为 43 pass / 0 fail / 5 not_executed / 0 obsolete；B10-B2 已完成剩余五项的 action plan、两个 allowlist stage、真实 HTTP 定向 E2E、六项后端门禁和全新 namespace browser_acceptance 冒烟。stage 首次/重复调用分别证明 `alreadyStaged=false/true`，replace 后 prepared verify 通过，两次 cleanup 均为 `residualCount=0` 且第二次 `matched=false`，seed 全程不变。下一阶段使用全新 namespace 重跑完整 `generation-workflow`；B10-B 与 B10 整体均未完成，`public-surface-security` 与 Batch D 尚未启动，不存在可填写的新 evidence commit。
