...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 的 B11-B2 已使用全新 namespace 完整执行 20 条 `core-workflow` route，结果为 2 route / 9 audit ID pass、18 route / 49 audit ID fail；当前产品阻断是 A21 public workflow actor 暴露内部 `operatorId`，B11-B 未完成且不得进入 B11-C。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11-B2 完整重跑失败，B11-B 尚未完成 | 20 条 core route 均已执行，2 route / 9 audit ID pass、18 route / 49 audit ID fail；A21 public workflow actor 暴露内部 `operatorId`，Stage 未执行且 post-browser verify 未通过，不进入 B11-C；B11-70 与 B14.1 仍待验 |
...
- Batch D 的 B11-B2 已使用全新 namespace 完整执行 B11-B `core-workflow` 的 20 条 route。当前结果为 2 route / 9 audit ID pass、18 route / 49 audit ID fail；A21 public workflow actor 暴露内部 `operatorId` 是稳定产品阻断，Stage 未执行且 post-browser verify 未通过。Browser、服务、runtime、测试产物和双次 cleanup 已完整收口，产品缺陷 1、fixture / Playwright 资产缺陷 0、稳定环境限制 0。B11-B、B11 与 Batch D 均保持未完成；修复产品阻断并形成新的明确任务前，不进入 B11-C，不关闭 B11-70；不存在可填写的新 evidence commit。
