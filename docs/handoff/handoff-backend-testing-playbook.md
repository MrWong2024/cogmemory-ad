...
> [B12-G2.1 Draft v3 执行与证据矩阵](./handoff-testing-evidence-matrix-b12.md) 已获用户批准，批准范围仅为 G3-A。G3-A1 execution layer 与 G3-A2 真实 Browser canary 已完成。G3-A2 复用未修改的 B12 fixture/CLI/verifier/cleanup 与 6 套旧 roots，在两个独立 profile namespace 中完成 prepare/prepared verify、Core/Resilience prepared re-verify 和双次 cleanup；4 个 execution group / 4 个 Session 的 6 个 canary owner 全部通过，其中 doctor-lock 仅执行一次真实 A22。backend build、B12 fixture 定向 E2E 1 suite / 11 tests 通过，产品与 fixture 代码均未修改。canary 不关闭任何 B12 ID；G3-A3 仍为 `not_executed`，G3-B 未批准，下一阶段仅为 G3-A3，不进入 B12-C。
