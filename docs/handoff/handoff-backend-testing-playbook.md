...
| Batch D / B15 | `B15-U01`、`B15-U02` 均已通过，状态 `passed=2`、`pending=0`、`failed=0`、`blocked=0`、`not_executed=0`。A25 三类合法竞态已收敛，C01～C23 已按最低充分层全部处置；P0/P1/P2/P3 均已完成，当前 evidence `gap=0`，B15 Browser 验收闭环完成 |
...
- B14：原 115 项已从活动候选正文退出，`B14-U01` 页面入口/代表性角色/归档后只读与 `B14-U02` 首次真实归档/回执/持久摘要两个活动场景均已通过，状态为 `passed=2`、`pending=0`、`failed=0`、`blocked=0`、`not_executed=0`。U01 零写入 post-browser verifier 与 U02 `u02-post-archive` 均通过；后者证明唯一 A24、全部受保护报告事实和 Patient/Visit/sources 不变、控制场景不变、独立 A24 AuditLog=0，两次 cleanup 均为 `residualCount=0`。`B14-P0-contract-evidence` 的真实双 Session HTTP 并发、C12/C13 frontend pure/static + Action recovery 与 C16 shared identity isolation 均继续关闭；P0 完成且 `gap=0`，`B14-P1-entry-readonly`、`B14-P2-first-archive`、`B14-P3-final-gates` 全部完成，B14 Browser 验收闭环完成。B14.1 不分配活动 ID，correction 真实用户页面行为仍由 B15 负责；B15 的后续完成状态不改变 B14 最终闭环。权威合同表与 Browser 明细见 frontend testing playbook 10.2～10.3。
- B15：已按当前 A25 合同独立重新生成并完成两个且仅两个活动场景：`B15-U01` 首次真实版本化更正与当前会话切换，`B15-U02` 显式恢复与网络不确定结果；最终状态 `passed=2`、`pending=0`、`failed=0`、`blocked=0`、`not_executed=0`。原 10 组只作反向迁移输入，不再是活动清单。既有 A25 HTTP E2E 与 workflow unit 已关闭 C10 及三类合法竞态；12 条 frontend Node-only 测试已关闭 C08/C13/C14。U01 的 1/1 Browser 与 `u01-post-correction` 已关闭 C16。U02 的 fixture prepare/prepared verifier 以正式 A21～A24 和 production readiness/plan/start 建立两个独立场景；目标 spec discovery 为 1 个文件/2 条测试，正式 Chromium 2/2 passed。恢复路径唯一 POST/200 返回 `alreadyCreated=false`、`resumedExisting=true` 并保留首次 correctionId/actor/time/text/plan；网络路径一次中止后无自动 POST/latest/replacement，完成三时点 Storage/URL/Cookie 与真实刷新边界。`u02-post-recovery` 证明恢复场景唯一 completed A25/correction record、V1/V2 各一且持久首事实哈希匹配，网络场景仍是无 correction 的 archived V1、无 V2/V3/branch，两个场景业务 baseline 与所有权隔离正确。两次 cleanup 均为 `residualCount=0`，第二次删除计数全 0，runtime descriptor 已移除。correctionId 继续只作内部 response/verifier 一致性标识，未进入 DOM、URL、Storage、Cookie、日志或报告；详细合同仍以 frontend testing playbook 10.4 为权威。
  ...
  `B15-P0-contract-evidence`、`B15-P1-first-correction`、`B15-P2-recovery-uncertain-result` 与 `B15-P3-final-gates` 均已完成。最终代码态 backend fixture lint:file/typecheck、frontend full lint/typecheck/production build 均退出码 0，U02 Browser 兼任唯一轻量跨层 smoke；活动状态为 `passed=2`、`pending=0`、其余为 0，当前 evidence `gap=0`，C19 与 C23 已分别由 U02/P3 关闭。详细 C01～C23 合同表、B16 复用表、supplemental compatibility/defensive、generic conflict 结论和原 10 组迁移均以 frontend testing playbook 10.4 为唯一权威来源，本手册不复制。
...
B15 后续仍不创建批次专属大型 fixture、evidence matrix、runner、journal、aggregator 或完整 manifest，也不要求一次原子运行关闭整个批次。frontend Node-only P0、U01/C16、U02/C19 与 P3/C23 均已关闭；当前没有剩余 B15 活动场景或通用门禁，只有发生影响性产品或合同变化时才按变化范围重新评估。

本手册不改变 B11 及以前完成状态，不改变 B12/B13/B14 已完成结果，不改变 B14/B15 产品范围；本次只同步 B15 U02/P3 完成事实、后端分层证据与 `gap=0` 摘要。B12～B15 的活动场景清单、具体状态、合同表和旧 ID 映射，以 frontend testing playbook 为权威来源；本手册继续只维护后端证据分层、数据库、fixture、verifier 与 cleanup 规则。
