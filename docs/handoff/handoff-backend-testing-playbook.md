...
| Batch D / B15 | 已从原 10 组候选重新生成 `B15-U01`、`B15-U02` 两个活动场景；状态 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`。A25 pre-start、replacement create-or-resolve 与 replacement record/complete 三类合法竞态均已收敛，C10 已由真实 doctor/admin 双 Session HTTP/DB 并发证据关闭；C08/C13/C14 已由同一份 frontend correction Node-only spec 关闭，`B15-P0-contract-evidence` 完成且 P0 `gap=0`。尚未执行 Browser，下一活动场景为 `B15-U01` |
...
- B15：已按当前 A25 合同独立重新生成两个且仅两个活动场景：`B15-U01` 首次真实版本化更正与当前会话切换，`B15-U02` 显式恢复与网络不确定结果；状态 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`。原 10 组只作反向迁移输入，不再是活动清单。`backend/test/clinical-report-correction.e2e-spec.ts` 的 `creates one linear correction under two concurrent authenticated HTTP requests` 已以 doctor/admin 两个独立合法 Session、同一 source/expectedUpdatedAt、不同文本和一次 `Promise.all` 证明两个 HTTP 200、一个 active completion、一个 completed replay、首次 owner 与 completion executor 边界、唯一 completed A25/correctionRecord/V2、零 V3/branch 及受保护事实不变，C10 已关闭；pre-start、replacement create-or-resolve 和 record/complete 三类合法竞态均有 workflow unit 回归。`frontend/test/browser-acceptance/contracts/b15-correction-non-browser.spec.ts` 已以 12 条 Node-only 测试直接覆盖 C08 persisted resume privacy、C13 correction pure/static 与 C14 Action recovery，P0-G2/G3/G4 均关闭。correctionId 继续作为内部 response/verifier 一致性标识，不面向页面展示；详细合同仍以 frontend testing playbook 10.4 为权威。
  ...
  `B15-P0-contract-evidence` 已完成且 P0 `gap=0`；下一活动场景为 `B15-U01`，随后执行 U02，最终只执行一次 P3 discovery/lint/typecheck/build/依赖/路由/脱敏/轻量 smoke。U01/U02 仍未执行，活动状态继续为 `passed=0`、`pending=2`、其余为 0；当前 evidence gap 总数为 3，仅对应 C16→U01、C19→U02、C23→P3。详细 C01～C23 合同表、B16 复用表、supplemental compatibility/defensive、generic conflict 结论和原 10 组迁移均以 frontend testing playbook 10.4 为唯一权威来源，本手册不复制。
...
B15 后续仍不创建批次专属大型 fixture、evidence matrix、runner、journal、aggregator 或完整 manifest，也不要求一次原子运行关闭整个批次。frontend Node-only P0 gap 已关闭；下一步依次以 `B15-U01`、`B15-U02` 两个最小 Browser Profile 关闭剩余 `ui_reachable` 风险，最后按影响范围执行一次轻量集成冒烟和通用门禁。
