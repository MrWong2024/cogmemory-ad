...
> [B12-G2.1 Draft v3 执行与证据矩阵](./handoff-testing-evidence-matrix-b12.md) 当前状态为：`G3-A approved — G3-A1 completed; G3-A2 completed; G3-A3.1 core matrix and profile evidence assets completed; G3-A3.2 core execution pending; G3-B deferred`。G3-A3.2-R1 已纠正并通过 namespace 前 localhost origin 预检，但正式 frontend/backend 运行单元被外层 120 秒总超时终止，首个 formal group 未 operationally pass；完整 verifier/profile closure 未形成，88 个 B12 ID 状态不变。下一阶段仍需使用全新 namespace 完整重跑 G3-A3.2，不进入 B12-C。
...
| Batch D / B11–B15 | B11 已完成；B12-G3-A3.2-R1 正式 core 执行已阻断；Batch D 尚未完成 | localhost build/runner/backend/CORS 预检通过；首个 formal group 因正式服务运行单元的外层 120 秒总超时而未 operationally pass。诊断性 prepared verify 证明 A22/业务写入与 Stage 均为 0，双次 cleanup、canonical seed 与资源收口通过；其余 9 group、两个 Stage、完整 verifier/profile aggregator 未执行，62 个 core Browser ID 均未关闭 |
...
`backend/.env.browser-acceptance` 当前提供数据库用途、Browser app 主连接、Browser 管理连接，以及批次专用 fixture 密码的稳定本地凭据来源；B9 与 B10 的专用变量均已建立并完成跨独立进程一致性校验，B11 / B12 CLI 分别只从 `B11_FIXTURE_PASSWORD` / `B12_FIXTURE_PASSWORD` 读取本批次稳定密码。本手册只记录变量职责，不记录实际密码或完整 URI。
...
当前最终代码态门禁证据为：lint 0 errors / 0 warnings，typecheck 0 errors，build 通过，full unit 89 suites / 765 tests，B12 fixture 定向 E2E 1 suite / 11 tests，full E2E 27 suites / 148 tests；普通与定向 E2E 实际连接 `cogmemory_ad_test`。这些是 fixture 与代码态证据，不代表任何 B12 Browser 项通过。
...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-B6 已用全新 namespace 完整执行 22 条 core route，最终 0 route pass / 22 route fail、0 audit pass / 62 audit fail；B12-B7 已完成 core 提前执行 B12-83 精确次数合同的代码级资产纠正但未重跑产品 Browser；下一阶段须用全新 namespace 完整重跑 22 条 core route；B12-B、B12 与 Batch D 未完成，B12-83 与 B12-86–B12-88 未关闭 |
...
B12 fixture 固定拆分为两个互不依赖的 profile，且每个 profile 都建立 doctor、admin、nurse、research_assistant、system 五个精确角色。`core-workflow` 覆盖 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID；`resilience-security` 覆盖 6 个 scenarioKey / 11 条 route / 23 个 Browser audit ID；B12-86–B12-88 是无 Browser route 的 3 个 static-gate。88 个稳定 ID 均有唯一 primary owner，85 个 Browser ID 均恰有一条 route，所有 route 各自拥有独立 Patient、Visit、ScaleInstance、ClinicalReport 与 marker，不共享可写 report，也不跨 profile 复用资源。B12-A1 调整后，core prepared 状态矩阵为 draft 1、pending 1、confirmed unlocked 11、quality blocked 1、confirmation missing 1、Visit locked V1 1、Visit voided V1 1、confirmed locked 4、historical locked fallback 1；状态矩阵总数和资源数量不变，resilience 仍为 confirmed unlocked 11。

B12 CLI 只允许 prepare、prepared / post-browser verify、replace、五个 allowlist Stage、runtime descriptor 与 cleanup。五个 Stage 固定为 `lock-conflict-touch`、`lock-conflict-latest-locked-touch`、`lock-audit-unavailable`、`lock-metadata-unsupported`、`forbidden-lock-role`；每次都校验 profile、namespace、scenario、route、transition、role 与 transition-aware 进度，重复执行只返回幂等结果，任意扩展或非法中间态均拒绝。四条 controlled public-read boundary 以合法持久化报告为基线，未来只允许单次初始 public read 分别改变 `isFinal`、top-level `lockedAt`、`lock` summary 或 `lock.lockedAt`；数据库、其他公开字段、后续 read 与写入合同均不得改变。runtime 仍只写 version、batch、profile、scenarioKey、routeKey、primaryRole、可选 secondaryRole、对应 login identifier 与 navigation path；`already-locked-idempotency` 的 primaryRole / secondaryRole 均为 doctor，两个 login identifier 可安全指向同一 fixture 账号，Browser 必须用两个独立 BrowserContext 建立两个独立 Session。字段白名单、目录、basename、existing target、symlink、原子写入和权限门禁均未放宽，CLI stdout 不输出动态账号或路径。

B12 prepared verifier 只读校验合法报告状态、真实 ReportsService summary 与 public mapper、A20/A21/A22 事实、资源矩阵、profile isolation、canonical seed 与 baseline hash。B12-A1 将历史 fallback 覆盖转移给 B12-16 `already-locked-no-repeat`：顶层锁定事实完整但无合法 `metadata.a22Lock`，公开 fallback 保持 lockId=null、role=unknown，且不猜测 operatorName 或 lockNote。B12-41–B12-43 `already-locked-idempotency` 改从 `confirmed_unlocked` 开始，使用 `secondary_lock_once_primary_idempotent`：未来 Secondary doctor 先真实 lock 并收到 `alreadyLocked=false`，Primary doctor 保留旧 expectedUpdatedAt 再提交并收到 HTTP 200 / `alreadyLocked=true`；两次 POST 均不自动 retry 或 polling，数据库只增加 Secondary 产生的一份合法 A22 audit，Primary lockNote 不持久化。Browser POST 次数、顺序和 false/true 分支由后续 Playwright Network 账本验证；fixture verifier 只验证一次合法 doctor lock、受保护报告/来源根不变和 A23–A25 为零，并拒绝零 lock、第二个或非法 A22、错误 actor、protected field、narrative / snapshot、Patient / Visit / ScaleInstance、marker、seed 与跨 namespace / profile 漂移。synthetic simulation 只生成一次合法 doctor lock，不冒充 Browser alreadyLocked 分支证据。

B12-A1 的 core CLI 冒烟在独立进程中只加载 `.env.browser-acceptance`，实际使用 `browser_acceptance` / `cogmemory_ad_browser_test` / db_admin / `dbOwner`，未叠加 `.env.test`；未来 Browser backend 固定使用同库 app / `readWrite`，未来 Playwright runner 数据库用途为 `none`。本阶段完成全新 core namespace 的 prepare、prepared verify、idempotency runtime 生成与删除、replace、再次 prepared verify 和双次 cleanup；cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`。冒烟前后 Browser 测试库的非 lifecycle 语义 hash 和 collection counts 完全一致，canonical seed、其他 namespace 与非 namespace 数据不变，namespace、Session、runtime 与 manifest 无残留。最终后端门禁为 lint、typecheck、build、89 suites / 765 unit tests、B12 定向 E2E 1 suite / 11 tests、full E2E 27 suites / 148 tests；standard_test 实际库为 `cogmemory_ad_test`。本阶段没有启动 production frontend、Browser test backend、Playwright 或 Chromium，也没有执行真实 B12 Browser Action；B12 的 88 项均未记为 pass，B12-86–B12-88 未关闭。B12 fixture 已重新达到 Browser-ready，下一阶段仍为 B12-B `core-workflow` Browser 验收；B11 已完成事实不变，Batch D 仍未完成，不填写不存在的 evidence commit。

B12-B 首次完整执行仍以指定基线 `28685a744bd4ddc021a112e656f7ef887e8423d7` 为历史事实：production frontend、Browser test backend 与 Playwright Chromium 实际执行了 `core-workflow` 的 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID，结果仍为 18 route / 45 audit ID pass、4 route / 17 audit ID fail；失败项仍为 B12-06–B12-08、B12-14、B12-33–B12-40、B12-44–B12-48，完整 post-browser verify、Session logout、Browser / 服务停止、runtime 与测试产物清理以及双次 cleanup 均已完成，cleanup 1 为 `matched=true,residualCount=0`、cleanup 2 为 `matched=false,residualCount=0`，canonical seed、其他 namespace 与非 namespace 数据不变。B12-B1 合同复核确认产品既有边界无需修改：`PATIENT_WORKFLOW_ROLES` 不包含 `system` 且 ClinicalReport controller 复用该类级 role 集合，因此 system 对 Patient / Visit / ClinicalReport 工作流的真实 403、无受保护报告泄露且无 A22 Action 已满足 B12-08；public readability 对 `confirmed` 且 `confirmation=null` 的报告真实返回 `409 / CLINICAL_REPORT_INCOMPLETE`，fail-closed 不公开残缺报告且不开放锁定已满足 B12-14。B12 route 合同新增固定 `expectedPublicReadOutcome` 枚举，默认 `readable`，仅 `confirmation-missing` 为 `clinical_report_incomplete`；该字段进入 audit matrix 与 safe manifest，不进入只承载账号和 navigationPath 的 runtime descriptor，且 `confirmation-missing` 继续保持 `boundaryType=none`、`controlledPublicResponseVariant=none`。prepared verifier 仍接受 `confirmed_confirmation_missing` 合法测试基线，post-browser verifier 仍以 baseline/hash 强制 ClinicalReport、A20–A25 metadata、updatedAt、Patient、Visit、ScaleInstance、fixture marker、canonical seed 及跨 route / profile / namespace 零变化，没有因页面读取失败跳过终态合同。三项 fixture / Playwright 验收资产问题——system 错误要求完整报告可读、confirmation-missing 错误要求部分报告可读及专用告警、doctor / admin 首次锁定错误标题定位——均已完成代码修复，其中 doctor / admin 仍定位产品真实正文 section；但 B12-B1 未启动 production frontend、Browser test backend 或正式 B12 Playwright 产品验收，仅 synthetic infrastructure 启动并关闭隔离 Chromium，未形成任何新 Browser 证据。当前产品缺陷计数修正为 0，fixture / Playwright 验收资产问题为 3 项且均等待完整 Browser 复验，稳定环境限制为 0；历史 45 pass / 17 fail 状态不变，B12-86–B12-88 未关闭，B12-B、B12 与 Batch D 仍未完成，不进入 B12-C。下一阶段必须使用全新 namespace 完整重跑 B12-B `core-workflow` 的 22 条 route；不填写不存在的 evidence commit。

B12-B1 的非 Browser 验证已闭环：backend lint、typecheck、build 均通过，unit 为 89 suites / 765 tests，B12 fixture 定向 E2E 为 1 suite / 11 tests，完整 E2E 为 27 suites / 148 tests；`standard_test` 实际库为 `cogmemory_ad_test`，用户类别为 standard_test app，角色为 `readWrite`。全新 core namespace 的 fixture CLI 已完成 prepare、prepared verify、两条目标 runtime 生成与删除、replace、再次 prepared verify及双次 cleanup；cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`，实际库为 `cogmemory_ad_browser_test`，用户类别为 db_admin，角色为 `dbOwner`，前后 seed、其他 namespace 与非 namespace 数据一致，runtime、manifest 与 namespace 资源无残留。frontend synthetic infrastructure 仅启动并关闭隔离 synthetic Chromium；本阶段未启动产品 Browser 或正式 B12 Playwright 验收。上述 fixture E2E、CLI、synthetic infrastructure 和静态结果均不得记为 Browser pass。

B12-B2 基于指定基线完整执行了全新 core namespace 的 prepare、prepared verify、23 个 runtime descriptor、production frontend、Browser test backend、22 条 Playwright route、两次 allowlist Stage、post-browser verify、Session logout、Browser / 服务关闭、runtime 与测试产物删除及双次 cleanup。后端 build 与 B12 fixture 定向 E2E 1 suite / 11 tests、前端 test list 17 files / 66 tests、synthetic infrastructure 13 / 13、lint、typecheck 与 production build 均通过。standard_test 子进程使用 `cogmemory_ad_test` / app / `readWrite`；Browser backend 使用 `cogmemory_ad_browser_test` / app / `readWrite`；fixture CLI 使用同一 Browser 测试库 / db_admin / `dbOwner`；Playwright runner 不加载数据库 URI。两次 Stage 和完整 post-browser verify 通过，doctor / admin 首次锁定、双 Session alreadyLocked、两条 conflict 的数据库终态符合合同，四条 controlled public-read route 数据库零变化，canonical seed 不变。22 条 Browser route 最终仍全部失败：每个 Session 已完成真实 logout 并进入登录页后，登录页的认证检查产生预期 unauthenticated `/auth/me`；B12 收集器把 logout 前后的全部 `/auth/me` 合并统计，却要求每条都为 2xx，因此所有 route 在业务验收之后统一失败。该 Playwright 资产阻断使 62 个 Browser audit ID 均不能关闭，post-browser verify 不能替代 Browser pass。所有 Context、服务、runtime 与测试产物已收口；cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`。当前分类为产品缺陷 0、fixture 缺陷 0、Playwright 验收资产缺陷 1、稳定环境限制 0。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C；B12-86–B12-88 未关闭，不填写不存在的 evidence commit。

B12-B3 只修改 B12 Playwright support、synthetic infrastructure spec 与前后端 testing playbook，没有修改 backend 代码或 fixture。B12 collector 现在在业务 capture flush、业务页面 network idle、Console 审计停止后且点击 logout 前仅一次记录完整 ledger 的固定索引；通用 `NetworkLedger` 的 Map 插入顺序与 entry 安全结构保持不变。纯函数按索引分离 `authenticated_workflow`、`logout_transition` 和 `post_logout_unauthenticated`：logout 前 `/auth/me` 必须存在且全部为无 request failure 的 2xx，唯一 logout POST 必须为页面 script 发起的无 request failure 2xx，logout 后唯一 `/auth/me` 必须为无 request failure 的 401，且 logout 后 Patient、Visit、ClinicalReport、A21–A25 与 PDF / print / download / signature / AI / LLM 等业务请求为 0。Console 边界仍停在 logout 前，没有为预期 401 添加全局 allowlist，原 Network、Console、写入与隐私断言未放宽。synthetic infrastructure 从 13 项增至 21 项并实际 21 / 21 通过，覆盖正常序列、logout 前非 2xx、logout 后仍为 2xx、auth probe 缺失/重复、logout 后 Patient / ClinicalReport、logout 缺失/重复/失败/非 script、边界越界/重复设置和输入不变性。本阶段数据库用途为 `none`，没有加载 `.env.test` 或 `.env.browser-acceptance`，没有启动 production frontend、Browser test backend、fixture CLI 或正式 B12 Playwright，也没有连接任何数据库。B12-B2 的 22 route 全部执行、0 pass / 22 fail、0 audit pass / 62 audit fail、post-browser verify 通过与双次 cleanup 通过事实保持不变；B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-86–B12-88 未关闭。下一阶段必须使用全新 namespace 从 prepare 开始完整重跑 B12-B `core-workflow` 的 22 条 route；不填写不存在的 evidence commit。

B12-B4 基于 `8e69ba8b8ccc4da7e2509b300e3a8a5ab13945be`，使用全新 `b12c-` namespace 从 prepare 开始完整执行 `core-workflow` 的 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID。backend build、B12 fixture 定向 E2E 1 suite / 11 tests、frontend test list 17 files / 74 tests、synthetic infrastructure 21 / 21、lint、typecheck、production build、prepared verify、23 个 runtime descriptor、production frontend、Browser test backend、health、精确 CORS 与 credentials 均通过；standard_test 使用 `cogmemory_ad_test` / app / `readWrite`，Browser backend 使用 `cogmemory_ad_browser_test` / app / `readWrite`，fixture CLI 使用同库 db_admin / `dbOwner`，Playwright runner 不连接数据库。22 条 route 全部实际执行且两条 allowlist Stage 各执行一次；完整 post-browser verify 通过，doctor / admin 首次锁定、双 Session alreadyLocked、两条 conflict、四条 controlled public-read 与全部零写入 route 的数据库终态符合合同，canonical seed 不变。最终 22 条 route 全部失败、62 个 Browser ID 均未关闭：20 条在三阶段认证账本收口时发现 authenticated slice 包含登录成功前的 401 `/auth/me`；doctor / admin 两条已完成真实锁定后，正文 section 联合 locator 命中两个真实 section，`not.toContainText` 触发 strict-mode。失败清理记录 26 个已创建 Session 中 25 个 logout 成功，denied-role 的 system Session logout 失败，故不能声明全部 Session 生命周期闭环。产品缺陷 0、fixture 缺陷 0、已确认 Playwright 验收资产缺陷 2，另有 1 项 Session cleanup 证据失败；稳定产品环境未发现限制。post-browser 后 Browser / Context、服务、runtime、Stage marker 与测试产物均已收口，端口无监听；cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`，canonical seed 保持不变。B12-B `core-workflow` 未完成；Batch D 暂不能进入 B12-C，B12-86–B12-88 仍未最终关闭，B13–B15 未执行，不填写不存在的 evidence commit。

B12-B5 基于 `5ee0081a263ddf712300009b218b9e97f552cba8` 只修改 B12 Playwright support、doctor / admin 共用首次锁定 spec、synthetic infrastructure 与前后端 testing playbook，没有修改 backend 代码、fixture、产品认证/权限/报告/锁定实现、通用 `NetworkLedger` 或 package / lock。完整 ledger 仍在进入 `/login` 前 attach；登录页初始唯一 `/auth/me` 401 完成且页面稳定后、点击登录前只记录一次 login boundary，业务页稳定且真实 logout 前只记录一次 logout boundary，两者必须为合法整数且严格有序。纯函数用双边界严格分离 `pre_authentication`、`login_transition`、`authenticated_workflow`、`logout_transition` 和 `post_logout_unauthenticated`，返回安全 entry / `bodyKeys` 副本并保持输入不变：登录前唯一 401 仍严格验证但不计入 authenticated/workflow，唯一 script login 与 logout 均须无 request failure 的 2xx，authenticated `/auth/me` 必须存在且全为 2xx，logout 后唯一 `/auth/me` 必须为无 failure 的 401，logout 后业务请求保持为 0。doctor / admin 首次锁定共用逐 section helper，要求联合 locator 非空，并逐个 `nth(index)` 验证可见且不含目标说明。普通 Session 始终优先真实 UI logout；只有 `eligibility-state / denied-role-entry / system / forbidden` 且仍有 HttpOnly Session Cookie、页面无可见按钮时才允许页面脚本对真实 backend `/auth/logout` 发出唯一 credentials-included POST，等待 request 完成后导航真实 `/login`、验证唯一 401 并确认 Cookie 清除；其他 role / route 不 fallback，无 Cookie 返回 `not_authenticated` 且不发 POST。synthetic infrastructure 从 21 项增至 30 项并实际 30 / 30 通过，覆盖五阶段正向序列、pre-login/login/authenticated/logout/post-logout 负向合同、边界与输入不变性、两 section / 泄露 / 空集合，以及真实 scripted logout 的 POST、script initiator、无 failure 2xx、导航、401、Cookie 与无 Cookie 不重发。本阶段数据库用途为 `none`，未加载数据库环境，未启动 production frontend、Browser test backend、B12 fixture CLI 或正式 B12 Playwright，未连接数据库；synthetic Chromium、BrowserContext、随机 localhost Node server 与端口均由 suite 关闭。B12-B4 的 22 route 全部执行、0 pass / 22 fail、0 audit pass / 62 audit fail、post-browser verify 与双 cleanup 通过事实保持不变；B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-86–B12-88 仍未关闭，B13–B15 未执行。下一阶段必须使用全新 namespace 从 prepare 开始完整重跑 B12-B `core-workflow` 的 22 条 route；不填写不存在的 evidence commit。
...
B12-G3-A3.2 首轮基于 `207eb264c78362d5b9740d56de6809e85b0ba5e5`。G3-A2 后没有 backend、fixture builder/contract/manifest、runtime schema、Stage、prepared/post-browser verifier、cleanup 或 Playwright 配置变化，因此执行 backend build 与 B12 fixture 定向 E2E 1 suite / 11 tests，并引用现有完整 E2E 27 suites / 148 tests，不机械重跑。`standard_test` 实际为 `cogmemory_ad_test` / app / `readWrite`。全新 core namespace 的 prepare、prepared verify 与 23 runtime 均通过；fixture CLI 实际为 `cogmemory_ad_browser_test` / db_admin / `dbOwner`，Browser backend 为同库 app / `readWrite`，production frontend、health、CORS、credentials 与端口监听均通过，Playwright runner 不连接数据库。

首个 `eg-doctor-eligibility-read-only` 在 setup/auth 阶段失败：12 owner 全部 `blocked_by_group_setup`、0 pass，`failureCategory=group_setup_auth`、`stopReason=group_setup_failed`、group cleanup 成功；唯一 version 2 finalized journal 没有用作 final closure。根因是 production build 的既有 API origin 为 `localhost:5002`，外层执行却向 runner/CORS 注入 `127.0.0.1` origin，真实认证响应无法匹配 adapter 的精确 backend-origin 等待条件；这是环境编排错误，不是产品、fixture、execution layer、formal adapter 或 Playwright/support 缺陷，因此未修改测试资产，也未使用第二个 namespace 重跑。其余 9 group、两个 Stage、零写入 re-verify、完整 post-browser verifier、profile spec 与 62 项 closure 均为 `not_executed`；失败路径 post-browser 诊断返回 `B12_FIXTURE_POST_BROWSER_MUTATION_INVALID`。cleanup 1 为 `matched=true,residualCount=0` 并删除剩余 11 runtime，cleanup 2 为 `matched=false,residualCount=0`；canonical seed 不变，BrowserContext、Chromium、服务、端口、runtime、Stage marker、journal、profile artifact、test-results 与 Session 最终均为 0。B12-B `core-workflow`、B12 整体与 Batch D 仍未完成，62 个 core ID 与其余 26 项均不关闭；下一阶段仍为在正确 origin 下用全新 namespace 完整重跑 G3-A3.2，不进入 resilience 正式执行，不填写 evidence commit。

B12-G3-A3.2-R1 基于 `main` / `f0f6134b2bca64af7011a39e959f61a1be0e559a` 的 clean 工作区；相对 `207eb264c78362d5b9740d56de6809e85b0ba5e5` 只有三份 handoff 文档变化，因此复用既有 backend/static 门禁。production build、Browser runner、Browser backend、health/readiness、namespace 前预检与正式运行态探针全部统一使用 frontend `http://localhost:3002`、backend `http://localhost:5002`；实际 `/auth/me` 为 401，CORS allow-origin/credentials 正确，禁止 backend origin 请求数为 0。临时预检拓扑在 namespace 创建前已完整收口。

唯一新 core namespace 使用 `browser_acceptance` 数据库用途：fixture CLI 为 db_admin / `dbOwner`，Browser backend 主连接为 app / `readWrite`，Playwright runner 无数据库变量。prepare、prepared verify 与 23 runtime 均通过；首个正式零写入 group 启动后，frontend/backend 受管运行单元被外层设置的 120 秒总超时终止，导致 group cleanup 失败。唯一 journal 为 v2/finalized，12 个 owner record 均完成 minimal cleanup，但 group `operationallyPassed=false`；该结果未进入 profile closure。其余 9 group、两个 Stage、post-browser verifier 与 profile aggregator 均未执行，Stage request/completed marker 始终为 0，A22 与其他产品业务写入均为 0。

停服后诊断性 prepared verify 通过，确认 22 套 fixture roots、prepared hash、资源计数、A22 零写入边界与 canonical seed 均保持 prepared 合同；因此没有把必然不完整的 post-browser verifier 当作正式或诊断 pass。cleanup 1 为 `matched=true,residualCount=0` 并删除剩余 11 runtime，cleanup 2 为 `matched=false,residualCount=0`、runtime 删除数 0；canonical seed 两次均不变，其他 namespace 与非 namespace 数据未受影响。Node、Chromium、Session、3002/5002、runtime、Stage marker、journal、profile artifact、test-results 与临时产物最终均为 0。分类为产品 0、fixture 0、已确认测试资产缺陷 0、外层环境编排阻断 1；B12 core-workflow、B12 与 Batch D 均未完成，62 个 core ID 与其余 26 项不关闭，不进入 resilience、G3-B 或 B13～B15，不填写 evidence commit。

## 6. Browser fixture 通用生命周期

需要真实 Browser 拓扑并依赖 fixture 或数据库事实的独立完整验收 profile，按以下标准 fixture 生命周期执行：
...
完整 profile 必须覆盖 fixture contract、prepare 或受控 replace、prepared verify、production frontend、真实 Browser test backend、完整 Browser 矩阵、完整 post-browser verify、logout、Browser 与进程关闭、namespace cleanup 和第二次幂等 cleanup。prepared verify 未通过不得进入 Browser；Browser 页面通过不能替代数据库终态验证；完整 profile 不得省略 post-browser verify；创建 namespace 后不得省略双次 cleanup；fixture-ready、静态门禁或 E2E 均不得冒充 Browser 通过。Playwright runner 仍不得直接连接 MongoDB，数据库生命周期只能由受门禁的 fixture CLI 执行。

纯静态门禁，lint、typecheck、build，Browser test list，synthetic infrastructure，不连接数据库的通用 Playwright helper 测试，实体设备验收，人工视觉或专业判断，以及不依赖 fixture 或数据库事实的独立检查，按各自合同执行，不机械套用完整 Browser fixture 生命周期。未创建 namespace 的纯静态或 synthetic 任务不机械执行数据库 cleanup，但仍须清理实际创建的 BrowserContext、Chromium / Browser、Node 进程、服务端口、runtime、test-results、Session 和其他临时产物。

某一 profile 已完成完整生命周期后，明确、局部的测试资产缺陷或产品缺陷可以进行定向重验（定向回归）。后置验证按本次 route 的副作用合同选择：

- 定向写入 route 必须执行能够覆盖本次 mutation 的 route-specific verifier；全 profile fixture contract 允许时也可执行完整 post-browser verify。
- 定向零写入 route 可以在 Browser 后执行 prepared re-verify，但必须证明报告、audit、`updatedAt` 和其他业务资源未变化。
- cleanup 只能收口资源，不能替代任何后置只读验证。
...
只要本次创建了 namespace，就必须执行两次 cleanup：第一次要求 `residualCount=0`，第二次要求 `residualCount=0` 且 `matched=false`。cleanup 后仍须确认 namespace-owned 记录与临时文件无残留，且非 namespace 数据、全局 seed 和其他 namespace 未受影响；无论是否创建 namespace，也必须关闭和清理本次实际创建的 Session、BrowserContext、Browser、Node 进程、服务端口、runtime、test-results 和其他临时文件。
...
- Batch D 的 B11 `core-workflow` 58 个 Browser audit ID、`resilience-security` 11 个 Browser audit ID 与 B11-70 static-gate 均已通过，共 70 项完成，B11 已完成。B12-B6 已完整执行 22 条 core route，但全部因 core support 错误要求工作流导航新增 `/auth/me` 精确为 1、实际观察为 2 而失败，62 个 core Browser ID 未关闭；route Action 未开始，post-browser verify 按不完整产品终态失败，双次 cleanup 与零残留收口通过。B12-B7 已完成该 Playwright 验收资产缺陷的代码级纠正：core 逐条验证导航区间认证探针为 GET、2xx、无 failure、script initiator 并记录实际数量，但不提前判断 B12-83 的精确“没有第二次 `/auth/me`”。B12-83 仍由 `resilience-security / presentation-safety / auth-route-deidentified` 独立拥有且未执行、未关闭；B12-B6 的 0 route pass / 22 route fail、0 audit pass / 62 audit fail 历史不变。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C；B12-86–B12-88 未关闭。下一阶段必须使用全新 namespace 完整重跑 B12-B 的 22 条 core route，不存在可填写的新 evidence commit。
- B12-B8 已在基线 `d21704f819c38e308443574698ebec994f0c90fd` 使用独立全新 namespace 执行三种 open mode 的零写入预检。前置门禁、prepare、prepared verify、四个 runtime descriptor、production frontend、Browser test backend、health 与 CORS 均通过；`draft-no-entry`、`denied-role-entry`、`confirmation-missing` 分别因登录前非 allowlist 请求、system 导航区间非 2xx `/auth/me`、真实 409 route 的 Console error 2 对合同 1 而失败。现有安全输出不足以把这些正式 Browser 阻断信号进一步归为产品根因；确认的 fixture 缺陷和稳定环境缺陷均为 0。Browser 后 prepared re-verify 通过，服务、Browser、runtime、marker、产物与端口均已收口，两次 cleanup 均为 `residualCount=0` 且第二次 `matched=false`，canonical seed 不变。按门禁未创建完整验收 namespace，22 条 core route、62 个 core Browser ID、两个 Stage 与完整 post-browser verify 均为 `not_executed`；B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-83 与 B12-86–B12-88 未关闭，不存在可填写的新 evidence commit。
- B12-B9 基于 `7489c6a576de692d56484f1fb6533746098dc901` 增加 B12 专属安全 Network 顺序摘要、非法 system navigation `/auth/me` 与受保护 403 顺序摘要、Console 脱敏 location / 唯一 409 逐事件相关性，以及 open 失败 logout mechanism / ledger detach / Context close 摘要；通用 `ConsoleAudit`、`NetworkLedger`、产品、fixture、allowlist、navigation 成功合同与 Console 允许数量均未改变。frontend test list 为 17 files / 112 tests，synthetic infrastructure 从 39 增至 59 且实际 59 / 59 通过，frontend lint / typecheck / production build、backend build 与 B12 fixture 定向 E2E 1 suite / 11 tests 均通过。全新预检 namespace 遵循固定 `b12c-` profile 前缀并带 `b12p` 标记，完成 prepare、prepared verify、四个 runtime descriptor与三条精确 route；三条均 fail。`draft-no-entry` 和 `denied-role-entry` primary 均得到 relativeIndex 11–14 的四条 `GET /`、200、fetch、script、无 failure，分类为 `normal_public_resource_missing_from_allowlist`，未扩张 allowlist。denied system 本轮通过严格导航 probe 合同并取得真实受保护 403，旧非 2xx `/auth/me` 未复现；route 后续因 primary pre-auth 失败，故旧 system 根因仍为 `insufficient_evidence`，但 nurse、research_assistant 的 UI logout 与 system `scripted_cleanup_fallback` 均成功且 Context 全部关闭。`confirmation-missing` 得到唯一 relativeIndex 49 的 GET latest 409；两个 network Console event 中，一个 exact latest、一个指向非 latest ClinicalReport 集合且 unmatched，计数 exact=1、phase-only=0、unmatched=1，既不足以证明 duplicate，也不足以证明 independent runtime error，分类为 `insufficient_evidence`，Console 允许数仍为 1。Browser 后 prepared re-verify 通过，产品业务写入为 0；服务、Browser、runtime、test-results、Node / Chromium、端口与 namespace 均无残留，cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`，canonical seed 不变。完整 22 条 core route、两个 Stage、完整 post-browser verify、resilience-security、B13–B15 均为 `not_executed`；不关闭任何 B12 audit ID，不填写 evidence commit。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-83 与 B12-86–B12-88 未关闭。
