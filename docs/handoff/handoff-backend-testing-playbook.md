...
> [B12-G2.1 Draft v3 执行与证据矩阵](./handoff-testing-evidence-matrix-b12.md) 当前状态为：`G3-A approved — G3-A1 completed; G3-A2 completed; G3-A3.1 core matrix and profile evidence assets completed; G3-A3.2 core execution pending; G3-B deferred`。G3-A3.2 首轮已在第一个 formal group 的 setup/auth 阶段因外层 Browser origin 编排不一致停止，并完成只读 post-browser 诊断、双次 cleanup 与零残留收口；没有 owner pass、完整 verifier 或 profile closure，88 个 B12 ID 状态不变。下一阶段仍为使用正确 origin 和全新 namespace 完整重跑 G3-A3.2，不进入 B12-C。
...
| Batch D / B11–B15 | B11 已完成；B12-G3-A3.2 首轮正式 core 执行已阻断；Batch D 尚未完成 | 首个 `eg-doctor-eligibility-read-only` 在 setup/auth 阶段因外层 Browser origin 编排不一致失败，12 owner 为 `blocked_by_group_setup`、0 pass；其余 9 group、两个 Stage、完整 verifier 与 profile aggregator 未执行。双次 cleanup 与全部资源收口通过，62 个 core Browser ID 均未关闭；B12-B、B12 与 Batch D 未完成，不进入 B12-C，B12-83 与 B12-86–B12-88 未关闭 |
