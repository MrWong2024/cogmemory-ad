...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-A1 alreadyLocked Browser 可执行合同已修复，fixture、verifier、runtime 与 CLI lifecycle 重新闭环并达到 Browser-ready；Browser 尚未启动，85 个 Browser ID 与 3 个 static-gate ID 均未最终关闭；下一阶段为 B12-B `core-workflow` Browser 验收 |
...
B12-B 基于指定基线 `28685a744bd4ddc021a112e656f7ef887e8423d7`，使用 production frontend、Browser test backend 与 Playwright Chromium 完整执行 `core-workflow` 的 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID。最终为 18 route / 45 audit ID pass、4 route / 17 audit ID fail；失败项为 B12-06–B12-08、B12-14、B12-33–B12-40、B12-44–B12-48。稳定产品阻断有两项：`PATIENT_WORKFLOW_ROLES` 不包含 `system` 且 ClinicalReport controller 复用该 role 集合，导致 system Session 被拒绝读取报告，B12-08 无法成立且同一原子 route 的 B12-06–B12-08 均未关闭；public readability 拒绝 `confirmed` 且 `confirmation=null` 的报告并返回 `CLINICAL_REPORT_INCOMPLETE`，页面无法呈现 B12-14 要求的缺失确认告警。另有一项 Playwright 测试资产缺陷：doctor / admin 首次锁定成功 route 的 pending-readable 断言定位了不存在的标题，导致 B12-33–B12-40、B12-44–B12-48 未形成完整 Browser pass；资产已改为正文 section 定位，但因前述稳定产品阻断未再启动新一轮 Browser，不能用实际 A22 写入或 post-browser verify 代替这些 ID 的页面断言。两项 Stage、完整 post-browser verify、Session logout、Browser / 服务停止、runtime 与测试产物清理均完成；cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`，canonical seed、其他 namespace 与非 namespace 数据不变。分类为产品缺陷 2、fixture 缺陷 0、Playwright 测试资产缺陷 1（已修正、尚未 Browser 复验）、稳定环境限制 0。B12-B 与 B12 均未完成，不进入 B12-C，B12-86–B12-88 不关闭；不填写不存在的 evidence commit。
