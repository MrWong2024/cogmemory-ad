...
| Batch D / B11–B15 | B11 已完成；B12-A1 fixture 合同修复已完成；Batch D 尚未完成 | B11 的 70 项保持闭环；B12 已建立 62 core + 23 resilience Browser ID 和 3 static-gate ID 的 fixture 合同并重新达到 Browser-ready，但 88 项均未最终关闭；下一阶段为 B12-B `core-workflow` Browser 验收 |
...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-B 首次完整执行历史状态保持 18 route / 45 audit ID pass、4 route / 17 audit ID fail；B12-B1 已纠正三项 fixture / Playwright 资产问题但尚未 Browser 复验，产品缺陷计数为 0；B12-B、B12 与 Batch D 未完成，B12-86–B12-88 未关闭；下一阶段使用全新 namespace 完整重跑 22 条 core route |
...
- Batch D 的 B11 `core-workflow` 58 个 Browser audit ID、`resilience-security` 11 个 Browser audit ID 与 B11-70 static-gate 均已通过，共 70 项完成，B11 已完成。B12-A1 已修复 alreadyLocked Browser 可执行合同：62 core + 23 resilience Browser ID 与 3 static-gate ID 的唯一 owner、fixture、verifier、runtime、Stage 与双 cleanup 均保持闭环，B12 fixture 重新达到 Browser-ready；B12 Browser 尚未启动，88 项均未最终关闭。Batch D 仍未完成；下一阶段为 B12-B `core-workflow` Browser 验收；不存在可填写的新 evidence commit。
