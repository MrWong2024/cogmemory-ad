...
| Batch D / B14（含 B14.1） | 已按当前 A24 合同设计 `B14-U01`、`B14-U02` 两个活动 Browser 场景，尚未执行；状态 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`；真实双 Session HTTP 并发、C12/C13 Archive Node-only 与 C16 shared identity isolation 证据均已关闭，`B14-P0-contract-evidence` 完成且 `gap=0`，下一阶段为 `B14-U01`；权威明细见 frontend testing playbook 10.2，B14.1 仍是累计证据索引而非独立 Browser 批次 |
...
- B14：原 115 项已从活动候选正文退出，当前只有 `B14-U01` 页面入口/代表性角色/归档后只读与 `B14-U02` 首次真实归档/回执/持久摘要两个已设计、尚未执行场景；状态为 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`。`B14-P0-contract-evidence` 的真实双 Session HTTP 并发、C12/C13 frontend pure/static + Action recovery 与 C16 shared identity isolation 均已关闭；P0 已完成且 `gap=0`，下一阶段为 `B14-U01`。`B14-P1-entry-readonly` 只承担 U01，`B14-P2-first-archive` 只承担 U02，`B14-P3-final-gates` 只承担最终通用门禁。B14.1 不分配活动 ID，correction 真实用户页面行为仍由 B15 负责；B15 pending 不阻断 B14 P0 shared contract。权威合同表见 frontend testing playbook 10.2～10.3。
