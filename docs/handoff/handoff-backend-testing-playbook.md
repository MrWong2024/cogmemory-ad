...
| WP-10-F2 | Current Browser regression 已闭合；P1 已按 current MMSE contract replacement，P2 reload recovery current green 保持 | current P1 post 为 completed/currentStep drawing、credential hashes cleared、protected facts 与 ItemResponse unchanged、ScaleInstance lifecycle `in_progress`、audio/handwriting/photo Evidence present、downstream=0 | Phase 1A 已解除旧 WP10-F1 support dependency；F3 Browser 资产治理仍待 Phase 2，WP-10 产品完成状态不变 |
...
| WP-10-F2 | `PatientAdministrationSessionService` unit 与 `patient-administration-step-flow.e2e-spec.ts` 已证明 stimulus 当前 run 的 `technicalReplayAuthorized` 为 `false → authorize → true → replay → false`；guidance / image 始终 false，且未扩张 Schema、endpoint 或 revision 合同。后端 unit、HTTP E2E、lint、typecheck、build 已通过 | F2-P1 post verifier：session=completed，captures=19/19，MediaEvidence=17（audio=15、handwriting=1、photo=1），ItemResponse / ScaleInstance unchanged，downstream=0；正常 Browser 主链不调用 review / ASR / submit / scoring / report | 证据形成时：WP-10-F2 完成；F2-P2 recovery 未执行并明确转 WP-10 最终 Browser 收口，staff Axe 2 项转同一最终 accessibility 收口；WP-10 尚在进行中；下一阶段为 F3 | frontend testing playbook“WP-10-F2 阶段证据与最终收口归属”及 roadmap |
