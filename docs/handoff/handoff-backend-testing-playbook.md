...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7 与 B8 已完成，B8 的 `core-workflow` 39 项与 `resilience-security` 21 项共 60 项全部闭环；B9-B4 已修复 core fixture 合同阻断，B9-32 因产品合法状态下不存在可共存的其他分组草稿而标记为 `obsolete`，core 口径为 37 active + 1 obsolete。B9-B3 的 37 项 Browser pass 事实继续保留，但不能直接据此判定修复后的 core 已完成；B9-B 与 B9 仍未完成，下一阶段须使用全新 namespace 重跑完整 `core-workflow`，B9 `resilience-security` 与 B10 尚未启动；Batch D 尚未启动；Batch E 的 8 项真实设备、辅助技术或人工验收继续保留。
...
| Batch C / B7–B10 | B7、B8 已完成；B9-B4 fixture 阻断已修复，B9-B 仍未完成；B10 尚未启动 | B9 core 为 37 active + 1 obsolete（B9-32）；B9 定向 E2E 1 suite / 7 tests、完整 E2E 24 suites / 110 tests 与 core fixture 冒烟通过；须用全新 namespace 重跑完整 core，`resilience-security` 尚未执行 |
...
| Batch C / B9 | `scripts/b9-browser-fixtures.ts` | 两个独立 profile；每个 5 角色 / 10 业务 scenarioKey；`core-workflow` 19 route / 38 audit ID（37 active + B9-32 obsolete），`resilience-security` 11 route / 14 active audit ID | B9-B4 fixture 阻断已修复且 core 冒烟完成；B9-B3 的 37 项 Browser pass 保留，但须用全新 namespace 重跑完整 core 后才能判定；`resilience-security` 与 B9 仍未完成 |
...
- B9-B1 已完成 canonical seed readiness、受保护 baseline 与稳定 `B9_FIXTURE_PASSWORD` 前置。B9-B2 已修复五条 `local_write_gate` route 以及 `CognitiveDomainScoreList`、`CognitiveDomainContributionList`、`CognitiveDomainMappingSummary` 三个前端展示边界。B9-B3 的 37 项 Browser pass 事实保留。B9-B4 已将 B9-32 标记为有稳定原因的唯一 `obsolete`，core 合同为 37 active + 1 obsolete；seed-drift 与 score-confirmation-only verifier 阻断已修复，定向 E2E、完整 E2E 和全新 namespace core fixture 冒烟均通过。冒烟的 prepare、prepared verify、显式 replace、再次 prepared verify 均通过；fixture CLI 建连后确认 `cogmemory_ad_browser_test` 与 db_admin / `dbOwner` 门禁，cleanup 1 与 cleanup 2 均为 `residualCount=0`。本阶段未启动 Browser，也未执行完整 core post-browser 终态；不能据历史 37 项 pass 直接判定 core 完成。B9-B 与 B9 仍未完成；下一阶段使用全新 namespace 重跑完整 `core-workflow`，B9 `resilience-security` 与 B10 尚未启动；不存在可填写的新 evidence commit。
