...
`database-purpose-gates.e2e-spec.ts` 的 current 证据由 standard E2E AppModule gate 与 generic `start-browser-test-backend.ts` pre-import gate 组成：前者证明 `standard_test` 只连接普通测试库并拒绝 Browser URI，后者证明 `browser_acceptance` launcher 在声明普通测试库时于 AppModule import / DB connection 前 fail-closed。`start-browser-test-backend.ts` 不导入任何 profile fixture，继续作为 generic Browser acceptance backend launcher，保留 database-purpose pre-import / connected-runtime gate、AppModule 延迟加载、bootstrap、`configureApp` 与 listen；它不是某个 scripted UI profile 的 fixture，对 v1.20 eligible live topology 及需要真实 backend 的 Agent-assisted UI smoke 仍有独立 current 价值。B10～B15、B18、old WP10-F1、WP10-F3 与本轮 handoff/F2 的 historical Browser fixture executable 已退役；历史 backend / Browser 通过证据仍由本手册、frontend testing playbook 与 Git 历史保留。
