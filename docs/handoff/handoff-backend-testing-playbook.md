...
| Batch D / B14（含 B14.1） | 已按当前 A24 合同设计 `B14-U01`、`B14-U02` 两个活动 Browser 场景，尚未执行；状态 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`；真实双 Session HTTP 并发与 C12/C13 frontend Node-only 缺口已关闭，C16 因缺少 same-route report identity 完整 RESET 仍为 `gap`，权威明细见 frontend testing playbook 10.2；B14.1 是累计证据索引，不是独立 Browser 批次 |
...
B14 已按当前 A24 合同完成文档重新生成：已有 A24 证据覆盖 401/403 与 doctor/admin、DTO/whitelist、ownership，以及 ready 门禁、首次单文档原子 archive、安全 response/mapper、completed 幂等、historical fallback、报告受保护事实和归档后 A20～A23 不恢复可写；A26 E2E 另以 V2/V3 真实生命周期覆盖 replacement lineage 与五类共享来源不被重写。`backend/test/clinical-report-archive.e2e-spec.ts` 的 `archives exactly once under two concurrent authenticated HTTP requests` 已以 doctor/admin 两个独立合法 Session、同一 `expectedUpdatedAt` 和真实 HTTP/DB 并发关闭 B14 P0 双 Session 缺口。C12/C13 已由 `frontend/test/browser-acceptance/contracts/b14-archive-non-browser.spec.ts` 直接覆盖资格、草稿、Body、一致性、latest continuation、错误分类、最多一次 latest、网络 0 次自动 latest、唯一 POST 与无自动 replay；shared spec 另直接覆盖 reducer 互斥/RESET、单 writingRef、统一 report update、route reset 与唯一 beforeunload。C16 仍缺同一 patientId/visitId 下 `report.id` 改变时完整 RESET，故 `B14-P0-contract-evidence` 尚未完成，U01/U02 不得提前执行。A24 generic conflict、historical fallback 与 defensive 分类不变；完整合同表仍仅由 frontend testing playbook 10.2 维护，本手册不复制。
...
- B14：原 115 项已从活动候选正文退出，当前只有 `B14-U01` 页面入口/代表性角色/归档后只读与 `B14-U02` 首次真实归档/回执/持久摘要两个已设计、尚未执行场景；状态为 `passed=0`、`pending=2`、`failed=0`、`blocked=0`、`not_executed=0`。`B14-P0-contract-evidence` 的真实双 Session HTTP 并发与 C12/C13 frontend pure/static + Action recovery 已关闭；C16 的 reducer、coordinator/façade 可取证部分已通过，但 same-route report identity 完整 RESET 缺失，仍是唯一 gap，因此 P0 尚未完成、下一项仍为 P0-G3 最小产品修复/定向测试，不进入 U01。`B14-P1-entry-readonly` 只承担 U01，`B14-P2-first-archive` 只承担 U02，`B14-P3-final-gates` 只承担最终通用门禁。B14.1 不分配活动 ID，correction 专属行为仍由 B15 负责；权威合同表见 frontend testing playbook 10.2～10.3。
