...
| WP-10-F1 | 产品范围已完成，WP-10 状态不回退；same-device / cross-device replacement green 保留为形成时有效的历史证据，两个 `UI_FLOW_DISQUALIFIED` executable 及专用 fixture 已按 v1.20 物理退役 | replacement 当轮使用两个独立 fresh namespace；prepare / 只读 prepared verify、Browser、post verifier 与 cleanup 均通过，实际数据库为 `cogmemory_ad_browser_test`，两次 cleanup 均 `residualCount=0`、runtime absent | future same / cross objective UI evidence 改由 Agent-assisted，subjective / professional / real-device 改由 human manual |
| WP-10-F2 | 产品范围已完成，P1 / P2 green 保留为形成时有效的历史证据；两个 `UI_FLOW_DISQUALIFIED` executable 及专用 fixture 已按 v1.20 物理退役 | P1 当轮 post 为 completed/currentStep drawing、credential hashes cleared、protected facts 与 ItemResponse unchanged、ScaleInstance lifecycle `in_progress`、audio/handwriting/photo Evidence present、downstream=0 | future F2 objective UI evidence 改由 Agent-assisted，subjective / professional / real-device 改由 human manual |
...
Current retained Browser fixture / admin CLI count 为 0。`patient-administration-handoff-browser-fixtures.ts` 与 `wp10-f2-browser-fixtures.ts` 曾分别服务 pre-v1.20 same-device / cross-device 和 F2 full / recovery scripted UI evidence；四个 UI profile 在 v1.20 requalification 后失去 scripted 资格，两份专用 fixture CLI 已随 executable 和专用 support 一并物理退役。文件名与当轮 fixture / verifier / cleanup 结果继续只作为 historical evidence 保留，不是 current executable inventory。

Current frontend scripted deterministic real-Browser executable count 为 0，旧 frontend Browser infrastructure / live / safe-output / shared support framework 已按 clean slate 退役；current `@playwright/test` 只运行 pure/static contracts。该 frontend 资产状态不删除 backend 的通用 Browser runtime 与数据库安全能力。

`database-purpose-gates.e2e-spec.ts` 的 current 证据由 standard E2E AppModule gate 与 generic `start-browser-test-backend.ts` pre-import gate 组成：前者证明 `standard_test` 只连接普通测试库并拒绝 Browser URI，后者证明 `browser_acceptance` launcher 在声明普通测试库时于 AppModule import / DB connection 前 fail-closed。`start-browser-test-backend.ts` 不导入任何 profile fixture，继续作为 generic Browser backend launcher，保留 database-purpose pre-import / connected-runtime gate、AppModule 延迟加载、bootstrap、`configureApp` 与 listen；它不是某个 scripted UI profile 的 fixture，可服务 future qualified non-UI Browser semantic、Agent-assisted interactive Browser smoke 或其它受控 Browser runtime tooling。上述 future execution mode 均继续服从 `browser_acceptance` pre-import / connected-runtime gate 与 app / db_admin 角色边界。B10～B15、B18、old WP10-F1、WP10-F3 与 handoff/F2 的 historical Browser fixture executable 已退役；历史 backend / Browser 通过证据仍由本手册、frontend testing playbook 与 Git 历史保留。
...
以下内容是 retained backend 主证据与 historical Browser fixture / verifier 结果的阶段证据索引；表内 B12～B15、B18、old WP10-F1 与 WP10-F3 fixture 路径描述证据形成时的 executable，现已按 current inventory 退役。工作包状态、下一阶段、当时未执行范围等阶段性描述只代表对应证据形成时的项目状态，不代表当前 roadmap 状态。当前产品范围、工作包完成状态和下一工作包以 `handoff-roadmap.md` 为唯一权威；其他 testing playbook 的状态摘要如与 roadmap 不一致，以 roadmap 为准。历史路径退役不表示本次重新动态执行，也不否定既有最终结果。

| 批次 | backend 主要证据 | Browser verifier / cleanup 摘要 | 证据形成当时的阶段状态 | 详细权威来源 |
|---|---|---|---|---|
...
| B14 | `backend/test/clinical-report-archive.e2e-spec.ts`；`backend/src/modules/reports/services/clinical-report-archive-workflow.service.spec.ts`；`frontend/test/contracts/b14-archive-non-browser.spec.ts`；A24 并发、幂等与 protected facts | `backend/scripts/b14-browser-fixtures.ts` 覆盖只读/唯一归档后置核对与精确 cleanup；生命周期已闭环 | backend-specific 证据完整 | frontend testing playbook“当前证据索引” |
| B15 | `backend/test/clinical-report-correction.e2e-spec.ts`；`backend/src/modules/reports/services/clinical-report-correction-workflow.service.spec.ts`；`frontend/test/contracts/b15-correction-non-browser.spec.ts`；A25 pre-start、start miss、record/complete 三类合法并发收敛 | `backend/scripts/b15-browser-fixtures.ts` 覆盖 U01/U02 prepared/post-correction/post-recovery 与精确 cleanup；生命周期已闭环 | backend-specific 证据完整 | frontend testing playbook“当前证据索引” |
...
| B18 Browser verifier | 复用 A29 / A30 既有生产合同；证据形成当时 backend `src` 与 fixture 零修改 | P9 当时使用 B18 fixture CLI 连接 `cogmemory_ad_browser_test`；prepared 与 `u09-post-media-failure` verifier 均通过。终态 revisionDelta=1、MediaEvidence=0、evidenceRefs 不变、photo requirement=pending、answeredItemCount 不变、实例 draft、score/domain/report=0、protected/adjacent facts matched；cleanup `residualCount=0`、runtime absent。该 fixture executable 现已退役 | backend-specific P3/P9 verifier 完整；B18 补充验证自动化 `gap=0`，WP-03 已完成 | frontend testing playbook“B18-A、B18-B1、B18-B2 与补充验证证据” |
| WP-10-A | 四个精确 unit suite 当时覆盖 released 资产校验、19 步 seed、受控物化补齐与安全 mapper；`assessment-execution-initialization.e2e-spec.ts` 当时覆盖真实 MongoDB 新插入、legacy 补齐、MoCA 零配置与公开摘要不泄漏；只读 CLI 校验真实 released package、22 个资产、19 步、8 页 PDF 和 source hash。该 legacy 补齐是当时的开发阶段证据，后续已由 PRE-ALIGN-03 删除，不代表当前产品行为 | 无患者 UI / API，未创建 Browser 批次；用户已人工确认相交五边形、21 个 MP3 与 MMSE package-001 整包 | 证据形成时：WP-10-A 后端与资产发布范围完成；WP-10 尚在进行中；下一阶段为 WP-10-B | 本行、roadmap 与 backend snapshot/service map |
| WP-10-B1 | 三个精确 unit suite 覆盖会话 / schema、患者 Guard 与 Cookie；`patient-administration-session.e2e-spec.ts` 使用 AppModule、只读 presentation stub 和 standard_test 覆盖角色、Cookie、一次性码、生命周期、并发 CAS、ownership、底层阻断、隐私及实际索引；同时回归 `assessment-execution-initialization.e2e-spec.ts` | 无前端、Browser、真实设备、真实患者或私有资产写入；E2E 仅清理 `B1-SESSION-TEST` namespace，未 drop / 清空集合 | 证据形成时：WP-10-B1 后端范围完成；WP-10-B 尚在进行中；下一阶段为 WP-10-B2 | 本行、roadmap 与 backend snapshot/API/DTO/service map |
| WP-10-B2 | `patient-administration-session.service.spec.ts` 扩展到 20 项，覆盖内嵌 capture / playback schema、步骤归属、完成前置、接管、redo / stepRun、顺序播放、重播授权、图片复核与 CAS 流关闭；新增 `patient-administration-step-flow.e2e-spec.ts`，与 B1 / 初始化回归共 3 suites / 13 tests | AppModule + `standard_test` + 只读内存 MMSE 19 步 / 22 资产 stub；覆盖 binary headers/body、DTO、19 步推进、播放/完成并发、paused replay、redo、takeover、完成凭证清理及 `ScaleInstance` / 11 个 `ItemResponse` 不变；无 Browser、真实设备、真实患者、真实私有文件或外部服务 | 证据形成时：WP-10-B2 与 WP-10-B 后端范围完成；WP-10 尚在进行中；下一阶段为 WP-10-C | 本行、roadmap 与 backend snapshot/API/DTO/service map |
| WP-10-C1 | 新增 audio validator 与患者 evidence 编排 unit，扩展 session / MediaEvidence unit；完整 unit 98 suites / 947 tests。精确 E2E discovery 命中 C1、B2、B1、初始化与既有 staff media 五个文件，正式结果 5 suites / 25 tests | AppModule + `standard_test`，连接后实际库逐字为 `cogmemory_ad_test`；PresentationAssets 只读内存 stub，Storage=fake / 可追踪 fake，验证 audio/photo/handwriting、DTO/隐私、当前 run gate、redo、takeover、并发上传、pause-CAS 精确补偿、ItemResponse / ScaleInstance 零变化、staff media 回归与精确 cleanup。Browser、真实设备、真实 OSS 未执行 | 证据形成时：WP-10-C1 完成；WP-10-C 与 WP-10 均尚在进行中；下一阶段为 WP-10-C2 | 本行、roadmap 与 backend snapshot/API/DTO/service map |
| WP-10-C2 | 受影响 unit 9 suites / 94 tests、报告适配定向 unit 1 suite / 2 tests、完整 unit 102 suites / 984 tests 均通过；最终 E2E discovery 精确命中 C2、C1、B2、B1、初始化、staff media、ItemResponse draft 与 submission 共 8 个文件，正式结果 8 suites / 52 tests；完整 lint / typecheck / build 通过 | AppModule + `standard_test` + `cogmemory_ad_test`，Storage=可追踪 fake、ASR=受控 stub、PresentationAssets=只读内存 stub；覆盖上传→review→转写→幂等→人工 ItemResponse CAS、failed/retry、并发 claim、超长拒绝、redo invalidated run、takeover observation、安全访问及精确 cleanup。真实百炼、真实 OSS、Browser、真实设备未执行 | 证据形成当时：WP-10-C2 与 WP-10-C 后端范围完成；后续 F1/F2 已完成；下一阶段为 F3 | 本行、roadmap 与 backend snapshot/API/DTO/service/config map |
| WP-10-F0（pre-F1 修复） | unit discovery 精确命中 session service 1 suite，定向 34 tests；完整 unit 102 suites / 999 tests。E2E discovery 精确命中 session 与 step-flow 两份，正式结果 2 suites / 7 tests；最终 lint / typecheck / build 均退出 0 | AppModule + `standard_test`，连接后实际库逐字为 `cogmemory_ad_test`；Storage=fake、LLM/SMS=stub、PresentationAssets=只读内存 stub。覆盖同设备 preparation→handoff、未准备零副作用拒绝、跨设备 enter→preparation、paused handoff / resume、startedAt / revision / events、Cookie 身份切换及 ScaleInstance / ItemResponse / MediaEvidence 零变化；按测试自有前缀 / ID 精确 cleanup，残留均为 0。Browser、前端、真实设备、真实麦克风未执行 | 证据形成当时：同设备后端缺口已关闭；后续 F1/F2 已完成；下一阶段为 F3 | 本行与 backend snapshot/API/service map；roadmap / DTO / config / 稳定合同 / frontend handoff 不变 |
| WP-10-F1 | 本轮不修改 backend `src`、既有 backend 测试、fixture CLI、依赖、配置或 env；最终 presentation-assets 校验为 22 assets / 19 steps / 22 references，hash 与 step binding 均通过 | P1 / P2 各自 prepare 与 verify-prepared 均确认 active Patient、in_progress Visit、MMSE 1.0 supervised draft instance、11 个 ItemResponse、19 个 presentation steps、初始 PatientAdministrationSession=0。P1 post 为 active revision 2、staff auth revoked、patient-only credential；P2 post 为 terminated revision 8、完整兑换/准备/暂停/恢复/重签/终止 controlEvents、credential cleared；item facts 均 unchanged，capture/playback/evidence 均为 0。两个 namespace cleanup 均 `residualCount=0`、runtime absent | 证据形成当时：F1-P1 / F1-P2 各正式运行一次并通过；后续 F2 已完成；下一阶段为 F3 | frontend testing playbook“WP-10-F1 最终证据与 Browser Audit 治理”及 roadmap |
| WP-10-F2 | `PatientAdministrationSessionService` unit 与 `patient-administration-step-flow.e2e-spec.ts` 已证明 stimulus 当前 run 的 `technicalReplayAuthorized` 为 `false → authorize → true → replay → false`；guidance / image 始终 false，且未扩张 Schema、endpoint 或 revision 合同。后端 unit、HTTP E2E、lint、typecheck、build 已通过 | 历史 F2-P1 post verifier：session=completed，captures=19/19，MediaEvidence=17（audio=15、handwriting=1、photo=1），ItemResponse / ScaleInstance unchanged，downstream=0；该 count 只描述当时 Browser 终态，不定义 current contract；正常 Browser 主链不调用 review / ASR / submit / scoring / report | 证据形成时：WP-10-F2 完成；F2-P2 recovery 未执行并明确转 WP-10 最终 Browser 收口，staff Axe 2 项转同一最终 accessibility 收口；WP-10 尚在进行中；下一阶段为 F3 | frontend testing playbook“WP-10-F2 阶段证据与最终收口归属”及 roadmap |
| WP-10-F3-PRE-ALIGN-01 | 定向 unit 为 3 suites / 94 tests，ScaleCatalog 最终重跑 1 suite / 17 tests；覆盖 MMSE 三步 seed、Session 既有 evidence / audio / ownership 门禁、精确 predecessor 迁移、第四处与 patientText drift 409、CAS 合法并发收敛。E2E discovery 精确命中初始化与 19-step flow 两文件，正式结果 2 suites / 8 tests；backend lint / typecheck / build 均退出 0。该 predecessor compatibility 后续已由 PRE-ALIGN-02 删除，所列迁移证据仅代表当时执行，不是当前产品行为 | AppModule + `standard_test`，连接后实际库逐字为 `cogmemory_ad_test`；Storage=fake，ASR/LLM/SMS=stub，19-step flow 的 PresentationAssets 为只读内存 stub。新物化 MMSE 保持 package / 19 步 / asset 绑定，naming 无 evidence 拒绝而上传后患者完成，reading 无 evidence 患者完成，three-step 无 stimulus 拒绝而播放后患者完成，并证明 staff complete 不再是目标步骤正常推进方式；沿用既有精确 cleanup，未运行 Browser | 证据形成时：F3 前最低代码对齐完成；F2 仍完成，WP-10 尚在进行中；下一阶段为 F3；尚未实施 F3 | 本行、roadmap 及 backend snapshot/service map |
| WP-10-F3-PRE-ALIGN-02 | 精确 unit discovery 仅命中 `scale-catalog.service.spec.ts`，正式结果 1 suite / 13 tests；删除开发期三步 predecessor compatibility 及专属测试矩阵，当时保留 current seed 插入、一致配置零写复用、通用 stored drift conflict 与既有双字段缺失补齐；该补齐随后由 PRE-ALIGN-03 删除。E2E discovery 仅命中 `assessment-execution-initialization.e2e-spec.ts`，正式结果 1 suite / 7 tests；backend lint / typecheck / build 均退出 0 | AppModule + `standard_test`，连接后实际库逐字为 `cogmemory_ad_test`；Storage=fake，ASR/LLM/SMS=stub。干净测试数据从 current seed 物化 MMSE package 与 19 步，三个目标步骤保持 patient advance；沿用既有精确 cleanup，未运行 Browser，也未重新打开 F2 | 证据形成时：F3 前最低代码对齐保持完成；F2 仍完成，WP-10 尚在进行中；下一阶段为 F3；尚未实施 F3 | 本行、backend snapshot/service map 与当前代码 |
| WP-10-F3-PRE-ALIGN-03 | 精确 unit discovery 仅命中 `scale-catalog.service.spec.ts`，正式结果 1 suite / 12 tests；删除双 presentation 字段缺失 runtime backfill、CAS / 重读并发收敛及两个专属 unit，以一个通用 absent-config case 锁定稳定 fail-closed。E2E discovery 仅命中 `assessment-execution-initialization.e2e-spec.ts`，正式结果 1 suite / 7 tests；初始化资产不再人为 `$unset` 或创建 LEGACY 路径，current MMSE materialization、三个 patient advance 与 MoCA 零 presentation 保持通过；backend lint / typecheck / build 均退出 0 | AppModule + `standard_test`，建连后实际库逐字为 `cogmemory_ad_test`；Storage=fake，ASR/LLM/SMS=stub。测试使用既有精确 cleanup；只读 residual 核对 users / patients / visits / instances / ItemResponse / scale definitions / versions 总残留为 0。未运行 Browser，也未重新打开 F2 | 证据形成时：F3 前最低代码对齐保持完成；F2 仍完成，WP-10 尚在进行中；下一阶段为 F3；尚未实施 F3 | 本行、roadmap、backend snapshot/service map 与当前代码 |
| WP-10-F3 基础对齐 | 定向 unit discovery 命中 MMSE seed validation、submission readiness、MediaEvidence workflow 与 controller，正式结果 4 suites / 70 tests；覆盖 11 项 optional note、通用 required-note 守恒、有效 / invalidated / evidence-only / 非患者 / 未完成 Session / 非要求类型 / CAS miss adoption 与 Storage / Evidence 零创建。定向 E2E discovery 仅命中 `patient-administration-evidence.e2e-spec.ts`，正式结果 1 suite / 3 tests；backend lint / typecheck / build 最终均退出 0 | AppModule + `standard_test`，建连前 purpose 门禁通过，建连后实际库逐字为 `cogmemory_ad_test`；Storage=fake、ASR/LLM/SMS=stub、PresentationAssets=只读内存 stub。真实 19 步 Session completed 后采用 drawing photo，同一 Evidence ID 写入 evidenceRefs、readiness requirement 满足、MediaEvidence 数量不增、upload/copy/delete 零调用；重复采用和 invalidated writing run 均 409 且 ref 安全不变。沿用精确 namespace cleanup，residual=0；未运行 Browser、前端、真实 OSS / ASR / 设备 | 证据形成时：F3-pre 保持完成；F2 仍完成；两个最低基础对齐完成；WP-10-F3 当时已进入进行中但尚未完成；下一步为现有 ScaleInstance 页面的正常复核 UI | 本行、roadmap 与 backend API/service/snapshot maps |
| WP-10-F3 current evidence governance | C2 review / transcription、F3 adoption / readiness 的既有 backend unit 与 HTTP E2E，以及 A14 / A16 各自主证据在 current backend contract 下继续有效；scripted automation 退役不删除这些独立责任 | 历史 F3 Browser evidence 继续作为形成时有效证据；2026-08-22 current Playwright re-automation 连续两轮因 `spec/test` locator / test-asset 失败后停止，未形成 current green、未证明 production contract conflict；F3 fixture / spec executable 现已退役 | 当前：WP-10-F3 与 WP-10 产品状态保持完成；测试工具治理不重新打开产品范围。若需要 current Browser flow evidence，execution mode 由 frontend testing playbook 决定 | frontend testing playbook“WP-10-F3 正常作答复核证据”及 roadmap |
...
证据形成当时的最终后端证据继续复用：完整 unit 为 92 suites / 853 tests，完整 HTTP E2E 为 26 suites / 158 tests；当轮未修改 backend `src` 或 B18 fixture，因此未机械重跑 backend lint/typecheck/build、A29/A30 unit 或 HTTP E2E。P9 fixture 与 verifier 当时以 `browser_acceptance` db_admin 身份逐字确认实际库 `cogmemory_ad_browser_test`，Browser backend 使用 readWrite 身份；prepared/post 与 cleanup 完整通过，MediaEvidence/evidenceRefs/相邻事实符合上传未到达后端的终态，cleanup `residualCount=0`、runtime absent。B18 fixture executable 现已退役；A29/A30 backend gap 仍为 0，跨层 B18 自动化历史闭环与 WP-03 完成事实不变。

WP-10-A 最终代码态的 unit discovery 精确命中 `presentation-assets.service.spec.ts`、`scale-seed-data.service.spec.ts`、`scale-catalog.service.spec.ts`、`scales.service.spec.ts`，正式定向结果为 4 suites / 75 tests；完整 unit 为 93 suites / 889 tests。HTTP E2E discovery 精确命中且仅命中 `assessment-execution-initialization.e2e-spec.ts`，建连后实际数据库逐字等于 `cogmemory_ad_test`，结果为 1 suite / 7 tests；未加载 Browser 配置。`lint`、`typecheck`、`build` 与真实 `presentation-assets:verify` 均退出 0；CLI 为 `none` 进程，不导入 AppModule、不连接数据库。人工证据仅覆盖用户已确认的相交五边形、A1 三段、A2 十八段和 MMSE package-001 整包；该证据形成时尚未执行患者 UI、患者 API、真实设备或 Browser 验收，这些边界当时留给 WP-10-B 及后续阶段。
