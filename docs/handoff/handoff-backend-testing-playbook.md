...
| WP-10-F1 | 产品范围已完成，WP-10 状态不回退；same-device / cross-device replacement green 保留为形成时有效的历史证据，scripted UI executable / fixture 已按 v1.20 退役 | backend session、identity、handoff 与 verifier 历史证据有效；Browser execution 详情见 frontend testing playbook“WP-10 F1 / F2 / F3 历史验证证据索引” | future same / cross objective UI evidence 改由 Agent-assisted，subjective / professional / real-device 改由 human manual |
| WP-10-F2 | 产品范围已完成，P1 / P2 green 保留为形成时有效的历史证据；scripted UI executable / fixture 已按 v1.20 退役 | patient step-flow、media / Evidence、technical replay 的 current unit / HTTP E2E asset 继续承担 lower-layer evidence；历史 post-verifier 已确认关键服务端 invariant 闭合，Browser execution 详情见 frontend testing playbook同一索引 | future F2 objective UI evidence 改由 Agent-assisted，subjective / professional / real-device 改由 human manual |
| WP-10-F3 | 产品范围保持完成；review / transcription、Evidence adoption / readiness、A14 / A16 的 current lower-layer evidence asset 继续有效，历史 Browser 证据保留 | Browser historical / current execution 状态见 frontend testing playbook“WP-10 F1 / F2 / F3 历史验证证据索引”，本手册不复制 Playwright execution journal | F3 scripted UI executable / fixture 已退役；测试工具状态不回退 WP-10 |
...
以下内容是 retained backend 主证据与 historical Browser fixture / verifier 结果的阶段证据索引；表内 B12～B15、B18 的 fixture 路径描述证据形成时的 executable，现已按 current inventory 退役。WP-10 只保留 current lower-layer evidence asset、persistent backend invariant 与 historical verifier 高层结论。工作包状态、下一阶段、当时未执行范围等阶段性描述只代表对应证据形成时的项目状态，不代表当前 roadmap 状态。当前产品范围、工作包完成状态和下一工作包以 `handoff-roadmap.md` 为唯一权威；其他 testing playbook 的状态摘要如与 roadmap 不一致，以 roadmap 为准。历史路径退役不表示本次重新动态执行，也不否定既有最终结果。
...
| WP-10-A | Current tracked locators：`backend/src/modules/scales/services/presentation-assets.service.spec.ts`、`backend/src/modules/scales/seeds/scale-seed-data.service.spec.ts`、`backend/src/modules/scales/services/scale-catalog.service.spec.ts`、`backend/src/modules/scales/services/scales.service.spec.ts`、`backend/test/assessment-execution-initialization.e2e-spec.ts`。主证 released presentation package validation、scale seed / materialization、assessment initialization、安全公开 projection，以及 MoCA 无 presentation config 的合法边界 | historical package / static validation 结论继续有效；不把开发期 compatibility / backfill 当作 current behavior | 对应范围形成时有效；current WP-10 状态以 roadmap 为准 | current tracked assets、backend snapshot / service map 与 roadmap |
| WP-10-B1 / B2 | Current tracked locators：`backend/src/modules/assessments/services/patient-administration-session.service.spec.ts`、`backend/src/modules/assessments/guards/patient-administration-session.guard.spec.ts`、`backend/src/modules/assessments/utils/patient-administration-cookie.util.spec.ts`、`backend/test/patient-administration-session.e2e-spec.ts`、`backend/test/patient-administration-step-flow.e2e-spec.ts`。主证 workflow role / auth、一次性 enter code / Cookie、session lifecycle / ownership / preparation / same-cross handoff、pause / resume / reissue / terminate、revision / CAS、step authority、capture / playback、redo / takeover / technical replay、completion credential cleanup、关键并发终态，以及受保护的 ItemResponse / ScaleInstance facts | 这些 current lower-layer assets 不等价于 Browser、真实设备或真实患者 evidence | 对应范围形成时有效；current WP-10 状态以 roadmap 为准 | current tracked assets、backend API / DTO / service map 与 roadmap |
| WP-10-C1 / C2 | Current tracked locators：`backend/src/modules/media/services/patient-administration-evidence.service.spec.ts`、`backend/src/modules/media/services/patient-administration-review.service.spec.ts`、`backend/src/modules/media/services/media-evidence-transcription.service.spec.ts`、`backend/src/modules/media/services/media-evidence-workflow.service.spec.ts`、`backend/src/modules/media/services/media-evidence-public.mapper.spec.ts`、`backend/test/patient-administration-evidence.e2e-spec.ts`、`backend/test/patient-administration-review-transcription.e2e-spec.ts`、`backend/test/media-evidence.e2e-spec.ts`、`backend/test/item-response-draft.e2e-spec.ts`、`backend/test/scale-instance-submission.e2e-spec.ts`。主证 patient audio / photo / handwriting Evidence、DTO / privacy、current-run ownership、redo / takeover invalidation、concurrent upload、Storage / DB consistency 与补偿、显式 ASR / transcription、retry / idempotency / claim、安全访问、staff observation、ItemResponse CAS、transcription candidate 不自动成为正式答案，以及 response 不泄漏 object key / signed URL 等内部事实 | controlled provider doubles 只形成 server-side API / state / persistence evidence | 对应范围形成时有效；current WP-10 状态以 roadmap 为准 | current tracked assets、backend API / DTO / service / config map 与 roadmap |
| WP-10-F0 / F1 / F2 | Current reusable evidence 主要复用 Session service、`backend/test/patient-administration-session.e2e-spec.ts`、`backend/test/patient-administration-step-flow.e2e-spec.ts` 与 `backend/test/patient-administration-evidence.e2e-spec.ts`：主证 preparation → handoff、未准备拒绝、cross-device enter / preparation、pause / resume、identity / Cookie transition、revision / events、`technicalReplayAuthorized` 与受保护 facts。F1 / F2 historical verifier 曾确认身份、会话、多模态流程、protected ItemResponse / ScaleInstance 与无意外 downstream 写入闭合 | Browser historical/current execution 详情见 frontend testing playbook“WP-10 F1 / F2 / F3 历史验证证据索引”；专用 scripted UI executable / fixture 已退役 | 对应范围形成时有效；current WP-10 状态以 roadmap 为准 | current tracked assets、frontend testing playbook 当前索引与 roadmap |
| WP-10-F3 | Current tracked locators：`backend/src/modules/media/services/patient-administration-review.service.spec.ts`、`backend/src/modules/media/services/media-evidence-transcription.service.spec.ts`、`backend/src/modules/assessments/lib/scale-instance-submission-readiness.spec.ts`、`backend/test/patient-administration-evidence.e2e-spec.ts`、`backend/test/patient-administration-review-transcription.e2e-spec.ts`、`backend/test/item-response-draft.e2e-spec.ts`、`backend/test/scale-instance-submission.e2e-spec.ts`、`backend/test/media-evidence.e2e-spec.ts`。主证 completed administration 后 review projection、显式 transcription、Evidence 按需访问、adoption 复用同一 Evidence、invalidated / wrong-run / wrong-type 拒绝、readiness、既有 A14 CAS / A16 submission，以及 review / transcription / adoption 不自动产生 scoring / domain / report downstream 写入 | F3 historical/current Browser execution 状态见 frontend testing playbook同一索引；scripted UI executable 已退役，不影响独立 backend unit / HTTP E2E evidence | 对应范围形成时有效；current WP-10 状态以 roadmap 为准 | current tracked assets、frontend testing playbook 当前索引与 roadmap |

- WP-10 current unit / HTTP E2E asset 是否重跑，按当前代码 diff 与 §3 / §6 的最低充分证据规则决定；本次只确认 tracked existence 与职责，没有形成 dynamic green。
- Storage / ASR / presentation 相关受控 test double 只证明服务端 API、state 与 persistence invariant，不代表真实 OSS、真实 ASR、真实 Browser 或真实设备验收。
- F1 / F2 / F3 Browser historical / current execution evidence 由 [Frontend Testing Playbook](./handoff-frontend-testing-playbook.md)“WP-10 F1 / F2 / F3 历史验证证据索引”统一维护；Backend Testing Playbook 不复制 execution journal，fixture / executable 退役也不否定历史结论或回退 roadmap 状态。
