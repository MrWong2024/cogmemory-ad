...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 的 B11 `core-workflow` 58 Browser 项、`resilience-security` 11 Browser 项与 B11-70 static-gate 1 项均已通过，共 70 项完成，B11 已完成。B12-B6 已基于指定基线用全新 namespace 从 prepare 开始完整执行 22 条 core route，结果为 0 route pass / 22 route fail、0 audit pass / 62 audit fail：全部 route 在工作流导航阶段发现相对 Dashboard 的额外 `/auth/me` 实际为 2、合同要求为 1，未进入后续 route Action。post-browser verify 按不完整产品终态失败，双次 cleanup 与零残留收口通过。当前分类为产品缺陷 0、fixture 缺陷 0、Playwright 验收资产缺陷 1、Session 收口证据缺口 1、稳定环境限制 0。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-86–B12-88 未关闭。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11 已完成；B12-B6 已完成全新 namespace 的 core 完整重跑但验收阻断；Batch D 尚未完成 | 22 条 core route 全部实际执行，最终 0 pass / 22 fail，62 个 core Browser ID 为 0 pass / 62 fail；全部 route 在工作流导航阶段发现相对 Dashboard 的额外 `/auth/me` 实际为 2、合同要求为 1，未进入后续 route Action；post-browser verify 按不完整产品终态失败，双次 cleanup 与零残留收口通过；B12-B、B12 与 Batch D 未完成，不进入 B12-C，B12-86–B12-88 未关闭 |
...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-B6 已用全新 namespace 完整执行 22 条 core route，最终 0 route pass / 22 route fail、0 audit pass / 62 audit fail；工作流额外 `/auth/me` 实际为 2、合同要求为 1，未进入 route Action，post-browser verify 失败；双次 cleanup 与零残留收口通过；B12-B、B12 与 Batch D 未完成 |
...
- Batch D 的 B11 `core-workflow` 58 个 Browser audit ID、`resilience-security` 11 个 Browser audit ID 与 B11-70 static-gate 均已通过，共 70 项完成，B11 已完成。B12-B6 已完整执行 22 条 core route，但全部因工作流额外 `/auth/me` 实际为 2、合同要求为 1 而失败，62 个 core Browser ID 未关闭；post-browser verify 按不完整产品终态失败，双次 cleanup 与零残留收口通过。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C；B12-86–B12-88 未关闭，不存在可填写的新 evidence commit。
