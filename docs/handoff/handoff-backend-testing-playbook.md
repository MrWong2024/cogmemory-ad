...
| A29 / WP-03 backend | `item-response-timing.spec.ts` + `item-response-draft.service.spec.ts`：2 suites / 68 tests；`item-response-draft.e2e-spec.ts` + `media-evidence.e2e-spec.ts`：2 suites / 17 tests；覆盖 legacy、完整 timing 状态 / 转换、CAS filter / update、stale / 并发冲突零写入、protected fields 与媒体版本隔离 | 本阶段不执行 Browser、fixture profile 或 Browser verifier；B18 负责前端自动保存、恢复与实时计时交互 | A29 后端范围完成；B18 pending；WP-03 进行中 | 本行及 roadmap / A29 maps |

B12～B15 的 Browser 操作、页面文案、keyboard、viewport、Storage 和活动场景结果只在 frontend testing playbook 的语义索引中维护；本手册不复制执行流水或历史迁移表。

A29 最终代码态已实际通过 backend `lint`、`typecheck`、`build`、完整 unit（90 suites / 826 tests）与完整 HTTP E2E（26 suites / 146 tests）；定向和完整 HTTP E2E 实际连接库名均由门禁锁定为 `cogmemory_ad_test`，Storage 为 fake、LLM / SMS 为 stub。完整 E2E 因既有 B10 runtime descriptor 需要写 `backend/test-results`，在 sandbox 内出现目录权限拒绝后按平台规则以沙箱外身份重跑并通过；未加载 browser acceptance 配置，未启动 Browser 或长期保留 namespace。本文只保留最终稳定证据与执行身份，不记录逐轮失败流水或 future commit SHA。
