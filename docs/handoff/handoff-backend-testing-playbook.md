...
本文档不改变产品、接口、DTO、Schema、测试合同或 roadmap 工作包状态。当前唯一事实是：WP-02、WP-04、Batch A 已完成；Batch B 桌面范围已完成；Batch C 的 B7–B10 已完成。B10 `generation-workflow` 为 48 pass，`public-surface-security` 为 47 pass，共 95 项全部完成。Batch D 的 B11 `core-workflow` 58 Browser 项、`resilience-security` 11 Browser 项与 B11-70 static-gate 1 项均已通过，共 70 项完成，B11 已完成。B12-B4 已基于指定基线用全新 namespace 完整执行 22 条 core route，但 20 条因 authenticated 阶段错误纳入登录前 401 而失败，doctor / admin 两条首次锁定因多元素 locator 的 strict-mode 资产缺陷失败；62 个 core Browser audit ID 均未关闭。完整 post-browser verify 与双次 cleanup 通过，denied-role 的 system Session 未取得成功 logout 证据。当前分类为产品缺陷 0、fixture 缺陷 0、已确认 Playwright 验收资产缺陷 2，另有 1 项 Session cleanup 证据失败；B12-B、B12 与 Batch D 仍未完成，不进入 B12-C，B12-86–B12-88 未关闭。Batch E 的 8 项真实设备或人工验收尚未执行并继续保留。
...
| Batch D / B11–B15 | B11 已完成；B12-B4 完整 Browser 重跑仍受 Playwright 资产阻断；Batch D 尚未完成 | 22 条 core route 均实际执行但最终均失败，62 个 core Browser ID 未关闭；20 条为认证阶段边界缺陷，doctor / admin 两条为正文 locator strict-mode 缺陷，system Session logout 证据失败；post-browser verify 与双 cleanup 通过；B12-B、B12 与 Batch D 未完成，不进入 B12-C，B12-86–B12-88 未关闭 |
...
| Batch D / B12 | `scripts/b12-browser-fixtures.ts` | 两个独立 profile、每个 5 角色；`core-workflow` 5 scenarioKey / 22 route / 62 Browser audit ID，`resilience-security` 6 scenarioKey / 11 route / 23 Browser audit ID；B12-86–B12-88 为 static-gate | B12-B4 的 22 条 core route 均实际执行但最终均失败，62 个 core Browser ID 未关闭；20 条为认证阶段边界缺陷，doctor / admin 两条为 locator strict-mode 缺陷，system Session logout 证据失败；post-browser verify 与双 cleanup 通过；产品/fixture 缺陷为 0，已确认 Playwright 资产缺陷为 2；B12-B、B12 与 Batch D 未完成 |
