...
关键边界：
- 现有医生侧工作台与量表执行工作流已经落地；`AuthDashboard` 本身仍是轻量入口，不等同未来 WP-12 的临床任务启动台。
- 面向患者的一步一屏施测、短期配对、系统指导语与刺激音频、录音与 ASR、医生同步监管和接管、患者端恢复、医生复核后形成正式 `ItemResponse` 及完整 MMSE / MoCA 多模态编排均尚未实现。
- AI 临床解释和科研脱敏导出仍未实现。
- 不实现永久离线草稿；Batch E 的 8 项真实设备或人工项目仍为 `pending`，最终主要归属为 WP-08。
- 一期核心量表由医护在场或监督，通过平板优先的患者施测端完成；不开放无人监督的居家 MMSE / MoCA 核心自测。
