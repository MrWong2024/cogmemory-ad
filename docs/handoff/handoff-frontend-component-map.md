...
- 职责：独立加载患者详情与访视列表，管理访视 URL 筛选、整日日期转换、分页、空态与分区错误；访视行提供“打开访视”链接，并提供患者评估历史与随访趋势入口
...
- 职责：识别 photo / handwriting requirement，组件实际渲染时按题加载 A15 列表，管理 loading / error / retry、上传 / 撤销患者正式采用 / direct formal 作废调用、临时 URL 内存缓存和稳定错误文案
...
- 正式动作：仅 attached 且可编辑的当前正式 Evidence 开放动作。`patientAdministrationOrigin=true` 显示“撤销正式采用”，确认说明明确只解除正式关联、原始证据 / 文件 / 施测记录保留且可再次采用，不显示作废原因；direct formal 显示“作废此正式证据”，原因继续必填 3–1000 字符并明确后台历史保留。两种动作不合并为含糊的移除操作
...
- 职责：语义化展示 blocking / warning 列表，按受控 code 使用稳定中文标题和说明；正式支持 `ITEM_MANUAL_OBSERVATION_INCOMPLETE` 与 `ITEM_BINARY_MANUAL_DECISION_INCOMPLETE` 的临床文案。结构化、阅读原始观察或 binary 判断未完成时明确引导医护补齐原始事实、评分确认并完成题目，仅附加允许的安全字段
- 展示复用：逐题工作单元复用同一 title / description 映射但隐藏“定位题目”；blocking 可启用纯 `buildInlineActionableIssuePresentations()`，将 reading observation + binary + answer + not-completed、structured+answer+not-completed、binary+answer+not-completed、answer+not-completed 同根因分别折叠为一个医生动作。required step / timing / media / evidence consistency / configuration / operator note / missing reason / 未知 code 不被吸收；warning 原样逐条展示。归并只影响 inline presentation，不 mutation / 回写 backend issue 数组、数量、ready 或 canSubmitNow；普通全局模式仍使用完整 server issues 与计数。inline details 省略 itemOrder / title / code / CRF / group，保留 missingStepCodes / evidence 要求 / mismatch codes；普通全局模式仍保留完整定位与 metadata
...
- 职责：作为唯一公开 façade 组合 edit、submit、confirm、lock、source_freeze、archive、correction 七类 Action；统一暴露八种 activeMode（含 idle）、单一 writingAction、各类 React 内存草稿、dirty / stale、error / receipt、liveMessage、角色 gate 与 open / cancel / update / save 方法。
...
- 生命周期：所有报告工作流 dirty 条件汇总到唯一 `useClinicalReportBeforeUnload`；路由 patientId / visitId 改变时清理内存草稿与回执。刷新页面后未保存内容消失是预期行为。
- 权限：replacement 的 A21–A25 写操作与 A22–A24 不可逆阶段仅向 doctor / admin 开放；nurse / research_assistant 只读。403 保留报告与本地草稿，后端 Guard 仍是最终边界。
- 边界：不渲染 JSX，不读取 Cookie，不持久化状态，不自动保存、串联或重试写请求，不分析或生成临床文本。
...
- 职责：展示 editorial 最新编辑摘要、submission、confirmation、当前报告 lock / sourceFreeze / archive / correction 事实，以及当前会话各阶段 receipt；replacement 显示当前 reportCode / Vn。
...
- 成功完整应用 response.report、保存 archiveReceipt、清草稿并回 idle；alreadyArchived false / true 均是成功。归档规则与一致性算法位于独立纯函数，统一公开 façade 与单写锁不变。
...
- 会话摘要：始终展示状态与设备方式；open session 仅在真实 `startedAt` 存在时展示开始时间并继续展示 `expiresAt`，且仅 cross-device open session 以业务文案显示“患者设备：待进入 / 已进入”。completed 展示真实开始 / 完成时间，terminated 展示可用的开始时间与真实终止时间，expired 展示可用的开始时间与真实过期时间；terminal 不展示 credential 或通用会话有效期。`revision` 继续完整用于 expectedRevision、stale response 防护与 conflict reconciliation，但不作为正常医护摘要字段展示
...
- `PatientAdministrationPreparation.tsx`：管理 `screen`、`input`、`sound`、`microphone` 四项 required device facts、八类影响因素和最长 500 字备注；四项只在组件内存中决定 `ready`。WebAudio 只播放合成测试音；`microphone` 只有本轮 MediaRecorder 最终形成 `Blob.size > 0` 且生成本地回放 URL 后才为 true，浏览器不支持、权限 / 设备失败、recorder 初始化 / 运行失败或空 Blob 均保持 false，每次重试开始先清除旧成功事实。MediaRecorder 最长生成 10 秒本地试听 Blob；默认折叠的“设备操作练习（可选）”保留 Canvas Pointer 练习，但不参与 `ready`。reset、重启或卸载会淘汰旧麦克风 run，迟到 stream 立即停止且不创建 recorder 或写状态；Blob URL 与持有的 track 在替换 / 卸载时释放。same-device staff 与 cross-device patient 共用该组件；检查和练习均不上传、不保存、不创建 `MediaEvidence`、不修改 `ItemResponse`。
...
- `types/patient-administration.ts`：只建模公开 staff / patient 会话、nullable summary `deviceMode`、required create input、nullable create `entryCode`、non-null reissue `entryCode`、revision、准备 / 控制、current step / asset、evidence response 与写请求白名单；current asset 含 `technicalReplayAuthorized:boolean`。不定义后端内部 token、hash、Session、asset file path、授权 count / history / reason / operator、审计对象或正式评分字段。
- `api/patient-administration-api.ts`：唯一 patient-administration fetch 所在；create 必须发送调用方明确选择的 `deviceMode`，不提供默认或旧 backend fallback；F2 增加 current private image、audio play、multipart evidence、patient / staff complete、takeover、redo 与 replay authorize。全部路径 ID 编码，统一 `credentials: include` / `no-store`，GET 接收 AbortSignal，写请求结果不确定映射为只读核对语义且不自动 retry。
...
- adoption 职责：仅对后端合同允许的 completed session、有效 capture、stored/attached photo 或 handwriting、且父页面 authoritative requirement 仍 pending/missing 的证据开放显式采用；成功把同一 Evidence 的 authoritative requirement（含 mediaEvidenceId）回传父页面并通过统一 mutation callback 自动重载 readiness，不自动采用、不复制 Evidence。adopted 状态不使用本地 `adoptedIds` 缓存，只在 requirement 同 evidenceType、status=attached、attached=true 且 mediaEvidenceId 精确等于当前 Evidence 时显示“患者证据已采用”；pending / null 在同一 React tree 中立即恢复采用按钮，指向同类型另一 Evidence 时显示“该题已有同类型正式证据”并禁用。`adoptingIds` 只保留为请求中的瞬态状态。患者原始 provenance 卡继续保留；同一 Evidence adoption 后可同时出现在患者参考与正式题目媒体区。
