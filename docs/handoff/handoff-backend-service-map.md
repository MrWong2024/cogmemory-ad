...
- 当前方法：`getVisitExecutionDetail()` 返回实际 ItemResponse 进度与 `visitMaintenance`；`ensureVisitAndScaleStarted()` 以完整 patient / visit / scale ownership 条件统一闭合父级首次开始；`hasCompletedPatientAdministrationSessionForScaleInstance()` 只按目标实例与 `status=completed` 查询历史成功终点；另有 `updateVisitForPatient()`、`deleteVisitForPatient()`、`voidVisitForPatient()`。A14 / A15 / A30 既有联合读取和 barrier 条件写方法保持不变。
...
- 职责边界：绑定 A15 七个题目级媒体路由（list / upload / adopt / revoke-adoption / access-url / transcribe / void）、路径 / body / query DTO、`SessionAuthGuard`、`RolesGuard`、临床角色和 multipart 内存文件接收；adopt / revoke-adoption 均复用 `MediaEvidenceParamDto` 且无 Body，Controller 不直接操作 Model 或 Storage。
...
- 当前方法：`listEvidence()`、`uploadEvidence()`、`adoptPatientAdministrationEvidence()`、`revokePatientAdministrationEvidenceAdoption()`、`createAccessUrl()`、`voidEvidence()`。
...
- 上传编排：校验证据要求、captureMode、主文件和可选轨迹；生成不含患者隐私与原始文件名的 UUID objectKey；依次上传 Storage、创建 MediaEvidence、条件绑定 evidenceRef。绑定仅允许父 / 子 barrier open、同 evidenceType、mediaEvidenceId 空且状态 pending / missing 的数组元素，形成并发边界；miss 后重读屏障并精确归类。正式 upload 在通用 ownership / editable / requirement 门禁之后、任何媒体或 Storage 写入之前，额外拒绝 `evidenceType=handwriting + supervised_patient_input + completed PatientAdministrationSession`，返回 409 `MEDIA_EVIDENCE_HANDWRITING_RECAPTURE_NOT_ALLOWED`；该窄门禁不进入 patient Evidence upload、adoption / revoke-adoption，不影响 clinician-administered handwriting 或 completed supervised photo。upload / adoption / revoke-adoption / void 的 `EvidenceRequirementStateResponse` 均从对应 attach / clear 返回的实际写后 ItemResponse evidenceRef 映射 status、attached 与 mediaEvidenceId，不另建状态源。
...
- 撤销正式采用：patient original MediaEvidence 与 formal `ItemResponse.evidenceRef` 是两层事实。revoke 只允许 patient-origin attached / unlocked / undeleted Evidence 且当前存在精确 attached ref；唯一业务写是 `clearItemEvidenceReference()`。MediaEvidence 的 status / storageStatus / voidedAt / deletedAt / metadata / `patientAdministrationContext` 与 Storage object 均不变，因此 review、access 与再次 adoption 保持可用。
...
- 访问 / 作废：签名访问固定使用 `DEFAULT_SIGNED_URL_EXPIRES_SECONDS`，仍只允许 attached / locked + stored，不为 voided 放宽。generic void 在 clear 前拒绝所有 patient-origin Evidence；仅 direct formal Evidence 继续 clear ref、标记 MediaEvidence voided并在后者失败时尝试恢复引用。restore 仅针对本次 clear 留下的空 pending 引用，是受控补偿例外，不受普通 barrier 门禁开放；正常作废不调用 deleteObject。
...
- 职责边界：public mapper 显式逐字段白名单映射、把非有限数归一化为 null，并只从 `patientAdministrationContext != null` 派生安全 `patientAdministrationOrigin` boolean；不公开 context、Storage 或其他内部字段。图片校验负责大小 / MIME / 魔数 / 元数据 / SHA-256；轨迹校验负责 application/json、2 MiB、结构限额、危险 key、深克隆、规范化 Buffer 与 SHA-256。纯函数不依赖 Nest DI、数据库或 Storage。
...
- 职责：依次读取 Patient / Visit / ScaleInstance，校验 definition / version 与 ItemResponse 归属和追溯；对 `supervised_patient_input` 从 `AssessmentsService` 读取 completed PatientAdministrationSession 事实并交给同一 readiness evaluation，其它施测方式显式视为不需要该门禁。readiness 以 `SCALE_INSTANCE_PATIENT_ADMINISTRATION_INCOMPLETE` 作为 blocking scale-level issue，正式 submit 复用同一 evaluation，不能由前端隐藏或直接 API 调用绕过；其余固定 scope、父 / 子屏障 fencing、二次实时读取、fenced completion、失败 release、阶段恢复、操作者优先级、startedAt / duration、幂等与并发 miss 重读保持不变。单请求最多执行 12 次有界状态转换，不 polling / sleep。
