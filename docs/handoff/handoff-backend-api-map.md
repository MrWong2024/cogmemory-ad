...
- binary manual 正式结构：`{ binaryManualDecision: { isCorrect: boolean | null } }`。仅上述 eligible item 接受且 root / nested shape 必须精确；null 是合法 partial draft，scoreValue / correctScore / maxScore / note / decidedBy / decidedAt 等未知或可伪造字段统一 400 `ITEM_RESPONSE_PAYLOAD_INVALID`。本次未提交 structuredResponse 时继续安全保留历史 stored value
- MMSE 1.0 阅读并执行 observation overlay 继续复用既有字段：`responseText` 记录患者实际念出的内容或“未能读出”等观察，`rawResponse` 的 null / true / false 分别表示闭眼动作未记录 / 已按要求闭眼 / 未按要求闭眼，`binaryManualDecision` 独立记录最终 0/1 判断。普通 partial draft 可只填其中一项或均为空；客户端主动提交 rawResponse 时只接受 null / boolean，其他类型为 400 `ITEM_RESPONSE_PAYLOAD_INVALID`，该收紧不影响其他 generic item
...
- 状态约束：Patient 必须 active；Visit 与 ScaleInstance 必须 draft / in_progress；ItemResponse 必须 not_started / in_progress / answered。`supervised_patient_input` 还必须由服务端确认目标实例历史存在 completed PatientAdministrationSession；无 Session 或 prepared / active / paused / terminated / expired 均返回 409 `PATIENT_ADMINISTRATION_NOT_COMPLETED`，clinician-administered 不查询该门禁。有效草稿使 not_started 进入 in_progress；普通题 markAsAnswered=true 且存在有效作答后进入 answered；有可执行 structured fields 的非 missing 题还必须在最终合并结果中恰好包含全部 code、每个 responseText trim 后非空且 isCorrect 为 boolean；binary eligible 的非 missing 题必须同时保留既有有效原始作答并具有 boolean `binaryManualDecision.isCorrect`，decision 自身不算 answer content；阅读并执行 overlay 的非 missing 题还必须同时具有非空 `responseText`、boolean `rawResponse`（false 合法）和 boolean decision。任一完成门禁失败返回 409 `ITEM_RESPONSE_CANNOT_MARK_ANSWERED`。missing 题继续只按 missingReason 合同，不要求以上三项；answered 后编辑不回退；不产生 scored
...
- 写库与安全边界：成功 CAS 在同一次更新中写草稿字段、`$inc draftRevision: 1` 与服务端 `draftSavedAt`；只有该原子写成功后，才以同一个 `draftSavedAt` 条件启动当前 ScaleInstance 与所属 Visit（startedAt 为空时写入，draft 推进为 in_progress，既有首次时间不覆盖）。stale revision、非法 payload、CAS miss 或保存失败不启动父级；患者施测后的医生复核只保存草稿，不覆盖患者首次开始时间。structuredResponse 中的 isCorrect 仅保存医护确认，不写客户端分数；binary decision 不替代 responseText / rawResponse / evidence 等原始事实，`rawResponse=false` 仍是有效记录。阅读文本、闭眼动作与最终评分没有自动映射，不采用 ASR 自动填充或评分。不修改 evidenceRefs，不回写 ScaleInstance.progress，不执行评分，不修改父级其他生命周期 / 审计 / barrier 字段、expectedValue / stepResults.isCorrect / scoreValue / countsTowardItemScore / countsTowardScore，不记录作答内容到日志
...
- 状态约束：Patient active；Visit / ScaleInstance draft 或 in_progress；ItemResponse not_started / in_progress / answered；父 / 子 submission barrier 必须 open；evidenceRefs 必须存在同类型 pending / missing 要求且无当前 attached / locked 证据。`supervised_patient_input` 且已有 completed PatientAdministrationSession 时，正式 upload 不允许新建 handwriting；患者原始 handwriting 应经独立 adoption 动作进入正式引用。clinician-administered handwriting 与 completed supervised 的 photo upload 继续按既有合同处理
...
- 检查：Patient -> Visit -> ScaleInstance 完整归属；definition / version 四项绑定；`supervised_patient_input` 由后端读取目标实例是否存在 completed PatientAdministrationSession；期望 / 实际 itemCode 集合；全部题目含 countsTowardTotal=false 项的完成状态；有效原始作答、missing reason、structured_manual 完整性、MMSE 阅读并执行的实际阅读文本 + boolean 闭眼动作完整性、binary manual 判断完整性、必填 step、计时、photo / handwriting、operatorNote 与 evidenceRef 一致性。历史 free-text-only answered structured item、历史阅读题缺少任一原始事实、历史 answered binary item 无 boolean decision 均不能绕过提交；missing item 不要求 observation / decision
...
- issue：supervised 未完成时稳定返回 blocking、scale-level `SCALE_INSTANCE_PATIENT_ADMINISTRATION_INCOMPLETE`，真实计入 blockingIssueCount 并使 ready / canSubmitNow=false；另返回 `SCALE_INSTANCE_ITEM_SET_MISMATCH`、`ITEM_NOT_COMPLETED`、`ITEM_ANSWER_CONTENT_MISSING`、`ITEM_STRUCTURED_SUBITEMS_INCOMPLETE`、`ITEM_MANUAL_OBSERVATION_INCOMPLETE`、`ITEM_BINARY_MANUAL_DECISION_INCOMPLETE`、missing / step / timing / media / note 等受控 code。原始观察不完整与评分判断不完整职责独立，decision 也不满足 answer-content 检查，故这些 issue 可独立并存；scale scope 在前、item 按 order / code 排序
...
- 首次提交：要求 active Patient、draft / in_progress Visit、draft / in_progress ScaleInstance 与无 blocking issue；supervised completed Session 门禁由同一 readiness evaluation 强制执行，直接调用 submit 或历史脏 ItemResponse 都不能绕过。第一次 readiness 后固化完整、稳定排序且无重复的 ItemResponse scope，以 UUID barrierId、首次 actor / 时间原子建立父 `fencing`，再幂等写入并重读验证同 token 子屏障，推进父 `fenced`
...
- 评分语义：严格 `multi_step_manual` 沿用既有 number / boolean 同类型规则；可执行 `structured_manual` 仅按医护已确认的 `isCorrect === true` 汇总对应 maxScore，再走统一 range / step 校验，不比较 responseText 与 referenceAnswer。binary eligible item 只把医护明确确认的 boolean decision 确定性映射为 false→0 / true→1，再用统一 range / step 校验，输出 auto_scored / auto_rule / includedInTotal=true；系统不识别患者回答、不做临床 / ASR / AI 判断，rawResponse 或 Evidence 不自动决定分数。binary decision 缺失或非法的历史数据继续 `MANUAL_SCORING_REQUIRED`；structured 非法 / 不完整存量数据仍以 `STRUCTURED_RESPONSE_INVALID` 进入 review，无法解析字段定义仍为 `MANUAL_SCORING_REQUIRED`。其他人工 / 未知模式、missing、既有题分进入 reviewQueue；非计分过程项排除
...
- Param / Body DTO：`ScoreResultParamDto`；`ConfirmScoreResultDto { confirm, reviewNote, expectedUpdatedAt }`。业务要求 confirm 严格 true，否则 400 `SCORE_RESULT_CONFIRMATION_REQUIRED`；reviewNote 保持必填 string shape，trim 后允许空字符串、最大 2000，expectedUpdatedAt strict ISO，force / ignoreWarnings / totals / metadata 等由 whitelist 拒绝。此放宽仅限最终确认；人工单题评分 reviewNote 仍为 3–2000。
...
- `POST /`：Body 必须为 `{ deviceMode: 'same_device' | 'cross_device' }`，缺失、非法值或额外字段为 400；设备方式在创建时持久化且不可修改。两种方式都创建 `prepared`、revision=0、绝对有效期两小时；prepared 不代表评估开始，Visit / ScaleInstance 继续保持 draft / startedAt=null。same-device 不生成或持久化进入码并返回 `entryCode=null`、`entryCodeExpiresAt=null`，cross-device 才签发十分钟六位码并仅持久化 hash。只允许 active Patient、可编辑 Visit / ScaleInstance、开放 submission barrier、`supervised_patient_input`、可解析步骤与 released package 全步骤 assetKey。该 `ScaleInstance` 完整 Session 历史中存在任意 completed 时，使用既有 409 `PATIENT_ADMINISTRATION_SESSION_CONFLICT` 永久拒绝再次创建，不能由 later terminated / expired 绕过；不存在 completed 历史时，terminated / expired 仍允许显式选择 deviceMode 后重新创建，prepared / active / paused 继续以同一 409 conflict 拒绝。
- `GET /`：返回该实例最新 staff session summary；不存在为 404 `PATIENT_ADMINISTRATION_SESSION_NOT_FOUND`。读取时惰性关闭已过期开放会话，不物理删除。
- `POST /handoff`：Body `{ expectedRevision }`；只允许 `deviceMode=same_device` 的 prepared / paused 且准备已确认。所有 ownership、有效期、revision、设备方式、状态、准备事实、凭证形态与继续施测检查都在撤销当前 staff Session 前只读完成；cross-device 或 legacy 无模式会话以既有 session conflict 拒绝且 staff Session 不被撤销。prepared 成功 CAS 签发患者 Token、转 active，并以 Session.startedAt 的同一个服务端时间首次启动当前 ScaleInstance 与 Visit；三者 startedAt 精确相等。paused 成功后仍为 paused，不改 Session 或父级 startedAt。Controller 随后沿用 revoke staff token → clear staff Cookie → patient CAS → set patient Cookie 的安全顺序；后段竞争失败不恢复 staff 身份且不设置新患者 Cookie。
- `POST /preparation/confirm`：Body 仍为 `{ expectedRevision, impactFactorCodes, impactFactorNote? }`；只允许未确认的 prepared，并按持久化设备方式校验凭证形态。same-device 必须尚无患者凭证，确认后保持 prepared且不写 Session / ScaleInstance / Visit startedAt，等待 handoff；cross-device 必须已经完成进入码兑换并存在患者凭证，确认后转 active，并以 Session.startedAt 的同一个服务端时间首次启动当前 ScaleInstance 与 Visit；legacy、same-device 带凭证、cross-device 无凭证均以既有 session conflict fail closed。成功记录准备时间、操作者、八类固定影响因素、`preparation_confirmed` event 并使单一 revision+1；第一次准备事实不可覆盖。
- `POST /pause` / `POST /resume`：Body `{ expectedRevision, reason? }`。pause 只允许 active 且不清除患者 Token；resume 要求 paused、准备已确认、患者凭证存在且底层仍可继续，不延长绝对过期时间，也不重置 Session / ScaleInstance / Visit 的首次 startedAt。
...
- `POST /patient-administration/current/evidence`：`PatientAdministrationSessionGuard`；`multipart/form-data`，文件字段固定 `file`、最多一个、最大 10 MiB，不接受 trajectory。Body 允许 expectedRevision、evidenceType（audio / photo / handwriting）、可选严格 ISO capturedAt、仅 audio 可用的 durationMs（1–600000），以及仅图片 / handwriting 可用的 optional imageWidth / imageHeight；handwriting 还可提交 optional strokeCount / trajectoryDurationMs / canvasWidth / canvasHeight / inputTool。缺少这些媒体属性不阻断旧客户端上传；未知 stepKey、stepRun、patient / visit / instance / item ID、captureMode、objectKey、文件名或任意通用 metadata 由白名单拒绝。
...
- 响应为 `PatientAdministrationReviewResponse { session, reviewEvents, items }`：session 只含状态、准备 / 影响因素及有限生命周期时间；reviewEvents 只含受控 action / 时间 / 原因 / 安全操作者；item 按权威步骤顺序分组，含安全 status / draftRevision、step / run、capture 与 evidence。evidence 只读投影安全 file mimeType / fileExtension / sizeBytes、imageMetadata、handwritingTrace 摘要、audioMetadata 与 transcription；不返回 storageDriver、bucket、objectKey / objectPrefix、publicUrl、checksum / algorithm、trajectoryObjectKey、签名 URL 或凭据。每个 `PatientAdministrationReviewStepResponse` 还返回 `structuredFieldCodes: string[]`，只表示 review-only 展示位置关联，不是正式答案、Evidence adoption、评分判断或评分；`[]` 表示没有可安全使用的具体字段关联。redo 的 invalidated run 和只有 evidence 的 run 均保留。
