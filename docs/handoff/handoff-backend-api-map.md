...
- 写库与安全边界：成功 CAS 在同一次更新中写草稿字段、`$inc draftRevision: 1` 与服务端 `draftSavedAt`；structuredResponse 中的 isCorrect 仅保存医护确认，不写客户端分数；binary decision 不替代 responseText / rawResponse / evidence 等原始事实，`rawResponse=false` 仍是有效记录。阅读文本、闭眼动作与最终评分没有自动映射，不采用 ASR 自动填充或评分。不修改 evidenceRefs、Visit / ScaleInstance 状态或 startedAt，不回写 ScaleInstance.progress，不执行评分，不修改 expectedValue / stepResults.isCorrect / scoreValue / countsTowardItemScore / countsTowardScore，不记录作答内容到日志
