...
- `POST /handoff`：Body `{ expectedRevision }`；只允许 prepared / paused 且准备已确认。所有 ownership、有效期、revision、状态、准备事实、凭证形态与继续施测检查都在撤销当前 staff Session 前只读完成；prepared 还要求不存在患者凭证，成功 CAS 签发患者 Token、转 active、首次写 startedAt 并清除进入码；paused 成功后仍为 paused，不改 startedAt。Controller 随后沿用 revoke staff token → clear staff Cookie → patient CAS → set patient Cookie 的安全顺序；后段竞争失败不恢复 staff 身份且不设置新患者 Cookie。
- `POST /preparation/confirm`：Body 仍为 `{ expectedRevision, impactFactorCodes, impactFactorNote? }`；只允许未确认的 prepared。成功始终记录准备时间、操作者、八类固定影响因素、`preparation_confirmed` event 并使单一 revision+1；无患者凭证时保持 prepared、不写 startedAt、不签发 Token且保留进入码，已有患者凭证时转 active并首次写 startedAt。重复确认、过期或 stale revision 均使用既有 session conflict，第一次准备事实不可覆盖。
