...
- `POST /`：Body 必须为 `{ deviceMode: 'same_device' | 'cross_device' }`，缺失、非法值或额外字段为 400；设备方式在创建时持久化且不可修改，选错时只能 terminate 后重新创建。两种方式都创建 `prepared`、revision=0、绝对有效期两小时；same-device 不生成或持久化进入码并返回 `entryCode=null`、`entryCodeExpiresAt=null`，cross-device 才签发十分钟六位码并仅持久化 hash。只允许 active Patient、可编辑 Visit / ScaleInstance、开放 submission barrier、`supervised_patient_input`、可解析步骤与 released package 全步骤 assetKey；同实例开放会话为 409 conflict。
