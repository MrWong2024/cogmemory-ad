...
- 状态约束：Patient 必须 active；Visit 与 ScaleInstance 必须 draft / in_progress；ItemResponse 必须 not_started / in_progress / answered。有效草稿使 not_started 进入 in_progress；普通题 markAsAnswered=true 且存在有效作答后进入 answered；有可执行 structured fields 的非 missing 题还必须在最终合并结果中恰好包含全部 code、每个 responseText trim 后非空且 isCorrect 为 boolean；binary eligible 的非 missing 题必须同时保留既有有效原始作答并具有 boolean `binaryManualDecision.isCorrect`，decision 自身不算 answer content；阅读并执行 overlay 的非 missing 题还必须同时具有非空 `responseText`、boolean `rawResponse`（false 合法）和 boolean decision。任一门禁失败返回 409 `ITEM_RESPONSE_CANNOT_MARK_ANSWERED`。missing 题继续只按 missingReason 合同，不要求以上三项；answered 后编辑不回退；不产生 scored
...
- issue：稳定返回 `SCALE_INSTANCE_ITEM_SET_MISMATCH`、`ITEM_NOT_COMPLETED`、`ITEM_ANSWER_CONTENT_MISSING`、`ITEM_STRUCTURED_SUBITEMS_INCOMPLETE`、`ITEM_MANUAL_OBSERVATION_INCOMPLETE`、`ITEM_BINARY_MANUAL_DECISION_INCOMPLETE`、missing / step / timing / media / note 等受控 code；原始观察不完整与评分判断不完整职责独立，decision 也不满足 answer-content 检查，故这些 issue 可独立并存；scale scope 在前、item 按 order / code 排序
