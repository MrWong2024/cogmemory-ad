...
- 写库与安全边界：成功 CAS 在同一次更新中写草稿字段、`$inc draftRevision: 1` 与服务端 `draftSavedAt`；structuredResponse 中的 isCorrect 仅保存医护确认，不写客户端分数；binary decision 不替代 responseText / rawResponse / evidence 等原始事实，`rawResponse=false` 仍是有效记录。不修改 evidenceRefs、Visit / ScaleInstance 状态或 startedAt，不回写 ScaleInstance.progress，不执行评分，不修改 expectedValue / stepResults.isCorrect / scoreValue / countsTowardItemScore / countsTowardScore，不记录作答内容到日志
...
- 检查：Patient -> Visit -> ScaleInstance 完整归属；definition / version 四项绑定；期望 / 实际 itemCode 集合；全部题目含 countsTowardTotal=false 项的完成状态；有效原始作答、missing reason、structured_manual 完整性、binary manual 判断完整性、必填 step、计时、photo / handwriting、operatorNote 与 evidenceRef 一致性。历史 free-text-only answered structured item以及历史 answered binary item 无 boolean decision 均不能绕过提交；missing binary item 不要求 decision
...
- issue：稳定返回 `SCALE_INSTANCE_ITEM_SET_MISMATCH`、`ITEM_NOT_COMPLETED`、`ITEM_ANSWER_CONTENT_MISSING`、`ITEM_STRUCTURED_SUBITEMS_INCOMPLETE`、`ITEM_BINARY_MANUAL_DECISION_INCOMPLETE`、missing / step / timing / media / note 等受控 code；decision 不满足 answer-content 检查，故两类 issue 可独立并存；scale scope 在前、item 按 order / code 排序
