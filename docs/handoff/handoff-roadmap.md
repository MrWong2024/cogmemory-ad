...
- 真实设备与人工签收继续归 WP-08；桌面 Browser 证据不能替代真实设备、真实硬件行为、主观体验或专业判断。
...
- WP-08 启动时必须基于 WP-10、WP-11、WP-12 的最终合同、标准设备和真实使用流程，按 [Codex instruction spec](../codex-instruction-spec.md) §3.9 重新治理当前自动化、真实设备与人工验收候选；不机械继承、重跑或维护历史 testing ID。
- WP-08 的完成标准是最终适用的自动化、真实设备和人工验收候选全部关闭；历史已完成 evidence 不替代因最终合同变化新产生或仍然适用的当前候选。
...
| WP-08 | 保留并重新定义 | 改为临床端到端联调与真实设备验收；最终验收候选依据 WP-10、WP-11、WP-12 完成后的当前合同生成 | 不适用；按本 roadmap 新定义实施 |
