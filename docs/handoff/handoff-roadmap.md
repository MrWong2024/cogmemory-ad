...
- 当前后端已具备 A1-A15 对应模型 / Service 底座、认证与临床 API，其中 A15 完成题目级 photo / handwriting 列表、上传、短期访问和作废；前端 B1-B4 仍只完成认证、患者 / 访视、实例初始化和逐题文字草稿页面，尚未接入 A15 文件采集。现状不代表完整患者管理、最终提交、批量 / 自动保存、媒体批量 / 直传 / 物理删除 / 原子替换、专用规则执行、计分触发、报告生成、疾病诊断或 AI 已完成。
...
当前进度：A1-A9 已完成量表、患者 / 访视 / 实例、ItemResponse、MediaEvidence、计分、认知域、报告、MMSE / MoCA seed 与评估初始化内部底座；A10-A11 已完成认证底座和三个认证 API；A12-A14 已完成患者 / 访视、目录 / 初始化、执行详情和单题草稿 API；A15 已完成题目级 photo / handwriting 列表、multipart 上传、短期签名访问与作废。A13 / A14 / A15 均不自动修改访视或实例状态、不执行最终提交；A15 也不修改 ItemResponse 状态或触发计分。前端 B1-B4 已完成认证、患者 / 访视、实例初始化和逐题手工草稿，但尚未接入媒体采集。当前公开 API 为 `GET /health`、三个认证 API、A12 五个、A13 三个、A14 两个与 A15 四个接口。

A15 接续更新：后端已开放题目下媒体证据列表、photo / handwriting multipart 上传、primary / trajectory 短期签名访问与作废四个 API；前端媒体采集、批量 / 分片 / 客户端直传、物理删除、原子替换、OCR / AI、计分和最终提交仍未实现。当前公开 API 清单应在上段 A14 口径上增加这四个 A15 接口。
