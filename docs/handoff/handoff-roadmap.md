...
- 当前前后端已形成“逐题记录 → 媒体证据 → readiness → 实例提交 → 阶段性评分 → 人工评分 → 评分确认 → 认知域计算与安全展示”的最小闭环。现状不代表完整患者管理、批量 / 自动保存、媒体批量 / 直传 / 物理删除 / 原子替换、评分 lock / void / rerun、认知域人工修改 / 确认 / 锁定 / 作废 / 重算、报告、疾病诊断或 AI 已完成。
...
当前进度：后端 A1-A19 与前端 B1-B9 已完成各自当前阶段。前端已在既有量表实例页安全接入 A19 认知域 compute / latest，未新增路由。
...
- 不实现 AI 能力。
- 不把 A12-A19 或 B2-B9 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，A18 confirmed 不等同于 locked，A19 computed 不等同于认知域 confirmed / locked、报告、诊断或 AI。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
