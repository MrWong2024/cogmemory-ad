...
- 后端 A1 已进入“量表定义模型底座”真实代码阶段：当前已新增 `scales` 内部模块、`ScaleDefinition` / `ScaleVersion` Mongoose Schema 和最小内部读取 Service。
- 本阶段 `scales` 仅代表量表定义与量表版本模型底座，不代表完整量表引擎、MMSE / MoCA 种子数据、评估执行、自动计分、报告或 AI 能力已完成。
...
当前进度：步骤 1 已在后端 A1 完成第一块模型底座，实现范围仅限量表定义 / 量表版本 Schema 与内部读取 Service。步骤 2 及之后仍未实现。

## 18. 当前非目标

- 除后端 A1 已落地的 `ScaleDefinition` / `ScaleVersion` Schema 外，不设计患者、访视、评估实例、作答、媒体证据、计分结果、报告、AI、随访或科研导出等后续数据库 Schema。
- 不定义业务请求 DTO 或前端调用契约。
...
- 不实现评估实例、作答记录、自动计分、报告生成、AI 解释、认证或权限。
- 不实现 AI 能力。
