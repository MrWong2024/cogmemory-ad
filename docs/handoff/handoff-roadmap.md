...
- 当前后端已具备 A1-A15 对应模型 / Service 底座、认证与临床 API，其中 A15 完成题目级 photo / handwriting 列表、上传、短期访问和作废；前端 B1-B5 已完成认证、患者 / 访视、实例初始化、逐题文字草稿与 A15 证据采集闭环。媒体操作与 A14 作答草稿相互独立，不修改题目、实例或访视状态，也不触发评分。现状不代表完整患者管理、最终提交、批量 / 自动保存、媒体批量 / 直传 / 物理删除 / 原子替换、专用规则执行、计分触发、报告生成、疾病诊断或 AI 已完成。
...
当前进度：A1-A9 已完成量表、患者 / 访视 / 实例、ItemResponse、MediaEvidence、计分、认知域、报告、MMSE / MoCA seed 与评估初始化内部底座；A10-A11 已完成认证底座和三个认证 API；A12-A14 已完成患者 / 访视、目录 / 初始化、执行详情和单题草稿 API；A15 已完成题目级 photo / handwriting 列表、multipart 上传、短期签名访问与作废。A13 / A14 / A15 均不自动修改访视或实例状态、不执行最终提交；A15 也不修改 ItemResponse 状态或触发计分。前端 B1-B5 已完成认证、患者 / 访视、实例初始化、逐题手工草稿，以及 photo / handwriting 采集、预览和作废闭环。当前公开 API 为 `GET /health`、三个认证 API、A12 五个、A13 三个、A14 两个与 A15 四个接口。

A15 接续更新：后端已开放题目下媒体证据列表、photo / handwriting multipart 上传、primary / trajectory 短期签名访问与作废四个 API；前端 B5 已接入这四个接口，并在浏览器内完成 photo 元数据清理重编码和平板手写最终 PNG / strokes 轨迹生成。批量 / 分片 / 客户端直传、物理删除、原子替换、OCR / AI、计分和最终提交仍未实现。
...
- 不新增除 `GET /health`、三个认证 API、A12 五个患者 / 访视 API、A13 三个评估初始化前置 API、A14 两个评估执行草稿 API 与 A15 四个媒体证据 API 以外的公开 API 路径。
- 除已落地的 `/`、`/login`、`/dashboard`、B2 四条患者 / 访视路由、B3 访视详情路由与 B4 量表实例施测执行路由外，不实现其他前端业务页面路由。
- 不实现完整量表引擎。
- 不实现 MMSE / MoCA 整份量表最终提交、自动或手工评分和报告页面；B4 负责安全施测展示与逐题手工草稿记录，B5 仅补充题目级 photo / handwriting 证据采集、预览和作废，不把上传视为题目完成或评分。
- 不实现 A14 单实例执行详情、单题草稿 PATCH 与 A15 四个媒体证据接口之外的其他公开评估执行 API。
- 不实现全量数据库 seed runner、量表配置编辑或公开完整 MMSE / MoCA 题目配置查询 API；A13 仅在初始化时按需物化对应 seed 版本。
- 不实现整份量表最终提交、批量或自动保存、媒体批量 / 分片 / 客户端直传、永久公开 URL、物理删除或原子替换、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、作答提交后自动计分或认知域计算触发、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、PDF 导出、疾病诊断、AI 解释、公开用户管理 API、权限菜单或完整权限矩阵管理。
- 不实现 AI 能力。
- 不把 A12 / A13 / A14 / A15 / B2 / B3 / B4 / B5 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，B4 手工草稿和 B5 题目媒体证据也不等同于整份量表最终提交、计分、报告或 AI。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
