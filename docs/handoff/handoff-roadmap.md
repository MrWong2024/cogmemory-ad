...
- 当前 `scales`、`patients`、`assessments`、`media`、`scoring`、`cognitive-domains`、`reports`、`users`、`auth` 与前端 B1 / B2 / B3 仅代表量表定义、MMSE / MoCA 初始配置 seed 与安全目录摘要、初始化时按需物化、患者档案、访视、访视详情、量表执行实例与题目作答骨架初始化、媒体证据元数据、计分结果快照、通用计分汇总、认知域结果快照、通用认知域汇总、临床报告快照、报告状态语义校验、系统账号、服务端会话、会话校验、角色装饰器、Guard、最小公开认证 API、患者 / 访视五个 A12 API、三个 A13 API，以及前端认证、患者 / 访视页面和量表实例初始化闭环。前端目录只展示安全摘要，不显示完整题目或评分规则；这不代表完整量表引擎、完整患者管理、完整评估流程、公开用户管理 API、权限菜单、完整权限系统、全量数据库 seed runner、作答查询 / 保存 / 提交、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 题目页面或专用规则执行、计分触发、报告生成、医生确认写库、报告归档 / 更正 / 作废、疾病诊断或 AI 能力已完成。
...
- 除 A12 / A13 已确认的患者、访视、量表目录与初始化 DTO / 公开响应外，不提前定义其他业务请求 DTO 或前端调用契约。
- 不新增除 `GET /health`、三个认证 API、A12 五个患者 / 访视 API 与 A13 三个评估初始化前置 API 以外的公开 API 路径。
- 除已落地的 `/`、`/login`、`/dashboard`、B2 四条患者 / 访视路由与 B3 访视详情路由外，不实现其他前端业务页面路由。
- 不实现完整量表引擎。
- 不实现 MMSE / MoCA 页面。
- 不实现 A13 访视详情 / 安全目录 / 量表实例初始化之外的公开评估执行 API。
...
- 不实现 AI 能力。
- 不把 A12 / A13 / B2 / B3 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，前端也尚未接入 ItemResponse 查询 / 保存 / 提交、真实题目作答、媒体、计分、报告或 AI。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
