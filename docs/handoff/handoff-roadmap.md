...
- 当前 `scales`、`patients`、`assessments`、`media`、`scoring`、`cognitive-domains` 仅代表量表定义、患者档案、访视、量表执行实例、题目作答记录、媒体证据元数据、计分结果快照、通用计分汇总、认知域结果快照和通用认知域汇总的模型 / Service 底座，不代表完整量表引擎、真实患者建档流程、真实作答提交流程、公开业务 API、MMSE / MoCA 种子数据、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、报告、疾病诊断或 AI 能力已完成。
...
- 除后端 A1 已落地的 `ScaleDefinition` / `ScaleVersion` Schema、后端 A2 已落地的 `Patient` / `AssessmentVisit` / `ScaleInstance` Schema、后端 A3 已落地的 `ItemResponse` Schema、后端 A4 已落地的 `MediaEvidence` Schema、后端 A5 已落地的 `ScoreResult` Schema，以及后端 A6 已落地的 `CognitiveDomainResult` Schema 外，不设计报告、AI、随访或科研导出等后续数据库 Schema。
- 不定义业务请求 DTO 或前端调用契约。
...
- 不实现真实作答提交、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、作答提交后自动计分或认知域计算触发、报告生成、疾病诊断、AI 解释、认证或权限。
- 不实现 AI 能力。
