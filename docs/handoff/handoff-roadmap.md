...
- 当前后端已具备 A1-A18 对应模型 / Service 底座、认证、临床 API，以及“阶段性评分 → 人工复核 → 显式确认”的最小评分闭环；前端 B1-B7 仍只形成“逐题记录 → 媒体证据 → 完整性检查 → 实例提交 → 阶段性评分 → 待人工复核展示”的闭环，尚未接入 A18 写接口。现状不代表完整患者管理、批量 / 自动保存、媒体批量 / 直传 / 物理删除 / 原子替换、前端人工复核 / 确认、认知域、报告、疾病诊断或 AI 已完成。
...
当前进度：A1-A15 已完成既有模型、认证、患者 / 访视、执行草稿与媒体证据能力；A16 已新增 submission readiness 与 submit 两个 API。A16 完成 `ScaleInstance`，但不完成或锁定 `AssessmentVisit`，不修改 `ItemResponse`，不创建评分、认知域或报告结果。前端 B1-B7 已完成认证、患者 / 访视、实例初始化、逐题手工草稿、媒体采集、readiness 问题定位、正式实例提交、阶段性评分查询 / 明确确认计算和待人工复核展示；本地 dirty / 写请求会阻断提交与首次评分计算。当前前端在 A16 两个接口基础上新增对接 A17 compute / latest。
...
A18 接续更新：后端已开放单题 manual-review 与 ScoreResult confirm。人工评分不修改原始作答，逐次追加受控审计，并使用 expectedUpdatedAt 防止并发覆盖；全部计分项完成后重新派生百分比，显式确认后为 confirmed / isFinal=true，但不是 locked。当前仍无 lock、void、撤销确认、reopen、重跑、认知域、报告或 AI；前端 B7 尚未接入 A18。
...
- 不新增除当前 A11-A18 已确认接口以外的公开 API 路径。
- 除已落地的 `/`、`/login`、`/dashboard`、B2 四条患者 / 访视路由、B3 访视详情路由与 B4 量表实例施测执行路由外，不实现其他前端业务页面路由。
- 不实现完整量表引擎。
- 不实现前端人工评分 / 确认页面或报告页面；A18 本阶段只完成后端单题人工复核与评分确认 API。
...
- 不实现 AI 能力。
- 不把 A12-A18 或 B2-B7 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，A18 confirmed 不等同于 locked、认知域、报告、诊断或 AI。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
