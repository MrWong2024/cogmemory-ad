...
- 当前后端已形成“逐题记录 → 媒体证据 → readiness → 实例提交 → 阶段性评分 → 人工评分 → 评分确认 → 认知域计算与安全展示 → 访视级规则报告 → 受控编辑 → 提交待确认 → 医生 / 管理员确认 → 不可逆锁定”的最小闭环；前端当前接至确认。现状仍不代表完整患者管理、批量 / 自动保存、媒体批量 / 直传 / 物理删除 / 原子替换、评分 lock / void / rerun、认知域人工修改 / 确认 / 锁定 / 作废 / 重算、报告退回 / 签名 / unlock / 归档 / 更正 / 作废 / PDF、疾病诊断或 AI 已完成。
...
当前进度：后端 A1-A22 与前端 B1-B11 已完成各自当前阶段。后端已形成报告 generate / latest / edit draft / submit confirmation / confirm / lock 闭环；下一阶段为前端 B12 锁定确认与安全展示。
...
- 不实现 AI 能力。
- 不把 A12-A22 或 B2-B11 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，A18 confirmed 不等同于 locked，A19 computed 不等同于认知域 confirmed / locked，A21 confirmed 不自动等于 locked，A22 只锁当前 ClinicalReport 而不锁来源数据。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
