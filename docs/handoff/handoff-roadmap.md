...
- 当前 `scales`、`patients`、`assessments`、`media`、`scoring`、`cognitive-domains`、`reports`、`users`、`auth` 与前端 B1 / B2 / B3 / B4 仅代表量表定义、MMSE / MoCA 初始配置 seed 与安全目录摘要、初始化时按需物化、患者档案、访视、访视详情、量表执行实例与题目作答骨架初始化、单实例安全执行详情、单题手工作答草稿保存、媒体证据元数据、计分结果快照、通用计分汇总、认知域结果快照、通用认知域汇总、临床报告快照、报告状态语义校验、系统账号、服务端会话、会话校验、角色装饰器、Guard、最小公开认证 API、患者 / 访视五个 A12 API、三个 A13 API、两个 A14 API，以及前端认证、患者 / 访视页面、量表实例初始化、动态安全题目展示和逐题手工草稿记录闭环。B4 不显示完整评分规则、正确答案或评分字段；这不代表完整量表引擎、完整患者管理、完整评估流程、整份量表最终提交、批量或自动保存、媒体上传 / 下载 / 签名 URL、专用规则执行、计分触发、报告生成、医生确认写库、报告归档 / 更正 / 作废、疾病诊断或 AI 能力已完成。
...
当前进度：步骤 1 已在后端 A1 完成模型底座，实现范围仅限量表定义 / 量表版本 Schema 与内部读取 Service；步骤 2 已在后端 A2 完成模型底座，实现范围仅限患者 / 访视 / 量表实例 Schema 与内部读取 Service；步骤 3 已在后端 A3 完成模型底座，实现范围仅限 `ItemResponse` Schema 与内部读取 Service；步骤 4 已在后端 A4 完成模型底座，实现范围仅限 `MediaEvidence` Schema 与内部读取 Service；步骤 5 已在后端 A5 完成计分结果模型与通用计分汇总底座，实现范围仅限 `ScoreResult` Schema、内部读取 Service 与基于单题得分快照的 `summarizeItemScores()` 纯函数；步骤 6 已在后端 A6 完成认知域结果模型与通用认知域汇总底座，实现范围仅限 `CognitiveDomainResult` Schema、内部读取 Service 与基于单题得分快照和认知域映射快照的 `summarizeDomainScores()` 纯函数；步骤 7 已在后端 A7 完成临床报告模型与医生确认流程底座，实现范围仅限 `ClinicalReport` Schema、内部读取 Service 与报告状态转换校验纯函数；步骤 8 已在后端 A8 完成 MMSE / MoCA 初始配置种子数据底座；步骤 9 的后端内部编排底座已在后端 A9 完成，并在 A13 通过三个最小公开 API 接入访视详情、目录读取和量表实例初始化。A13 仅在初始化时按需物化 seed、创建 `ScaleInstance` 与初始 `ItemResponse` 骨架，并使用补偿式一致性；A14 已开放单实例安全执行详情与单题手工草稿 PATCH，进度从实际 ItemResponse 派生。A13 / A14 均不自动变更访视或实例状态、不执行最终提交、不计分。后端 A10 / A11 已完成认证底座和三个认证 API；后端 A12 已完成患者 / 访视五个受保护 API；前端 B1 / B2 / B3 / B4 已完成认证、患者 / 访视列表与创建、访视详情、安全量表目录、量表实例初始化、真实安全施测执行页与逐题手工作答草稿保存。患者编辑 / 删除 / 归档 / 合并、访视编辑 / 删除 / 状态流转、整份量表最终提交、批量或自动保存、媒体上传、自动或手工计分、认知域计算触发、报告、AI、用户管理、角色权限管理和权限菜单仍未实现。当前公开 API 为 `GET /health`、三个认证 API、A12 五个患者 / 访视 API、A13 三个评估初始化前置 API 与 A14 两个评估执行草稿 API。
...
- 除 A12 / A13 / A14 已确认的患者、访视、量表目录、实例初始化、执行详情与单题草稿 DTO / 公开响应外，不提前定义其他业务请求 DTO 或前端调用契约。
- 不新增除 `GET /health`、三个认证 API、A12 五个患者 / 访视 API、A13 三个评估初始化前置 API 与 A14 两个评估执行草稿 API 以外的公开 API 路径。
- 除已落地的 `/`、`/login`、`/dashboard`、B2 四条患者 / 访视路由、B3 访视详情路由与 B4 量表实例施测执行路由外，不实现其他前端业务页面路由。
- 不实现完整量表引擎。
- 不实现 MMSE / MoCA 整份量表最终提交、自动或手工评分、媒体采集和报告页面；B4 仅实现安全施测展示与逐题手工草稿记录。
- 不实现 A14 单实例执行详情与单题草稿 PATCH 之外的其他公开评估执行 API。
- 不实现全量数据库 seed runner、量表配置编辑或公开完整 MMSE / MoCA 题目配置查询 API；A13 仅在初始化时按需物化对应 seed 版本。
- 不实现整份量表最终提交、批量或自动保存、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、作答提交后自动计分或认知域计算触发、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、PDF 导出、疾病诊断、AI 解释、公开用户管理 API、权限菜单或完整权限矩阵管理。
- 不实现 AI 能力。
- 不把 A12 / A13 / A14 / B2 / B3 / B4 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，B4 的真实手工草稿记录也不等同于整份量表最终提交、媒体、计分、报告或 AI。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
