...
当前进度：后端 A1-A25 与前端 B1-B14 已完成各自当前阶段。当前报告闭环增加 archived source corrections：源报告保留并转 corrected，replacement 成为 latest 且可进入独立 A21；仍无 correction cancel / branch / replacement lock / freeze / archive、PDF 或 AI。下一步先做前端 B14.1 等价 Hook 拆分，再进入 B15 更正 UI。
