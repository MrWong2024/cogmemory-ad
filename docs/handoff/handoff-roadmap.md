...
当前进度：后端 A1-A23 与前端 B1-B13 已完成各自当前阶段。当前报告闭环为 generate / latest / edit draft / submit confirmation / confirm / lock / freeze-sources；来源冻结不可撤销且支持由 doctor / admin 明确恢复同一 in_progress 流程。仍无 unlock / unfreeze、归档、更正、作废、签名、PDF 或 AI。
...
- 不实现 AI 能力。
- 不把 A12-A23 或 B2-B13 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，A18 confirmed 不等同于 locked，A19 computed 不等同于认知域 confirmed，A21 confirmed 不自动等于 locked；A22 / B12 只锁报告，A23 / B13 再按已锁报告固化的精确 scope 冻结来源，但不冻结 Patient、Visit 或 Storage，也不提供 unfreeze。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
