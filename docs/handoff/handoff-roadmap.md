...
当前进度：后端 A1-A22 与前端 B1-B12 已完成各自当前阶段。前后端已形成报告 generate / latest / edit draft / submit confirmation / confirm / lock 闭环；锁定不可撤销，仍无 unlock、归档、更正、作废、签名、PDF 或 AI。
...
- 不实现 AI 能力。
- 不把 A12-A22 或 B2-B12 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现，A18 confirmed 不等同于 locked，A19 computed 不等同于认知域 confirmed / locked，A21 confirmed 不自动等于 locked，A22 / B12 只锁当前 ClinicalReport 而不锁来源数据。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
