...
- 当前 `scales`、`patients`、`assessments`、`media`、`scoring`、`cognitive-domains`、`reports` 仅代表量表定义、患者档案、访视、量表执行实例、题目作答记录、媒体证据元数据、计分结果快照、通用计分汇总、认知域结果快照、通用认知域汇总、临床报告快照和报告状态语义校验的模型 / Service 底座，不代表完整量表引擎、真实患者建档流程、真实作答提交流程、公开业务 API、MMSE / MoCA 种子数据、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、疾病诊断或 AI 能力已完成。
