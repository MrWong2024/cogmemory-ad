...
- 不定义业务请求 DTO 或前端调用契约。
- 不新增除 `GET /health`、`POST /auth/login`、`POST /auth/logout`、`GET /auth/me` 以外的公开 API 路径。
- 不定义前端页面路由。
- 不实现完整量表引擎。
- 不实现 MMSE / MoCA 页面。
- 不实现公开评估执行 API 或量表实例初始化公开接口。
- 不实现数据库 seed runner、seed 写库或公开 MMSE / MoCA 配置查询 API。
- 不实现真实作答提交、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、作答提交后自动计分或认知域计算触发、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、PDF 导出、疾病诊断、AI 解释、公开用户管理 API、前端认证态或完整权限矩阵管理。
- 不实现 AI 能力。
