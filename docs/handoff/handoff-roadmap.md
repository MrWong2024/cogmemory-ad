...
- 当前 `scales`、`patients`、`assessments`、`media`、`scoring` 仅代表量表定义、患者档案、访视、量表执行实例、题目作答记录、媒体证据元数据、计分结果快照和通用计分汇总的模型 / Service 底座，不代表完整量表引擎、真实患者建档流程、真实作答提交流程、公开业务 API、MMSE / MoCA 种子数据、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、报告或 AI 能力已完成。
...
当前进度：步骤 1 已在后端 A1 完成模型底座，实现范围仅限量表定义 / 量表版本 Schema 与内部读取 Service；步骤 2 已在后端 A2 完成模型底座，实现范围仅限患者 / 访视 / 量表实例 Schema 与内部读取 Service；步骤 3 已在后端 A3 完成模型底座，实现范围仅限 `ItemResponse` Schema 与内部读取 Service；步骤 4 已在后端 A4 完成模型底座，实现范围仅限 `MediaEvidence` Schema 与内部读取 Service；步骤 5 已在后端 A5 完成计分结果模型与通用计分汇总底座，实现范围仅限 `ScoreResult` Schema、内部读取 Service 与基于单题得分快照的 `summarizeItemScores()` 纯函数。步骤 6 及之后仍未实现。

## 18. 当前非目标

- 除后端 A1 已落地的 `ScaleDefinition` / `ScaleVersion` Schema、后端 A2 已落地的 `Patient` / `AssessmentVisit` / `ScaleInstance` Schema、后端 A3 已落地的 `ItemResponse` Schema、后端 A4 已落地的 `MediaEvidence` Schema，以及后端 A5 已落地的 `ScoreResult` Schema 外，不设计认知域结果、报告、AI、随访或科研导出等后续数据库 Schema。
- 不定义业务请求 DTO 或前端调用契约。
...
- 不实现真实作答提交、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、作答提交后自动计分触发、报告生成、AI 解释、认证或权限。
- 不实现 AI 能力。
