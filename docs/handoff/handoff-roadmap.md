...
→ 医护施测人员及时查看步骤、状态、进度、患者求助和异常，并只在必要时暂停、接管、重做、technical replay、重签、恢复或终止
→ 完成结果和医护现场观察送入后续异常优先复核；医生能够展开必要事实、形成正式观察记录、记录修订原因并显式整体确认
...
`clinician_administered`、`supervised_patient_input + same_device` 与 `supervised_patient_input + cross_device` 是三种不同施测方式。same-device 的正常链为医护准备后安全交接、staff Session 失效、患者在同一设备连续施测、设备交回、医护 / 医生重新认证后复核；cross-device 中 staff 设备保留 staff Session、patient 设备持有 patient Session。cross-device 推荐用于受监督患者直接输入，但不是唯一方式；双设备是角色分工，不等于双方并发写同一 Session。
...
- cross-device 中 staff 设备保留 staff Session、patient 设备持有 patient Session；患者端负责正常题目和患者操作，医护端负责准备、现实观察、必要辅助与异常控制。双设备不等于双写者，正常 happy path 不要求医护逐题写入同一 Session。
...
- 医护施测模式支持选择患者 / 访视 / 量表、确认准备、发起交接或跨设备进入、查看进度和求助，以及在后续复核记录动作观察及结构化影响因素；异常时可暂停、接管、重做、technical replay、重签、恢复、换设备或终止。最低分类已由患者施测合同锁定，记录不自动改分或诊断。
