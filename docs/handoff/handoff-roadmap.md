...
- 当前 `scales`、`patients`、`assessments`、`media`、`scoring`、`cognitive-domains`、`reports` 仅代表量表定义、MMSE / MoCA 初始配置 seed、患者档案、访视、量表执行实例、题目作答记录、评估执行初始化内部编排、媒体证据元数据、计分结果快照、通用计分汇总、认知域结果快照、通用认知域汇总、临床报告快照和报告状态语义校验的模型 / Service 底座，不代表完整量表引擎、真实患者建档流程、真实作答提交流程、公开业务 API、数据库 seed runner、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则执行、MMSE / MoCA 专用认知域规则执行、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、疾病诊断或 AI 能力已完成。
...
- 不实现真实作答提交、媒体上传 / 下载 / 签名 URL、MMSE / MoCA 专用自动计分规则、MMSE / MoCA 专用认知域规则、作答提交后自动计分或认知域计算触发、报告生成接口、医生确认写库流程、报告归档 / 更正 / 作废接口、PDF 导出、疾病诊断、AI 解释、认证或权限。
- 不实现 AI 能力。
