...
- 后端 A24 已完成“已冻结 ClinicalReport 归档、安全审计、乐观并发与幂等公开 API”：新增 doctor / admin 专用 `POST .../:reportId/archive`，Body 只接收 `confirm=true`、`archiveNote`、`expectedUpdatedAt`。首次归档只接受完整 confirmed / mixed / passed、A22 已锁定且 A23 sourceFreeze completed 的报告，复用既有 `confirmed -> archived` 转换，以单文档条件更新写 status、archivedAt、archivedBy 与一次性 `metadata.a24Archive`；Patient inactive 与 Visit locked 不阻断，Patient / Visit 只用于存在性和 ownership。重复归档允许旧 expectedUpdatedAt 且不写库，历史 archived / corrected 支持安全 fallback；不重读或修改五类来源，不改变锁定、来源冻结、confirmation、narrative 或快照，不提供 unarchive / correction / void / PDF / AI。前端 B14 已完成归档确认、并发恢复与安全摘要展示接入。
- 后端 A25 已完成“归档报告版本化更正、替代报告创建、可恢复编排与安全审计最小公开 API”：新增 doctor / admin 专用 `POST .../:reportId/corrections`。源必须是 latest archived 报告并具备完整 A21 confirmation、A22 lock、A23 completed freeze 和 A24 archive 审计；原报告保留正文、快照、scope、确认、锁定、冻结与归档事实，只由 archived 转为 corrected 并追加一次版本替换记录。系统使用确定性 code 创建 version+1 的 draft / mixed / needs_review replacement，深复制固定快照和 A20 provenance、重置 A21-A24 生命周期；编排以 source `in_progress / completed` 审计恢复，不使用 transaction、不删除 replacement、不允许分支。合法 replacement 的 A21 edit / submit / confirm 仅 doctor / admin，且不因 Patient inactive 或 Visit locked / voided 阻断；普通 V1 规则不变，A22-A24 尚未泛化到 V2。
...
- 前端 B14 已在同一访视详情路由接入 A24 archive。仅 doctor / admin 对 confirmed、isFinal、mixed、passed、锁定摘要完整且 sourceFreeze completed 的报告显示首次归档入口；Patient active、Visit editable 与 Visit locked 不作为前端资格条件。请求只发送 `confirm=true`、用户填写的 `archiveNote` 与服务端 `report.updatedAt`；冲突 / 失败保留说明、清除 checkbox、最多 latest 一次且不自动重发。成功完整采用服务端 report，status 真实为 archived，顶层 archivedAt、archive 安全摘要和当前会话 receipt 分开展示；alreadyArchived 按幂等成功处理。完整 A24 摘要校验归档时间与 sourceFreeze 锚点；历史 fallback 的 archiveId、actor name / role 或锚点缺失时不猜测、不补写，也不开放再次归档。
- 前端 B14.1 已完成报告工作流 Hook 行为等价结构治理：保留唯一公开 `useClinicalReportWorkflow()` façade、原 9 个 options、原 99 个 result keys 与七个 mode；中央 reducer / coordinator 统一管理 activeMode、writingAction、mountedRef、writingRef、报告路由身份重置、错误恢复和 beforeunload，六类既有写动作拆入独立内部 Action。B14.1 没有新增用户功能、API、路由、页面、文案或 correction mode，也未接入 A25。
- 当前后端与前端已形成“逐题记录 → 媒体证据 → readiness → 实例提交 → 阶段性评分 → 人工评分 → 评分确认 → 认知域计算与安全展示 → 访视级规则报告 → 受控编辑 → 提交待确认 → 医生 / 管理员确认 → 不可逆锁定 → 来源冻结 → 报告归档”的最小闭环。归档后 status=archived，报告锁定与 completed 来源冻结事实继续保留；归档不等于删除、作废、更正或 PDF。现状仍不代表完整患者管理、批量 / 自动保存、评分 lock / void / rerun、认知域人工修改 / 确认 / 锁定 / 作废 / 重算、报告退回 / 签名 / unlock / unfreeze / unarchive / 更正 / 作废 / PDF、疾病诊断或 AI 已完成。
...
当前进度：后端 A1-A25、前端 B1-B14 业务能力与前端 B14.1 结构治理已完成各自当前阶段。当前报告闭环增加 archived source corrections：源报告保留并转 corrected，replacement 成为 latest 且可进入独立 A21；前端尚未接入 A25，仍无 correction cancel / branch / replacement lock / freeze / archive、PDF 或 AI。下一业务阶段为 B15 版本化更正交互。
...
B14 接续更新：前端继续复用 `/patients/[patientId]/visits/[visitId]`、PatientsWorkspaceContext、既有 report latest / 完整应用路径与 B11-B13 单一 writingAction，不新增路由、认证请求或第二套 Hook。归档草稿固定 reportId、服务端 updatedAt、锁定与 sourceFreeze completed 锚点，用户填写 3–2000 字 archiveNote 并勾选不可撤销确认；Patient active、Visit status / editable / locked 均不参与前端资格。冲突、not archivable、failed、voided、not found 最多 latest 一次，保留说明且不自动 POST；latest 已 archived 时只读展示并保留本地说明到明确关闭。成功完整应用服务端 report 与当前会话 receipt，status=archived；alreadyArchived 只表示幂等成功。完整摘要校验 archivedAt 与 sourceFreeze anchor，历史 fallback 不猜测缺失 ID、actor、说明或锚点。归档后 edit / submit / confirm / lock / source-freeze / archive 全部只读；仍无 unarchive / restore confirmed / correction / void / delete / unlock / unfreeze / PDF / Word / 下载或 AI。

B14.1 接续更新：本阶段仅将 1651 行 `useClinicalReportWorkflow.ts` 拆为 228 行 façade、中央 state/reducer、coordinator、recovery、单一 beforeunload 与六类 Action。公开 options / result、mode、角色、eligibility、dirty / stale、401 / 403、错误 latest 集合、请求 Body、成功文案、回执与 UI 行为保持不变；组件、API Client、ClinicalReport types、draft libs、display 和路由均未修改。A25 correction 仍未接入，下一业务阶段为 B15。
...
- 不新增除当前 A11-A25 已确认接口以外的公开 API 路径。
...
- 不实现撤销 / reopen / force submit、批量或自动保存、媒体批量 / 分片 / 客户端直传、永久公开 URL、物理删除或原子替换、MMSE / MoCA 专用自动计分、weighted mapping、认知域人工修改 / 确认 / 锁定 / 重算、提交后自动计分、报告退回 / 签名 / unlock / unfreeze / unarchive / 更正 / 作废 / 重生成 / version 2、PDF、疾病诊断、AI、公开用户管理或权限菜单。
- 不实现 AI 能力。
- 不把 A12-A25 或 B2-B14 写成完整患者管理或完整评估工作流；患者编辑 / 删除 / 归档 / 合并与访视编辑 / 删除 / 状态流转仍未实现。A22 / B12 只锁报告，A23 / B13 冻结来源，A24 / B14 归档，A25 建立线性 replacement，但不冻结或归档 Patient、Visit、Storage，也不提供 unfreeze / unarchive / correction cancel / branch / replacement archive。
- 不生成正式需求规格说明书。
- 不创建新的业务文档。
- 不处理任何代码初始化或工程配置。
