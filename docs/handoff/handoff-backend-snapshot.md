...
- `AssessmentsModule` 当前只提供访视 / 量表实例运行时 Schema 与内部 `AssessmentsService` 读取底座，不提供公开业务接口。
...
- 当前已有 `scales`、`patients` 与 `assessments` 内部模型底座，但无公开业务 API、认证、真实患者建档流程、评估执行业务接口、作答、媒体证据、计分或报告。
...
## 5. 当前 patients / assessments 运行时模型底座
...
- 当前已建立 `Patient` -> `AssessmentVisit` -> `ScaleInstance` 的运行时 ObjectId 引用关系，并在 `ScaleInstance` 中保存量表定义、量表版本和版本追溯快照字段。
- 当前未实现真实患者建档流程、访视创建流程、作答记录、媒体证据、自动计分、认知域结果、报告、AI、认证或权限。
...
- 尚无用户管理。
- 尚无医生端或患者端业务。
- 尚无公开患者、访视、量表实例或量表业务接口、评估、报告或诊断建议业务。
- 尚无短信发送接口。
- 尚无 AI / LLM 调用接口。
