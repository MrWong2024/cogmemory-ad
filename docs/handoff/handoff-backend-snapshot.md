...
- 医生侧工作台与临床工作流底座已经实现；尚未实现院内受监督患者施测所需的患者短期受控施测会话与安全进入、患者端和医护监管端步骤分工、医生及时状态查看和控制权合同、适用语音回答步骤的录音及一种基础 ASR 产生的转写候选、患者点击 / 书写 / 绘图等原始作答与必要过程事实、医护动作观察与实物操作结果的记录确认、基于这些事实完成医生复核并将确认结果受控形成或更新为正式 `ItemResponse`，以及基于服务端权威状态的患者端安全响应和会话恢复合同。
- 上述缺口是能力边界，不代表未来数据实体、集合数量、接口形态、事务边界或传输协议已经确定，也不预设独立 attempt / capture / review 集合、通用工作流或投影子系统、事件溯源、双写平台或专用投影队列；同设备交接不等同必须跨设备配对，跨设备安全进入也不预设二维码。现有 staff `Session`、`administrationMode` 值、`ItemResponse`、`MediaEvidence` 和提交屏障不能等同于受监督患者施测会话及其完整闭环，也不能推出强实时同步或特定事件平台。
