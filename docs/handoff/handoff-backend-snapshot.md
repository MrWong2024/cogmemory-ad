...
- `ensureSeedScaleVersionMaterialized()` 新插入 MMSE 时随 `$setOnInsert` 写入两项呈现字段；legacy MMSE 两字段均不存在时仍以 `_id` + 双 `$exists: false` 单次原子补齐并重读校验。已物化配置与 current seed 一致时零写复用；任意 stored drift 均返回 `SCALE_CATALOG_VERSION_CONFLICT` 且不覆盖。MoCA 行为不变，`currentVersionId` 仍只在为空或缺失时设置。
