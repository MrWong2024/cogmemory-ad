...
- 前端公共底座与 B1 登录页、认证状态、Auth API Client、工作台认证占位页已落地。
- `frontend\package.json` 已存在，自动验证命令以其中真实脚本为准。
- B1 不新增测试代码、测试框架、E2E 或第三方依赖。
- B1 只对接三个公开认证 API，不验证患者、评估、报告等业务流程。
...
- 确认静态能力卡片均标记“后续建设”，且没有患者、评估、报告等业务 API 请求。
...
- 确认登录页与工作台占位页符合浅色、低饱和蓝绿、清晰分区、大字号、少装饰、少动画的设计基线。
