...
- fixture 已提供量表作答/草稿、媒体文件与手写轨迹、readiness/最终提交、授权/错误/并发/网络最终状态等确定性前置；正式 namespace 已完成 prepared verify 并保留。临时上传文件只存在于操作系统临时目录，Browser 完成后的 cleanup 会一并删除。
- 键盘继续采用 18.7 固化的风险抽样策略：普通原生控件不设置全面 Tab/Shift+Tab/Enter/Space 门禁；画布、自定义复合控件、Modal/Dialog、焦点管理或可访问性修复仍须做真实键盘风险抽查。
- 本节只记录 fixture 前置。Batch B Browser 尚未执行，prepared verify 不等于 Browser 通过，post-browser verify 也尚未执行。
...
- 冒烟通过后已退出登录并关闭 Browser，`b456-media-smoke` 连续两次 cleanup 均为 `residualCount=0`；正式 `b456-browser-final` 已按修订合同 replace、prepared verify 并继续保留。
- Batch B 仍未完成；下一步必须从 B4 起完整重跑 B4–B6 / Batch B Browser 矩阵，本次结果只纠正媒体文件合同并提供定向回归证据。
