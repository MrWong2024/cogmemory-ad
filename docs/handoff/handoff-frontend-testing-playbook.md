...
| WP-10-F1 | 产品范围已完成，WP-10 状态不回退；`patient-administration-handoff` same-device / cross-device replacement 的 green 继续作为形成时策略下有效的历史证据。两个 `UI_FLOW_DISQUALIFIED` executable 已按 v1.20 物理退役，不再属于 current / future scripted UI regression gate | “WP-10-F1 最终证据与 Browser Audit 治理” |
| WP-10-F2 | 产品范围已完成且历史 P1 / P2 green 保留；两个 `UI_FLOW_DISQUALIFIED` executable 已按 v1.20 物理退役，不再属于 current / future scripted UI regression gate | “WP-10-F2 阶段与最终收口证据” |
...
v1.20 requalification：P1 的 19 步录音、Canvas / Pointer、file chooser / upload 与完成流程，以及 P2 经 UI 创建 / 准备 / 录音上传后的页面 reload / resume，主要 assertion target 均为 production UI workflow，因此均为 `UI_FLOW_DISQUALIFIED`。P2 的 server-authoritative 与 no-duplicate 不变量优先复用低层证据；当轮 requalification 没有识别出值得保留的独立 `NEEDS_THINNING` profile，P1 / P2 executable 现已物理退役。未来 objective UI restore / workflow 体验归 Agent-assisted，subjective / professional / real-device 归 human manual；不得恢复已退役的 UI Playwright body。
