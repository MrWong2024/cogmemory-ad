...
| Batch D / B11–B15 | B11 已完成；B12-G3-A3.2-R1 正式 core 执行已阻断；Batch D 尚未完成 | localhost build/runner/backend/CORS 预检通过；首个 `eg-doctor-eligibility-read-only` 因正式服务运行单元的外层 120 秒总超时而未 operationally pass。唯一 journal 为 12 owner record / 4 pass / 8 fail、minimal cleanup 全完成、group cleanup 失败；其余 9 group、两个 Stage、完整 verifier 与 profile aggregator 未执行。双次 cleanup 与资源收口通过，62 个 core Browser ID 均未关闭；B12-B、B12 与 Batch D 未完成，不进入 B12-C，B12-83 与 B12-86–B12-88 未关闭 |
...
B12-G3-A3.2 首轮基于 `207eb264c78362d5b9740d56de6809e85b0ba5e5`。frontend 四组 synthetic 为 20/20、11/11、15/15、35/35，完整 infrastructure 140/140，test list 31 files / 208 tests（旧 core 22、formal group 10、profile 1、canary 4、resilience formal 0），lint、typecheck 与 production build 均通过。全新 core namespace 的 prepare、prepared verify 与 23 个 runtime descriptor 通过，production frontend、Browser backend、health、CORS、credentials 与端口监听通过；Playwright runner 为数据库用途 `none`，workers=1、retries=0、max-failures=1，trace、video、screenshot 和 HTML report 均关闭。

第一个 `eg-doctor-eligibility-read-only` 在 group setup/auth 阶段失败：12 owner 全部 `blocked_by_group_setup`，0 pass，`failureCategory=group_setup_auth`，`stopReason=group_setup_failed`，group cleanup 成功；version 2 finalized journal 存在且只保留 `group_provisional`，没有用作 final closure。根因是 production build 的既有 API origin 为 `localhost:5002`，外层执行却向 runner/CORS 注入 `127.0.0.1` origin，真实认证响应无法匹配 adapter 的精确 backend-origin 等待条件；这是环境编排错误，不是产品、fixture、execution layer、formal adapter 或 Playwright/support 缺陷，因此未修改测试资产，也未使用第二个 namespace 重跑。其余 9 group、22 owner 的业务执行、两个 Stage、零写入 re-verify、完整 post-browser verifier、profile spec 与 62 项 closure 均为 `not_executed`；失败路径 post-browser 诊断返回 `B12_FIXTURE_POST_BROWSER_MUTATION_INVALID`。cleanup 1 为 `matched=true,residualCount=0`，cleanup 2 为 `matched=false,residualCount=0`，canonical seed 不变；BrowserContext、Chromium、服务、端口、runtime、Stage marker、journal、profile artifact、test-results 与 Session 最终均为 0。B12-B `core-workflow`、B12 整体与 Batch D 仍未完成，62 个 core ID 与其余 26 项均不关闭；下一阶段仍为在正确 origin 下用全新 namespace 完整重跑 G3-A3.2，不进入 resilience 正式执行，不填写 evidence commit。

B12-G3-A3.2-R1 基于 `main` / `f0f6134b2bca64af7011a39e959f61a1be0e559a` 的 clean 工作区执行；相对 `207eb264c78362d5b9740d56de6809e85b0ba5e5` 仅三份 handoff 文档变化，因此只重跑显式 origin production build 与正式 fixture/Browser 生命周期。build 明确设置 `NEXT_PUBLIC_API_BASE_URL=http://localhost:5002` 并通过，构建产物的两个禁止 HTTP origin 命中数均为 0。namespace 前临时 Playwright 预检实际打开 `http://localhost:3002/login`，页面 origin 与初始 `/auth/me` 响应 origin 分别精确为 localhost:3002 / localhost:5002；响应为 401，CORS allow-origin/credentials 正确，禁止 backend origin 请求数为 0，runner 与 backend 配置解析也均为固定 localhost 值。临时 Browser、服务、脚本、结果与端口在创建 namespace 前已收口。

唯一新 core namespace 的 prepare、prepared verify 与 23 runtime 均通过；正式拓扑的 health、login、`/auth/me` 401 与 CORS 再次以同一 localhost 字面值通过。首个 `eg-doctor-eligibility-read-only` 启动 1 个 BrowserContext / Session，但正式 frontend/backend 的受管运行单元被外层配置的 120 秒总超时终止，共享 Page/Context/Browser 随之关闭。Playwright exit 1；journal v2/finalized，`groupSetupSucceeded=true`、`groupCleanupSucceeded=false`、`stopReason=none`、`operationallyPassed=false`。12 个 owner 全部 started/finalized 且 minimal cleanup 完成：`draft-no-entry`、`pending-no-entry`、`confirmed-doctor-entry`、`quality-not-passed` 四个 record 为 pass；`finality-inconsistent`、`confirmation-missing`、locked/voided、already-locked-no-repeat 与三个 warning owner 共八个 record 为 `owner_assertion` fail。由于 group 未 operationally pass，这些 provisional owner record 不关闭任何 Direct Audit ID，也不与后续证据拼接。

其余 9 group / 10 owner、零写入阶段正式 re-verify、两个 Stage、post-browser verifier 与 profile aggregator 均未执行；Stage marker 始终为 0。停服后的诊断性 prepared verify 通过，证明产品业务写入为 0、fixture 保持 prepared、canonical seed 不变。cleanup 1 为 `matched=true,residualCount=0` 并删除剩余 11 runtime，cleanup 2 为 `matched=false,residualCount=0`；BrowserContext、Chromium、Node、Session、服务、端口、runtime、marker、journal、profile artifact、test-results 与临时产物均已收口。耗时为 build 23,694ms、origin 预检 4,540ms、prepare 18,566ms、prepared verify 15,232ms、runtime 380,916ms、首组 Playwright 65,315ms、诊断 verify 20,529ms、两次 cleanup 17,130ms / 16,364ms，build-to-cleanup 墙钟约 1,244,619ms；Stage、verifier、aggregator 与测试资产修改耗时均为 0。分类为外层环境编排阻断 1，已确认产品/fixture/测试资产缺陷均为 0。B12 core-workflow 未完成，62 个 core ID 与其余 26 项均不关闭；不执行 resilience、G3-B 或 B13～B15，不填写 evidence commit。
