...
Batch D 已完成 B11，并已进入 B12。B12-B2 已用全新 namespace 完整执行 `core-workflow` 的 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID；22 条 route 都实际启动并执行，但统一在 logout 后的 `/auth/me` 账本断言失败，最终为 0 route / 0 audit ID pass、22 route / 62 audit ID fail。完整 post-browser verify 与双次 cleanup 通过。B11 fixture 建立的两个互斥 profile 均已闭环：`core-workflow` 覆盖 58 个 Browser 项，`resilience-security` 覆盖 11 个 Browser 项；B11-70 是单独的 `static-gate`。共 70 个稳定 ID、69 个 Browser 项和 1 个静态项，每个 ID 只有一个 primary owner。B11 已完成事实保持不变。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C；B12-86–B12-88 未关闭，也尚未执行 B13–B15。
