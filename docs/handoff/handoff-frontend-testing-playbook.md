...
> [B12-G2.1 Draft v3 执行与证据矩阵](./handoff-testing-evidence-matrix-b12.md) 当前状态为：`G3-A approved — G3-A1 completed; G3-A2 completed; G3-A3.1 core matrix and profile evidence assets completed; G3-A3.2 core execution pending; G3-B deferred`。G3-A3.2 首轮已在第一个 formal group 的 setup/auth 阶段因外层 Browser origin 编排不一致停止，并完成诊断、双次 cleanup 与零残留收口；没有 owner pass、完整 verifier 或 profile closure，88 个 B12 ID 状态不变。下一阶段仍为使用正确 origin 和全新 namespace 完整重跑 G3-A3.2，不进入 B12-C。
