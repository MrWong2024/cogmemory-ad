...
| Batch D / B11–B15 | B11 已完成；B12-B2 完整重跑存在 Playwright 资产阻断；Batch D 尚未完成 | B12-B2 的 22 条 core route 均实际执行，但 logout 后账本断言使 22 条均失败，62 个 core Browser ID 未关闭；post-browser verify 与双 cleanup 通过；B12-B、B12 与 Batch D 未完成，不进入 B12-C，B12-86–B12-88 未关闭 |
...
Batch D 已完成 B11，并已进入 B12。B12-B2 已用全新 namespace 完整执行 `core-workflow` 的 5 个 scenarioKey / 22 条 route / 62 个 Browser audit ID；22 条 route 都实际启动并执行，但统一在 logout 后的 `/auth/me` 账本断言失败，最终为 0 route / 0 audit ID pass、22 route / 62 audit ID fail。完整 post-browser verify 与双次 cleanup 通过。B12-B3 已完成该 Playwright 认证生命周期账本缺陷的代码级修复和 21 / 21 synthetic infrastructure 回归，但没有重跑产品 Browser，因此上述历史通过数不变。B11 fixture 建立的两个互斥 profile 均已闭环：`core-workflow` 覆盖 58 个 Browser 项，`resilience-security` 覆盖 11 个 Browser 项；B11-70 是单独的 `static-gate`。共 70 个稳定 ID、69 个 Browser 项和 1 个静态项，每个 ID 只有一个 primary owner。B11 已完成事实保持不变。B12-B、B12 与 Batch D 仍未完成，不进入 B12-C；B12-86–B12-88 未关闭，也尚未执行 B13–B15。
...
当前处置：B12-B2 已完整执行 22 条 core route，但统一因 logout 后的 `/auth/me` 账本断言失败，62 个 core Browser audit ID 均未关闭；B12-B3 已完成该 Playwright 资产缺陷的代码级修复与 synthetic 回归，但未产生新的 Browser pass。产品缺陷 0、fixture 缺陷 0、Playwright 验收资产缺陷 1、稳定环境限制 0。B12 合同固定为 `core-workflow` 62 个 Browser audit ID（5 个 scenarioKey / 22 条 route）、`resilience-security` 23 个 Browser audit ID（6 个 scenarioKey / 11 条 route）和 B12-86–B12-88 三个 `static-gate`；88 个 ID 均有唯一 owner，85 个 Browser ID 均恰有一条 route，三个 static-gate 无 Browser route。两个 profile 均使用 doctor、admin、nurse、research_assistant、system 五角色和互不共享的 Patient / Visit / ScaleInstance / ClinicalReport / marker 根。B12-A1 调整后的 core 合法状态矩阵为 confirmed unlocked 11、confirmed locked 4、historical locked fallback 1，其余状态计数不变；状态矩阵总数和资源数量不变，resilience 仍使用 11 个独立 confirmed unlocked 根。
