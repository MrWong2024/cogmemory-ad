...
| Batch D / B15 | 当前 A25 合同只保留的 `B15-U01`、`B15-U02` 两个活动场景均已通过，状态 `passed=2`、`pending=0`、`failed=0`、`blocked=0`、`not_executed=0`。C01～C23 已按最低充分层全部处置，`B15-P0-contract-evidence`、P1、P2、P3 均已完成，当前 evidence `gap=0`，B15 Browser 验收闭环完成 |
...
## 10. B13、B14 已完成；B15 已重新生成

B13 已按当前 A23 合同完成三个活动场景。B14 已以当前 A24 合同为起点独立重新生成并完成两个活动场景；原 B14-01～B14-115 不再是活动关闭清单。B14.1 只维护 B11～B15 共享工作流的累计证据归属，不拥有独立活动 ID。B15 已从当前 A25 合同独立重新生成两个活动场景；旧 10 组只作最后的反向迁移输入，不再作为活动清单或执行顺序。
...
执行结果（2026-08-02）：`passed`。单文件 B14 fixture CLI 建立两个独立、schema-complete 脱敏场景：`archive-ready` 通过正式 A22 lock 与 A23 source-freeze Workflow Service 后保持 `confirmed` 且尚未归档；`archive-completed` 另通过正式 A24 archive Workflow Service 形成唯一 `archived` 事实，未直接拼装 A22/A23/A24 metadata 或顶层状态。prepared verifier 证明 confirmation、lock、completed source-freeze、archive anchor、正文/快照、来源 scope 与 ownership 完整，且独立 A24 `AuditLog` 为 0。Browser discovery 精确为 1 个文件、2 条测试；Chromium headless、workers=1、retries=0 的正式结果为 2/2 passed（6.1s）。doctor 在 ready 报告打开并取消空白二次确认表单，nurse 使用独立 Context/Session 读取同一报告、看到本地只读说明、无归档入口且未制造 403；两个 Context 的 HttpOnly Cookie 不同并最终全部关闭。archived 报告继续显示正文、报告锁定、来源冻结与持久归档安全摘要，三类服务端时间按各自标签核对，A24 anchor 与 A23 一致；旧 edit/submit/confirm/lock/freeze/archive 入口均关闭。当前实现显示 1 个符合条件的 correction 入口，U01 未点击且不将其判为失败。两条测试的 archive POST 与全部报告业务写请求均为 0；post-browser verifier 证明全部 prepared 业务 baseline 未变化；两次精确 cleanup 均为 `residualCount=0`，第二次所有删除计数为 0。
...
“当前状态”记录可定位的已有证据；B14-C11 记录定向 HTTP E2E 动态证据，C12/C13 与 C16 记录各自 Node-only 动态证据；其他行不因本轮只读复用而视为重新动态执行。
...
| B14-C16 | B14.1 shared façade/coordinator/reducer/beforeunload/report identity reset 累计证据 | `frontend_static_or_pure` + `ui_reachable` | `frontend_static_or_pure` + 代表性 `browser_micro_profile` | `frontend/test/browser-acceptance/contracts/clinical-report-workflow-shared-non-browser.spec.ts` 直接覆盖安全初态/RESET、activeMode 与 draft 互斥、BEGIN/FINISH_WRITE、CANCEL_ALL、COMPLETE_ARCHIVE、unexpected same-route identity 完整 RESET、expected correction replacement 精确保真与非法保真降级；有界核对 façade 传稳定 `reportId`、route RESET 优先、服务端 receipt 声明一次性 transition、identity generation 与旧 writingRef 的失效在 `useLayoutEffect` 中完成、新 identity 提交后在浏览器重绘及 passive effect 之前完成隔离、success/error/finally 的既有 generation guard 继续拒绝旧请求结果并保护新 writingRef，从而关闭原普通 passive `useEffect` 留下的窄时序窗口；同时核对统一 `onReportUpdated`、共享 latest、七 Action 共用 coordinator 与唯一 beforeunload；B11～B13 代表性 Browser 证据继续承担动作行为 | `covered` | 否 | route 改变始终完整 RESET；unexpected same-route report change 清空全部旧 draft/error/receipt/sourceReport/liveMessage/writeProhibited 与写状态。只有 receipt 的 source/replacement 与实际 A→B 精确一致时保留本次 correction receipt/sourceReport/liveMessage，同时清除其他 workflow 状态；generation 变化后旧请求不能写回或清除新 writingRef。 |
...
#### 10.2.10 P0 gap 关闭结论与下一步

| 已关闭 Gap | 真实触发路径 | 既有证据 | 本轮闭合证据 | 最低充分层 | 处置 |
|---|---|---|---|---|---|
...
| Correction | B15 | `b15-correction-non-browser.spec.ts` 已直接覆盖 correction pure/static、Action recovery 与 persisted resume privacy；shared Node-only spec 继续覆盖 expected correction identity transition、same-route 身份隔离、identity generation、旧请求失效、单一 writingRef 与唯一 beforeunload。`B15-U01` 已由 `u01-first-correction.spec.ts` 与 `u01-post-correction` verifier 完成首次真实页面、当前会话切换和刷新持久 lineage；`B15-U02` 已由 `u02-recovery-uncertain-result.spec.ts` 与 `u02-post-recovery` verifier 完成显式恢复、网络不确定、刷新及隐私边界。不恢复 B14.1 独立 Browser suite。 |
...
| route/report identity reset | shared spec 的 reducer/pure 直接测试与 coordinator/façade/correction Action 有界结构核对 | patientId/visitId 变化始终完整 RESET；unexpected same-route `report.id` 变化完整清理旧 workflow，expected correction A→B 只在服务端 receipt 与实际 identity 精确匹配时保留当前 correction receipt/sourceReport/liveMessage；其他状态清理。identity generation 与旧 writingRef 的失效在 `useLayoutEffect` 中完成，新 identity 提交后在浏览器重绘及 passive effect 之前完成隔离；success/error/finally 的既有 generation guard 继续拒绝旧请求结果并保护新 writingRef，关闭原普通 passive `useEffect` 留下的窄时序窗口。C16 / P0-G3 已关闭。 |
...
| latest 最多一次 | `b12-lock-non-browser.spec.ts`、`b13-source-freeze-non-browser.spec.ts`、`b14-archive-non-browser.spec.ts`、`b15-correction-non-browser.spec.ts` + 对应 Browser | shared helper 与各动作特定错误分类已有直接证据；correction 受控业务错误 latest≤1，network/unknown 自动 latest=0。 |
...
| 不自动 retry/poll；草稿不持久化 | B12/B13 Node-only + Browser；B14 Archive 与 B15 Correction Node-only + Browser | Archive/Correction 均无自动 POST replay/retry/poll，手工 latest 与错误恢复独立；`B15-U02` 已直接证明 correction 网络中止后零自动 POST/latest、当前内存文本保留及刷新后 DOM/Storage/URL 清空，不建 B14.1 独立 Profile。 |

若后续又发现其他 shared 行为没有最低充分证据，必须进入相应合同表并写明最低充分层；不得恢复 B14.1 大型 Browser suite，也不得创建跨七个 Action 的全量组合测试。当前累计记录 shared workflow through archive、B15-U01 首次 correction 和 B15-U02 恢复/不确定结果 Browser 证据；B15 已无剩余 pending，且不改变 B14 P0 shared contract 的既有闭环。

### 10.4 B15 版本化更正：按当前 A25 合同重新生成

#### 10.4.1 设计结论与活动汇总

B15 从当前 controller、DTO、workflow、原子更新、mapper、frontend correction façade/Action/coordinator 和现有测试证据重新生成，而不是直接执行旧 10 组。当前不可互换的用户可见发布风险只有两个：首次不可逆版本转换及当前会话身份切换；服务端持久恢复及网络结果不确定时的显式用户恢复。认证、权限、DTO、ownership、readiness、幂等、并发、pure/static、V2+ 后续生命周期、防御状态和通用门禁分别交给最低充分层。旧 10 组只在 10.4.12 反向对账。

| 活动场景 | 分类 | Profile | 状态 |
|---|---|---|---|
| `B15-U01` 首次真实版本化更正与当前会话切换 | `ui_reachable` | `B15-P1-first-correction` | `passed` |
| `B15-U02` 显式恢复与网络不确定结果 | `ui_reachable` | `B15-P2-recovery-uncertain-result` | `passed` |

活动 ID 恰好为 `B15-U01`、`B15-U02`，不创建 `B15-U03`。最终汇总恰好为：`passed=2 / pending=0 / failed=0 / blocked=0 / not_executed=0`。C10 的后端动态证据、C08/C13/C14 的 correction Node-only 证据、C16 的 U01 Browser/DB 证据、C19 的 U02 Browser/DB 证据与 C23 的最终门禁均已关闭；P0～P3 全部完成，B15 Browser 验收闭环完成。

#### 10.4.2 `B15-U01` 首次真实版本化更正与当前会话切换

- 起始状态与角色：latest archived V1，A21 confirmation、A22 lock、A23 completed、A24 archive 与 anchors 完整，无 correction；doctor 可操作，nurse 代表性只读。完整角色矩阵由 HTTP E2E 承担，不为 Browser 403 暴露隐藏按钮。
- 真实触发路径：doctor 从正式报告页打开版本化更正；reason/summary 初始为空，不预填 lock、freeze、archive、confirmation 或 clinician 文本；核对 label、字符计数、checkbox、最小无效值、修改文本清除 checkbox，以及写入期间共享写锁和正文可读。
- 一次真实写入：只发送一次 correction POST；Body keys 精确为 `confirm`、trim 后 `correctionReason`、trim 后 `changeSummary`、服务端 latest `expectedUpdatedAt`。HTTP 200 且 `alreadyCreated=false`、`resumedExisting=false`；V1 变 corrected，服务端创建唯一 V2 replacement，双向 correction/`replacementOf`、actor、time、reason、summary 一致。
- 当前会话：不调用额外 latest、不刷新、不跳转；页面原地切换 replacement，并保留本次 sourceReport、correction receipt 与 liveMessage。合法服务端 receipt 声明的 identity transition 才可保留这些事实；旧 V1 的 edit/submit/confirm/lock/freeze/archive receipt 全部隔离。
- 刷新边界：刷新后会话 receipt/sourceReport 消失，latest 仍是 replacement，只依据持久 `replacementOf` 与 source correction；不伪造历史链接，完整历史页归 B17。
- 动作隔离：不自动 edit、submit、confirm、lock、freeze、archive、void、PDF/download 或 AI；页面不显示 metadata、原始 correctionRecords、内部 lineage/correction/source ID。
- 后置证据：source 唯一 corrected、replacement 唯一创建、无 V3/branch；双向 anchors 一致；受保护报告事实、Patient、Visit、五类来源与 Storage 不变；独立 A25 AuditLog=0，未自动写入 A21～A24。
- 发布阻断风险：重复/分支版本、错误 actor/anchor、旧报告回执串入新 identity、隐私泄露或自动串联任何后续生命周期都会破坏不可逆临床事实和当前会话可追溯性。

执行结果（2026-08-03）：`passed`。`backend/scripts/b15-browser-fixtures.ts` 以 Profile `B15-P1-first-correction` 和唯一场景 `first-correction-ready` 建立 schemaVersion 1 的安全 runtime descriptor；prepare 与 prepared verifier 证明 latest 为唯一 archived V1，A21/A22/A23/A24 完整，V2/V3 与 completed A25 均为 0。`frontend/test/browser-acceptance/b15/u01-first-correction.spec.ts` discovery 精确为 1 个文件/1 条测试；Chromium headless、workers=1、retries=0 的正式结果为 1/1 passed（5.7s）。nurse 先以独立 Session 只读访问且 correction 入口、报告业务写入和人为 403 均为 0；doctor 随后在 390×844 视口完成真实键盘、label、计数、最小无效输入与 checkbox reset/focus 路径。ControlledRequestGate 对唯一 correction POST 的 matched/continued/aborted 为 `1/1/0`，Body 仅含 `changeSummary`、`confirm`、`correctionReason`、`expectedUpdatedAt`，HTTP 200 且为首次完成；在途期间共享写入口锁定、正文可读，无进度伪装、latest、polling 或重放。页面不刷新、不跳转即切换唯一 V2，并保留当前会话 source、receipt 与 live message；旧 V1 会话回执全部隔离。主动刷新后会话辅助事实消失而持久 `replacementOf` lineage 保留，correctionId 未进入 DOM 或 URL，全部相邻报告写入、PDF/download 与 AI 调用为 0。`verify u01-post-correction` 证明唯一 completed A25、唯一 correction record、V1/V2 各一、V3/branch 为 0，来源保护事实、正式 replacement copy/reset、Patient/Visit/sources/Storage 与独立 AuditLog 边界匹配；两次精确 cleanup 均为 `residualCount=0`，第二次删除计数全为 0。

#### 10.4.3 `B15-U02` 显式恢复与网络不确定结果

本活动含同一恢复风险域的两个紧密路径，不拆第三个活动 ID。

- 路径 A 起始状态：latest archived source 持久化正式 workflow 支持的合法 A25 `in_progress`，最低状态优先使用已 start、尚无 replacement 的恢复点；不制造产品不支持的中间态。
- 路径 A 页面与请求：页面显示原 correctionNo、started actor/time、source/replacement 版本关系；服务端首次 reason/summary 只读，checkbox 初始未选，用户必须显式确认。correctionId 只用于客户端内部一致性与后端终态核对，不渲染到页面或 DOM；只发送一次 POST，Body 沿用服务端首次 reason/summary 且不提交 correctionId，不生成新技术关联标识，也不覆盖首次 actor/time。
- 路径 A 结果：`alreadyCreated=false`、`resumedExisting=true`，形成唯一 replacement 与 completed source。Browser Network evidence 只可在内存中核对安全 response 的 correctionId 且不得输出，post-browser verifier 必须证明 persisted correctionId 未变化；B16 的历史 Chrome 路径在证据后发生 Correction Action/coordinator/identity barrier 影响性变化，因此只作 supporting evidence，路径 A 仍是 U02 mandatory evidence。
- 路径 B 起始状态与操作：全新 archived latest source；当前 React 页面输入脱敏 reason/summary，一次真实 correction POST 被网络层一次性中止。
- 路径 B 不确定边界：不自动重试 POST、不自动 latest、不自动切换 replacement，也不宣称成功、失败、回滚或 completed；当前内存保留文本，checkbox 清除，显示正式不确定结果和手工 latest 入口，但不自动点击。
- 路径 B 刷新与隐私：刷新后本地文本、checkbox 和 receipt 消失；页面只根据服务端 latest 显示首次入口、`in_progress` 恢复或 completed replacement，不猜测结果。correctionId 不渲染到 DOM；reason、summary、receipt、correctionId 不写 localStorage、sessionStorage、IndexedDB、URL query/hash 或 Cookie。
- 发布阻断风险：自动重放不可逆 POST、覆盖首次审计、误报服务端结果、丢失显式恢复或持久化临床说明都足以阻断发布。

执行结果（2026-08-03）：`passed`。`backend/scripts/b15-browser-fixtures.ts` 以 Profile `B15-P2-recovery-uncertain-result` 建立两个独立 Patient/Visit/source 场景；prepared verifier 证明 `correction-in-progress` 是唯一 archived V1 上由正式 readiness/plan/start 工作流形成的合法 A25 `in_progress`，`correction-network-uncertain` 是无 correction 的唯一 archived V1，二者均无 V2/V3/branch。`frontend/test/browser-acceptance/b15/u02-recovery-uncertain-result.spec.ts` discovery 精确为 1 个文件/2 条测试，Chromium 正式结果为 2/2 passed（6.2s，workers=1）。路径 A 只发送一次四字段 correction POST 并获 HTTP 200、`alreadyCreated=false`、`resumedExisting=true`；持久化 reason/summary 只读，首次 correctionId/actor/time/text/plan 只在内存与安全哈希中核对且未改变，当前会话 recovery receipt 与 live message 出现，真实刷新后会话辅助事实消失而持久 lineage 保留，唯一 V2、无 V3/branch。路径 B 的 OneShotRequestAbort 精确中止一次首次 correction POST；页面保留本地文本、清除 checkbox、显示正式不确定提示与手工 latest 入口，自动 POST/latest/replacement 均为 0；请求前、中止后、刷新后三个时点的 localStorage、sessionStorage、IndexedDB、query、hash 与 Cookie 审计均为空，真实刷新后本地文本、错误与 receipt 消失，后端仍为无 correction 的 archived V1。两条路径的相邻报告写入、PDF/download/AI 均为 0。`verify u02-post-recovery` 证明恢复场景唯一 completed A25/correction record、V1/V2 各一、持久化首事实哈希匹配，网络场景 V2/V3/branch 为 0 且业务 baseline 不变，跨场景隔离成立；两次精确 cleanup 均为 `residualCount=0`，第二次删除计数全为 0，runtime descriptor 已移除。

#### 10.4.4 B15 Profile 职责

| Profile | 职责 | Browser / 活动 ID |
|---|---|---|
| `B15-P0-contract-evidence` | 已完成：后端角色/DTO/ownership/readiness/版本/恢复/幂等/合法并发，frontend correction pure/static、Action recovery、persisted resume privacy，B16 复用和 defensive 审计均已收口；P0 `gap=0` | 不执行 Browser；不拥有活动 ID |
| `B15-P1-first-correction` | 已完成：U01 首次更正、当前会话切换、刷新与后置事实均通过 | Browser；只拥有 `B15-U01` |
| `B15-P2-recovery-uncertain-result` | 已完成：U02 的 persisted resume 与 network uncertain 两条紧密路径及后置数据库验证均通过 | Browser；只拥有 `B15-U02` |
| `B15-P3-final-gates` | 已完成：最终代码态一次 discovery、lint、typecheck、production build、依赖/路由/脱敏和 U02 轻量跨层 Browser 冒烟均通过 | 不拥有业务活动 ID |
...
状态只描述当前可定位证据。C10 记录后端动态证据；C08/C13/C14 记录同一份 frontend correction Node-only spec 的直接动态证据；C16 记录 U01 Browser 与后置数据库动态证据；C19 记录 U02 Browser 与 `u02-post-recovery` 动态证据；C23 记录最终代码态 P3。P0～P3 均已完成，当前 `gap=0`。

| # | 合同或风险 | 可达性分类 | 最低充分证据层 | 当前精确证据文件及测试名称 | 状态 | 后续任务与缺口 | B16 复用结论 |
|---|---|---|---|---|---|---|---|
| C01 | 401；doctor/admin；nurse/research/system | `public_api_reachable` | `backend_http_e2e` | `backend/test/clinical-report-correction.e2e-spec.ts`：`enforces authentication, roles, strict confirmation and whitelist`；`creates one replacement, returns latest and remains idempotent` | `covered` | 否；完整角色由 HTTP，不建 Browser 403 矩阵 | B16 角色 Browser 仅 supporting |
| C02 | DTO、confirm、reason、summary、expectedUpdatedAt、whitelist | `public_api_reachable` | `backend_unit` + `backend_http_e2e` | `backend/src/modules/reports/dto/clinical-report-correction-dto.spec.ts`：`accepts the exact body and trims controlled text`、`rejects invalid controlled input %#`、`leaves confirm=%p to the workflow strict business check`、`rejects client-controlled fields`；上述 E2E access test | `covered` | 否 | 无需 B16 |
| C03 | ownership、latest、V1/合法 V2+、Patient/Visit 状态 | `public_api_reachable` | `backend_unit` + `backend_http_e2e` | `reports.service.spec.ts`：`uses complete ownership for direct report lookup`、`bypasses V1 and scopes a V2 predecessor lookup to current ownership`；`clinical-report-correction.spec.ts`：`accepts only a fully audited latest archived source`；A25 E2E `creates one replacement…`、`runs V2 and V3 lifecycles without rewriting shared frozen sources` | `covered_by_layered_evidence` | 否；fixture 的 inactive Patient/locked Visit 与 A26 voided Visit 证明不误加生命周期门禁 | 历史 V2→V3 仅 supporting |
| C04 | A21 confirmation、A22 lock、A23 completed、A24 archive、anchors、首次 readiness | `public_api_reachable` | `backend_unit` + `backend_http_e2e` | `clinical-report-correction.spec.ts`：`accepts only a fully audited latest archived source`、`rejects historical archive fallback and stale optimistic concurrency`；A25 E2E `rejects a report that has not been archived` | `covered_by_layered_evidence` | 否 | 无需 B16 |
| C05 | 版本计划、correctionNo、确定性 code、唯一 replacement、禁止 branch/merge/跳版 | `public_api_reachable` | `backend_unit` + `backend_http_e2e` | `clinical-report-correction.spec.ts`：`builds a deterministic linear plan and an immutable start audit`、`rejects a mismatched replacement instead of overwriting or branching`；A25 E2E `creates one replacement…`；`reports.service.spec.ts`：`defines collection and indexes` | `covered_by_layered_evidence` | 并发唯一性单列 C10 | 历史 V2→V3 仅 supporting |
| C06 | start/create-or-resolve/record/complete；非 transaction；无 rollback/delete | `public_api_reachable` | `backend_unit` | `clinical-report-correction.spec.ts`：`records the replacement before completing exactly one correction event`；`reports.service.spec.ts`：`starts A25 with a single source-only metadata update`、`records and completes only the matching deterministic replacement` | `covered_by_layered_evidence` | 否；实现审查确认无 transaction/rollback/delete | fixture builder 只支持，不冒充产品测试 |
...
| C08 | persisted `in_progress` 恢复并保留首次 ID/文本/actor/time | `public_api_reachable` + `ui_reachable` | `frontend_static_or_pure` | `frontend/test/browser-acceptance/contracts/b15-correction-non-browser.spec.ts`：`C13 creates faithful start and persisted resume drafts`、`C13 validates dirty and report matching semantics`、`C13 keeps exact correction request and API client whitelist`、`C08 keeps persisted Panel fields read-only and correctionId internal`、`C08 exposes business traceability without rendering correctionId` | `covered` | 否；resume eligibility 与 draft 内部绑定 persisted correctionId，服务端首次 reason/summary 保真，persisted fields 只读，请求不提交 correctionId；Panel/Summary 只展示 correctionNo、版本、started/completed actor/time、reason/summary，限定 JSX render 区块不绑定 correctionId。correctionId 的 DOM/Storage/URL 真实 Browser 核对仍归 U02 | B16 历史 Browser 继续只作 supporting；当前直接 Node-only 证据已关闭 P0-G4 |
...
| C13 | frontend 首次/resume 资格、草稿、校验、dirty/stale、Body、continuation、consistency/lineage pure | `frontend_static_or_pure` | correction 专属 Node-only contract spec | `frontend/test/browser-acceptance/contracts/b15-correction-non-browser.spec.ts`：`C13 preserves representative roles and V1 or V2+ source eligibility`、`C13 enforces correction consistency and safe replacement lineage`、`C13 creates faithful start and persisted resume drafts`、`C13 validates dirty and report matching semantics`、`C13 keeps exact correction request and API client whitelist`、`C13 continues a stale start only with a safe latest source` | `covered` | 否；production pure functions 直接证明 V1/安全 V2+ source、start/resume eligibility、consistency/replacement、draft/validation/dirty/stale/matching、exact request Body 与 latest continuation | B16 Browser 不能替代 pure/static；当前直接证据已关闭 P0-G2 |
| C14 | Action 单 POST、controlled latest≤1、network latest=0、no replay、统一 update、server receipt identity | `frontend_static_or_pure` | correction 专属 Node-only Action contract | `frontend/test/browser-acceptance/contracts/b15-correction-non-browser.spec.ts`：`C14 limits controlled latest refresh and correction write prohibition`、`C14 keeps one correction POST and applies server replacement identity`、`C14 separates error recovery manual latest and prevents replay or polling`、`C14 keeps input updates and baseline staleness bounded` | `covered` | 否；真实 shared recovery 证明 correction 业务错误 latest≤1、network/unknown=0，audit/replacement corruption 禁止写入；Action 有界核对证明唯一生产 POST、成功应用完整 replacementReport、identity transition 只用 receipt IDs、onError 不 replay，手工 latest 独立 | B16 success 证据仍只作 supporting；当前直接证据已关闭 P0-G3 |
| C15 | shared coordinator 单写锁、generation、layout barrier、expected transition、旧请求失效、唯一 beforeunload | `frontend_static_or_pure` | current Node-only shared spec | `clinical-report-workflow-shared-non-browser.spec.ts`：`C16 coordinator has one write lock…`、`C16 façade shares one coordinator and one beforeunload…`、`C16 wires stable route/report identity…`、`C16 preserves only a complete expected correction replacement result`、`C16 degrades every invalid…` | `covered` | 否；这是当前基线直接证据，不是 B16 历史推定 | B16 旧 shared flow 已被当前证据取代 |
| C16 | completed source 无入口；replacement 当前显示；会话/刷新边界 | `ui_reachable` | `browser_micro_profile` + current shared spec + DB verifier | `frontend/test/browser-acceptance/b15/u01-first-correction.spec.ts`：`doctor creates one replacement while nurse remains readonly and reload keeps persistent lineage`；`backend/scripts/b15-browser-fixtures.ts`：`verify u01-post-correction`；current shared spec 的合法 transition/非法 reset 证据 | `covered` | 否；`B15-U01` 与 `B15-P1-first-correction` 已完成 | 历史 B16 仅 supporting；当前 Browser/DB 为主证据 |
| C17 | replacement A21～A24 角色/页面/API 与合法后续生命周期 | `duplicate_or_covered` | A21/A26 HTTP + current B12～B14 + frontend pure | A25 E2E `lets doctor/admin edit, submit and confirm replacement only`；A26 `runs V2 and V3 lifecycles…`；replacement lineage spec；B12/B13/B14 当前累计证据 | `covered_by_layered_evidence` | 否；不为 B15 重复 V2+ 生命周期矩阵 | B16 完整矩阵作 supporting；当前分层证据为主 |
| C18 | correction 不自动串联 edit/submit/confirm/lock/freeze/archive/void/PDF/AI | `duplicate_or_covered` | current static/action ownership + representative Browser | current Correction Action success 仅 apply replacement + complete correction；shared façade 动作分离；U01 仍核对 Network 零相邻写入 | `covered_by_layered_evidence` | 不创建独立活动 ID | B16 no-auto-chain 在代码变化审查后仍可作 supporting，不单独重跑 |
| C19 | network uncertain、手工 latest、内存文本、Storage/URL | `ui_reachable` + `frontend_static_or_pure` | Action contract + `browser_micro_profile` | `b15-correction-non-browser.spec.ts` 的 network/unknown、manual latest、no replay 子合同；`u02-recovery-uncertain-result.spec.ts` 两条正式 Browser 测试；`b15-browser-fixtures.ts` 的 `verify u02-post-recovery` | `covered` | 否；U02 已证明 persisted resume 首事实保真，以及一次网络中止后的本地文本、checkbox、正式提示、零自动 POST/latest、三时点 Storage/URL/Cookie 和真实刷新边界 | 历史 Storage 仅 supporting；当前 Browser/DB 为主证据 |
...
| C23 | discovery、lint、typecheck、build、依赖、路由、脱敏 | `general_gate` | `static_gate` | 2026-08-03 最终代码态：目标 spec discovery=1 file/2 tests；backend fixture lint:file/typecheck、frontend full lint/typecheck/production build 均退出码 0；U02 Browser 2/2 与后置 verifier 通过；测试数据和输出完成脱敏审计 | `covered` | 否；`B15-P3-final-gates` 已完成 | B16 历史 gates 未冒充当前 P3 |

分类汇总：`covered=12`、`covered_by_layered_evidence=8`、`duplicate_or_covered=1`、`gap=0`、`supplemental_defensive=1`、`supplemental_compatibility=0`、`retired_currently_unreachable=1`、`retired_obsolete=0`，共 23 行。`B15-P0-contract-evidence`、P1、P2、P3 均已完成，B15 当前无 confirmed gap。

Node-only 执行结果（2026-08-02）：定向 discovery 只发现 `b15-correction-non-browser.spec.ts` 的 12 条测试；正式运行 `passed=12`、`failed=0`、`skipped=0`，Playwright 仅作为 Node TypeScript runner，未申请 Browser/request fixture、未启动 webServer、未访问 HTTP 或数据库。该 spec 的定向 lint 与 frontend 正式 typecheck 均通过；边界扫描未发现 Browser runtime、fetch、DOM/Storage runtime、数据库 client、环境 Secret、Hook testing framework 或整文件 snapshot。

#### 10.4.7 B16 证据复用审计

Git 历史没有 B16 correction Browser spec 文件，也没有可引用的 Playwright test 名称；不得虚构。真实 Chrome/内置 Browser 结果只以 evidence commit 中的精确记录和 scenario key 定位。fixture E2E 是动态夹具证据，不是 Browser。

| B16 证据 | evidence commit / 精确定位 | 证据类型 | 证据后代码变化 | 最终处置 |
|---|---|---|---|---|
| fixture contract、prepare/verify/cleanup | `712ff2ea2953ac61bc22416ad28d7147c5cd0f5c` 初建；`9099f66660b85f194443b307422cf742404ed652` 加 `v2_correction_in_progress`/unsafe；`backend/test/b16-browser-fixtures.e2e-spec.ts`：`validates namespace, pre-import environment, database gate, password, and safe manifest fields`、`prepares, verifies, authenticates, rejects duplicate prepare, isolates namespaces, replaces, and cleans precisely`、`fails read-only verify for a missing scenario without repairing it` | fixture HTTP/DB E2E；非 Browser | 当前资产仍存在；只证明 4 角色、22 keys、合法前置和 cleanup | supporting；不得关闭 UI/产品合同 |
| persisted V2 correction resume→V3 | `9099f66660b85f194443b307422cf742404ed652`；记录场景 key `v2_correction_in_progress`，无 spec/test name | 真实 Chrome 手工/工具证据 | `f1b26a07cbcc64f3d2a0d9db282b1dfb075e0a1a`、`2e1b028c55082d9ba0373645be546cd60ff608cb` 后改 Correction Action、expected transition、coordinator generation/layout barrier | 不能直接复用整条当前流程；U02 路径 A mandatory |
| archived V2→V3 smoke 与 V2+ correction | `9099f66660b85f194443b307422cf742404ed652`；记录“安全 archived V2→V3，1 次 A25 POST/200”，无 spec | 真实 Chrome smoke | 同上，成功后的 identity switch 路径发生影响性变化；backend correction workflow 未变 | backend/eligibility supporting；不能关闭当前 UI switch |
| V2+ A21～A24 与 no-auto-chain | `066ee8715499a7da12bb8b2781743e155c484919` 产品修复基线，`9099f66660b85f194443b307422cf742404ed652` 记录前轮完整矩阵 | 历史真实 Chrome 矩阵，无持久 spec | shared coordinator/部分 Action 后续变化；当前 A26 E2E、B12～B14 与 shared spec 已取代关键主证据 | `duplicate_or_covered` supporting；不建 B15 活动 ID |
| source/replacement lineage privacy | `066ee8715499a7da12bb8b2781743e155c484919` 修复隐藏内部 ID；`9099f66660b85f194443b307422cf742404ed652` Chrome 记录 | 历史 Browser + current mapper unit | CorrectionPanel/CorrectionSummary/draft 自 9099 后未变；mapper/types 后续进一步收紧 actor/history | 对未变化展示子合同可直接 supporting；`b15-correction-non-browser.spec.ts` 已补齐当前 resume draft、request whitelist 与限定 JSX privacy 直接证据并关闭 C08。历史 Browser 仍不能关闭当前完整恢复流程，真实 DOM/Storage/URL 继续归 U02 |
| unsafe replacement summary | `9099f66660b85f194443b307422cf742404ed652` scenario `v2_replacement_summary_unsafe`，无 spec | 真实 Chrome defensive | 当前 internal/public lineage gates 仍有 unit/E2E；非正式 UI 可制造状态 | `supplemental_defensive` supporting，不阻断 U01/U02 |
| Web Storage/URL/Cookie 审计 | `95b778448603e5eb4f96eafb82136edc36d3ab0e`；五个 checkpoint 与 unsafe 复核，无 spec | Codex 内置 Browser 隔离审计 | Correction Action/coordinator/identity 之后已改；当前 shared beforeunload 有直接 Node-only 证据 | 旧 storage 事实 supporting；U02 路径 B 仍须当前 correction 专属验证 |

结论：B16 persisted resume 和 archived V2→V3 整条流程均不可直接关闭当前 B15；V2+ 后续 lifecycle、lineage privacy 和 no-auto-chain 可按未变化子合同与当前 A26/B12～B14/shared 证据分层复用，不再生成独立 B15 活动。

历史定位完整性：`v2_correction_in_progress` 使用 doctor、以 archived V2 + 合法 persisted A25 in_progress + 无 V3 起步，经 Tab/Enter/Space 显式确认，一次 corrections POST 返回 200/`resumedExisting=true`，原地到 V3，刷新零重发；记录仅称后端 E2E 确认唯一 V3/无 V4，没有可恢复的专属 post-browser verifier 名称。`v2_replacement_summary_unsafe` 使用建议 admin 起点，页面阻断 A22～A25、写请求 0，并扫描 DOM/URL/Console。记录还说明 fixture replace/verify 通过、最终 cleanup 两次 residual=0；但这些结果没有持久 Browser spec/test name，故只按表中范围复用。

#### 10.4.8 gaps 关闭结果

C08、C13、C14 已由同一份 `frontend/test/browser-acceptance/contracts/b15-correction-non-browser.spec.ts` 关闭，C16 已由 `B15-U01` 的当前 Browser/DB 证据关闭；最后两个 gap 也已由 U02 与 P3 关闭：

| 原 Gap | 真实路径与风险 | 关闭证据 | 最低充分层 | 最终状态 |
|---|---|---|---|---|
| `C19 / U02` | persisted resume 与首次 POST 网络结果不确定 | 当前 U02 两条 Browser 测试 + `u02-post-recovery` + Node-only Action/recovery/privacy | `browser_micro_profile` | `covered`；`B15-P2-recovery-uncertain-result` 完成 |
| `C23 / P3` | 最终代码态 discovery/build/route/dependency/data gate | 最终代码态 discovery、frontend/backend lint/typecheck、production build、脱敏审计与 U02 smoke | `static_gate` | `covered`；`B15-P3-final-gates` 完成 |

#### 10.4.9 supplemental compatibility / defensive

- `supplemental_compatibility`：A25 当前没有 historical correction fallback。不得模仿 A24 创建 fallback 场景，也不得猜测缺失 actor、anchor 或 correction 事实；本表计数为 0。
- `supplemental_defensive`：in_progress 缺字段、completed audit/correctionRecords 不一致、corrected 无 replacement、双向 lineage/correctionId/correctionNo/version/actor/time/anchor 冲突、branch/merge/orphan、metadata 根不安全和 ownership 内部错配，只保留廉价 correction/replacement-lineage/mapper unit。除非出现正式导入/迁移、已知事故或明确合规要求，不建 Browser 或大型损坏 fixture，不阻断 U01/U02。

#### 10.4.10 generic optimistic conflict 可达性结论

当前公开路径中，A21 edit/submit/confirm 不写 archived source；A22/A23/A24 对已完成 source 为幂等且不形成“仍可首次更正”的新 updatedAt；A20 latest/generate 不改该 source；void 不保留首次更正资格。除另一个合法 correction 请求外，没有正式页面或公开 API 能只改变同一 archived latest source 的 updatedAt、同时保持它满足首次更正资格。因此 generic conflict 为 `retired_currently_unreachable`；直接改库制造 stale 仅作 atomic defensive。两个合法用户同时 correction 是 C10，不得伪装成 generic conflict。

#### 10.4.11 B15 最终门禁

`B15-P3-final-gates` 不拥有业务活动 ID，已于 2026-08-03 在最终代码态执行一次。目标 spec discovery 精确发现 1 个文件/2 条测试；backend fixture `lint:file` 与正式 `npm run typecheck`、frontend `npm run lint`、正式 `npm run typecheck` 与带 browser-acceptance API origin 的 production `npm run build` 均退出码 0，输出无 EPERM、Unhandled Rejection 或 uncaughtException。production build 路由包含正式报告页；fixture/runtime/测试输出采用脱敏标记与安全哈希，Browser runner 未继承数据库 URI、database purpose 或 fixture password。U02 2/2 正式 Browser 结果兼任唯一轻量跨层 smoke，不额外重跑整个批次。P3 已完成。

#### 10.4.12 原 10 组反向迁移

| 原组号 | 原核心意图 | 新归属 | 分类 | 处理理由 |
|---|---|---|---|---|
| 1 | archived V1 首次更正、表单与切 V2 | U01 + C02/C07/C13 | `ui_reachable` + layered | 一次真实首次流程；DTO/backend/pure 不在 Browser 穷举 |
| 2 | persisted in_progress 显式恢复 | U02 路径 A + C08 | `ui_reachable` + supporting | B16 受后续 Action/coordinator 变化影响，当前路径仍 mandatory |
| 3 | completed 幂等与页面只读 | C09 + U01 刷新边界 | `public_api_reachable` + layered | 真实 HTTP 顺序幂等已覆盖，不单建活动 |
| 4 | controlled errors、latest、stale/no replay | C13/C14 Node-only + U02 | `frontend_static_or_pure` + `ui_reachable` | correction pure/Action 合同已 covered；不按错误码拆 Browser，真实不确定恢复仍归 U02 |
| 5 | 401/403、audit/replacement conflict、network | C01/C11/C21 + U02 | layered | 权限/冲突给 HTTP/unit；网络用户恢复给 U02 |
| 6 | replacement A21 角色与资格 | C17 | `duplicate_or_covered` | A21/A26、B16 与当前 B12～B14 已分层覆盖 |
| 7 | no automatic A22～A24 | C18 + U01 Network | `covered_by_layered_evidence` | 当前静态 ownership + B16 supporting；U01 代表性核对零相邻写入 |
| 8 | source/replacement summary、lineage privacy | C07/C16/C17 + U01 | layered | mapper/current shared/B16 supporting；不建虚假历史链接矩阵 |
| 9 | viewport、label、错误、键盘、POST disabled | C22，附着 U01/U02 | `duplicate_or_covered` | 不单建活动或 Profile |
| 10 | beforeunload、Storage、URL | C15/C19，附着 U02 | layered | shared contract + U02 代表性真实路径，不独立 Profile |

迁移核对：覆盖组数=10，唯一组数=10，缺组=0，重复=0。旧 10 bullet 已退出活动清单正文，逐字历史由 Git 负责追溯。

#### 10.4.13 后续边界

B15 当前没有剩余活动场景或最终门禁；后续只有在 A25 产品合同、shared correction coordinator、公开 API 或正式迁移/合规要求发生影响性变化时，才按变化范围重新评估。任何历史 B16 证据仍不得写成本次动态通过。
