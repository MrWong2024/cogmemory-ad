...
| Batch C / B7–B10 | B7 已完成；B8 `core-workflow` 39 项已完成；B8 `resilience-security` 19 pass / 2 fail；B8 整体仍未完成；B9–B10 尚未启动 | resilience post-browser verify 通过，双次 cleanup 均为 `residualCount=0`；B8-56、B8-59 受 fixture 前置状态阻断 |
...
## 5. Batch C 当前状态与待验合同：B7 与 B8 core-workflow 已完成

B7 的 40 项已通过组合证据全部闭环；B8 `core-workflow` 的 39 项已完成，`resilience-security` 本轮 21 项为 19 pass / 2 fail，B8 整体仍未完成；B9–B10 尚未启动。以下序号与减肥前基线完全一致；fixture 设计必须覆盖列出的前置状态、角色、错误、无副作用、Network 和隐私边界。
...
B8 fixture 已按互不依赖的两个 profile 准备完成：`core-workflow` 覆盖人工评分、输入校验、服务端汇总、复核队列、显式确认与只读状态；`resilience-security` 覆盖并发/stale、401/403、网络失败、草稿保护、安全审计、隐私与响应式。每个 B8 编号只属于一个 profile；两者使用独立 namespace、manifest、prepared / post-browser verifier 与 cleanup 范围。`core-workflow` 的 39 项已完成。`resilience-security` 本轮 prepared verify、19 项 Browser 行为、post-browser verify 和双次 `residualCount=0` cleanup 均已闭环，但 B8-56、B8-59 因 fixture 的 `needs_review` 前置状态无法建立合同要求的最终确认草稿而 fail；B8 整体仍未完成。
...
`core-workflow` 的 B8-01–B8-19、B8-29–B8-44、B8-47–B8-49、B8-60 共 39 项已全部闭环；core post-browser verify 通过，双次 cleanup 均为 `residualCount=0`。`resilience-security` 的 21 项已实际执行：除 B8-56、B8-59 外其余 19 项 pass；resilience post-browser verify 通过，双次 cleanup 均为 `residualCount=0`。稳定阻断是：`network_failure` 和 `responsive_route_draft` 均准备为 `needs_review`，最终确认入口按产品合同不可用，因而不能建立并验证两项要求的最终确认本地草稿；本轮不修改产品或 fixture，也不填写 evidence commit。B8 整体仍未完成；B9–B10 尚未启动，下一步是修正 B8 fixture 前置合同后以全新 namespace 重跑完整 `resilience-security` 生命周期。
