...
11. 只有同时满足以下全部条件，凭据才可作为“本地隔离测试专用固定凭据”适用有限例外：用户明确授权直接使用；仅用于本地或等价隔离测试环境；无权访问生产、真实业务数据或其他非目标数据库；不作为其他环境的复用凭据；只在临时 Codex 指令、一次性命令或目标进程的进程级环境变量中使用。此类凭据允许按授权临时直接使用，但不得持久化，不得写入仓库、持久配置、日志、生成物、最终报告或提交记录。除非用户另有要求，不得强制此类低风险凭据额外采用剪贴板授权、一次性密码、Secret Manager 或人工交互输入。具体 Codex 指令仍可填写允许和禁止的具体数据库名；连接凭据的处理必须符合本条有限例外或上一条默认严格保护规则。
