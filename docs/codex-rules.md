...
12. 符合上一条条件的固定测试凭据默认全自动使用：Codex 可以从项目约定且被 Git 忽略的本地凭据文件读取，或使用任务明确给定的固定测试密码，并自动注入对应的独立目标进程环境变量或测试数据构造流程。除非用户明确另有要求，不得机械强制剪贴板授权、一次性密码、每次重新生成、Secret Manager、人工输入、人工交互或每次向用户询问。
13. 固定测试凭据可以从项目约定的本地忽略文件读取，但不得复制到 Git 跟踪文件、日志、生成物、最终报告或提交记录，也不得随 commit 进入版本库；具体 Codex 指令仍可填写允许和禁止的具体数据库名，但不得回显密码或完整连接串。
14. 固定测试凭据不要求仅为了保密而在每一步或命令结束后立即清空。`standard_test` 与 `browser_acceptance` 的配置不得叠加加载到同一进程；同一 shell 将切换数据库用途或被复用时，必须优先启动独立进程，或在切换前清除或覆盖数据库主连接变量、管理连接变量、项目定义的数据库用途变量及其他用途相关变量。具体变量名必须从项目级 testing playbook、项目环境约定、任务声明或用户明确输入中解析；通用规则不得硬编码项目专属变量名。该清理是防止串库的数据库隔离要求，不是密码保密仪式，也不得依赖旧变量或后加载覆盖行为选择数据库。
15. 上述有限规则不削弱第 10 条的默认严格保护范围；`development`、`production_or_operations`、真实业务数据环境、可访问非隔离环境或真实数据的凭据，以及未经授权的其他 Secret，仍必须按严格保护对象处理。
