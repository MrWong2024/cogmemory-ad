...
14. 固定测试凭据不要求仅为了保密而在每一步或命令结束后立即清空。`standard_test` 与 `browser_acceptance` 的配置不得叠加加载到同一进程；同一 shell 将切换数据库用途或被复用时，必须优先启动独立进程，或在切换前清除或覆盖 `MONGO_URI`、`MONGO_ADMIN_URI`、`COGMEMORY_DATABASE_PURPOSE` 及其他用途相关变量。该清理是防止串库的数据库隔离要求，不是密码保密仪式，也不得依赖旧变量或后加载覆盖行为选择数据库。
