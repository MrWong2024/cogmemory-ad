...
- 不默认增加用户未要求的自动 retry、自动 merge、自动 replay、queue、lease、distributed lock、多套 revision、mutation coordinator 或多人实时协同编辑。同一业务聚合内写操作串行优先不等于系统全局串行；不同患者、访视、量表实例和患者施测会话仍可正常并行，读操作正常并发。
