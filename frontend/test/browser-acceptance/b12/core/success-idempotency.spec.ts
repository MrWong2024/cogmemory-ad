...
test.describe("B12 core / success-idempotency", () => {
  ...
  test("doctor-lock-success owns B12-33 through B12-40, B12-44, and B12-46 through B12-48", async ({
    roleContexts,
  }) => {
  ...
  test("admin-lock-success owns B12-45 and supporting first-lock response evidence", async ({
    roleContexts,
  }) => {
    test.skip(!environment.enabled, "B12_BROWSER_ACCEPTANCE_RUN=1 is required");
    if (!environment.enabled) return;
  ...
  test("already-locked-idempotency owns B12-41 through B12-43", async ({
    roleContexts,
  }) => {
    test.skip(!environment.enabled, "B12_BROWSER_ACCEPTANCE_RUN=1 is required");
    if (!environment.enabled) return;
    ...
    await runB12CoreRoute(
      ...
      async (run) => {
        ...
        await expect(
          primary.page
            .getByText(B12_NEUTRAL_TEXT.secondaryLock, {
              exact: true,
            })
            .first(),
        ).toBeVisible();
