...
export const conflictOwnerActions = Object.freeze({
  "core-workflow/conflict/lock-conflict-continue": async ({ run }) => {
    const session = await run.primary();
    const initialUpdatedAt = session.initialUpdatedAt();
    const form = await openLockForm(session.page, B12_NEUTRAL_TEXT.conflictLock);
  ...
  "core-workflow/conflict/lock-conflict-latest-locked": async ({ run }) => {
    ...
    await primary.page.getByRole("button", { name: "取消", exact: true }).click();
    await expect(
    ...
    await expect(
      primary.page
        .getByText(B12_NEUTRAL_TEXT.latestLockedSecondary, { exact: true })
        .first(),
    ).toBeVisible();
