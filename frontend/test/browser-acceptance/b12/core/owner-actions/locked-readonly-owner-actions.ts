...
export const lockedReadonlyOwnerActions = Object.freeze({
  "core-workflow/locked-readonly/locked-readonly-semantics": async ({ run }) => {
    ...
    await expect(reportNarrativeSections(session.page).first()).toBeVisible();
    await expect(reportNarrativeSections(session.page).last()).toBeVisible();
    await expect(
